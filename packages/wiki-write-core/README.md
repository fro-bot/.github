...
The build also emits `dist/gate-contract.json` from `GATE_CONTRACT_VERSION`. Downstream writers can fetch this marker through the repository contents API; the same distribution drift check verifies that the committed marker matches the source constant.
