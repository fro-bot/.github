...
export function resolveMarkdownLinks(content, containingFile, options = {}) {
    const links = [];
    let fence;
    for (const [lineIndex, line] of content.split('\n').entries()) {
        ...
        for (const match of line.matchAll(MARKDOWN_LINK_PATTERN)) {
            ...
            const resolvedPath = normalize(pathTarget.startsWith('/') ? pathTarget.slice(1) : join(dirname(containingFile), pathTarget)).replaceAll('\\', '/');
            const exists = options.files === undefined
