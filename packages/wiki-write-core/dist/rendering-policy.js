const UNSAFE_TAG_PATTERN = /<\/?\s*(?:script|iframe|object|embed|applet|base|form|input|button|textarea|select|option|meta|link|style|template|svg|math)\b[^>]*>/iu;
const EVENT_HANDLER_PATTERN = /\s+on[a-z][\w:-]*\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/iu;
const UNSAFE_URI_PATTERN = /\s+(?:href|src|srcset|action|formaction|xlink:href|poster|background|cite)\s*=\s*(?:"\s*|')?(?:javascript|vbscript|data):/iu;
/** Return save-side findings for HTML that the render policy will strip. */
export function validateRenderingPolicy(params) {
    const findings = [];
    const renderableMarkdown = maskCodeContent(params.content);
    if (UNSAFE_TAG_PATTERN.test(renderableMarkdown) ||
...
export function maskCodeContent(content) {
 ...
 */
export function maskNonProseContent(content) {
    ...
    return originalLines
        ...
        if (isBlockquoteLine)
            inBlockquote = true;
        else if (isBlank || (inBlockquote && isBlockStart))
            inBlockquote = false;
        if (isBlank)
        ...
        if (listMarker !== undefined && !isBlockquoteLine)
            listIndent = listMarker;
        if (isBlockquoteLine || inBlockquote || isIndentedCode)
            return originalLine.replaceAll(/[^\n]/gu, ' ');
        return maskedLine;
    })
        .join('\n');
}
