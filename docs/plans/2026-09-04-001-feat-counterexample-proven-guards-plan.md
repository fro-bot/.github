...
**Standing rule for 5A-2 onward:** before shipping any `Stryker disable next-line <Mutator>` directive, enumerate every variant that named mutator generates and confirm the reason addresses all of them, not just the one variant that happened to be the first one noticed. A directive whose reason only proves one variant equivalent is not a valid directive for that mutator.
