...
**Unit 5A-2 (`scripts/check-private-leak.ts`) — IN PROGRESS, not clean.** Baseline (run 34007429970): 326 non-clean (248 Survived, 76 NoCoverage, 2 Timeout) out of ~640 mutants. This PR's first pass, scoped `pnpm exec stryker run --mutate scripts/check-private-leak.ts`: **314 non-clean (233 Survived, 81 NoCoverage, 0 Timeout), 323 Killed, 3 Ignored** — net movement of only 12 mutants despite real fixes, because the bulk (138 StringLiteral, 30 BlockStatement, 63 ConditionalExpression) is still unaddressed. Landed in this pass:
- The 2 Timeout mutants at the missing-node-id sentinel loop (`UpdateOperator i--`, `EqualityOperator i >= missingNodeIdCount`): line-scoped directives. Verified `<=`/`>` (the other two `EqualityOperator` variants for `<`) are already Killed by the existing exact-sentinel-count assertions (absent from the baseline survivor list) — the directive's reason does not need to re-argue their equivalence, only `>=`'s deterministic-hang case (`missingNodeIdCount === 0` makes `i >= 0` true forever with `i` monotonically non-decreasing).
- `isGh404Error`: 3 new tests (isRecord() false branch via a non-object thrown value; stdout/stderr present but non-string; a single payload with whitespace on both sides of the colon in `"status" : "404"` that discriminates both `\S*`-swapped Regex variants from the real `\s*` pattern in one assertion). Kills all of ConditionalExpression/LogicalOperator/Regex on this function — confirmed via the scoped run (Regex mutator count: 2 → 0).
- 4 inverse-control tests added to `packages/wiki-write-core/src/private-leak.test.ts` (promised by the learning doc): `diff --git a/... b/...`, `rename to <path>`, and `copy to <path>` text appearing as `+`-prefixed added content are all scanned, not misread as structure — all three pass green (no defect found; the parser's `+`-prefix-first-character property makes these grammar-safe by construction, now pinned directly). A fourth test **documents a real, currently-open gap, not fixed here**: a genuine `@@ ... @@` hunk marker's own trailing context text (e.g. `@@ -1,2 +1,2 @@ function secret-repo() {`) is never scanned, because it is filtered by the unconditional `if (!line.startsWith('+')) continue` before reaching the content path regardless of the `@@` branch's own fall-through-vs-continue choice — out of scope for a mutation-coverage pass (behavior change).

**Remaining, not done in this PR:** the StringLiteral contract/prose split (138 mutants), the `main()`/`runPromotionCli()` default-seam ArrowFunction survivors (`defaultWorkflowRunReader`, `defaultGitDiffRunner`, `defaultReposYamlReader`, `defaultResolverFactory` — none of the existing tests exercise the un-injected default path), the `readWorkflowRunContext`/`fetchPrivateNodeIds`/`runPromotionScan` BlockStatement/ConditionalExpression/ObjectLiteral survivors, and the `import.meta.url === ...` CLI self-invoke guard (survived here in a way check-wiki-authority.ts's structurally identical idiom does not — root cause not yet isolated; flagged for the next pass rather than directived away without understanding why). Follow-up PR required before this module can report 0/0/0.

Gate for this partial pass: `pnpm check-types`, `pnpm lint`, `pnpm test` (75 files, 3109 tests + 3 todo, up from 3102+3), `pnpm exec vitest run scripts/mutation-guards-config.test.ts` (26/26). Overall verdict remains `mutant-timeout` (corrections-survival.ts's 5 timeouts untouched; this module also not yet clean).
