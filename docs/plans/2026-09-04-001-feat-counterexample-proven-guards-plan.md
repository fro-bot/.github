...
**Follow-up (#3838, PR fix/private-leak-plus-header-bypass):** the 2 remaining `Ignored` directives above (both on `checkPathAsNew`'s state) turned out to be a symptom of a real defect, not equivalence — filed as #3838 in review: `line.startsWith('+++')` treated ANY line starting with three `+` characters as a diff header, so an added content line whose own text begins with `++` (e.g. `+++ see acme/secret-repo...`) escaped the content scan entirely, a false negative in this Tier 0 disclosure guard. Fixed per the issue's pairing option: a `+++` line is now a header only when the immediately preceding line was a `--- ` line, tracked with one `pendingNewFileCheck: boolean | undefined` flag (replacing the old persistent `checkPathAsNew` boolean) that is set only inside the `--- ` branch and read only at the top of the very next iteration. That refactor made the persistent `checkPathAsNew` variable's every reset (initial value, both `diff --git` branches, the `+++` handler's own reset) provably dead — all deleted along with both `Ignored` directives, rather than re-justified. Result: **118 Killed, 0 Survived, 0 NoCoverage, 0 Timeout, 0 Ignored** — 118 total mutants, 0 directives (down from 119/2). `pnpm test`: 75 files, 3041 tests + 3 todo (up from 3037+3).
