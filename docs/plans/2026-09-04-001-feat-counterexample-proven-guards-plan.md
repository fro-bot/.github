...
**Result (Unit 5A-2 — `scripts/check-private-leak.ts`): CLEAN.** Baseline (run 34007429970): 326 non-clean (248 Survived, 76 NoCoverage, 2 Timeout) out of ~640 mutants. A first WIP pass (commit `9efaecf`) landed the sentinel-loop timeout directives, `isGh404Error`'s remaining kills, and four `private-leak.test.ts` inverse-control tests, moving the module to 314 non-clean (233 Survived, 81 NoCoverage, 0 Timeout). This pass finished the module to **580 Killed, 0 Survived, 0 NoCoverage, 0 Timeout, 18 Ignored (directives)** — 598 total mutants.

| | Baseline (34007429970) | WIP (`9efaecf`) | Final |
|---|---|---|---|
| Killed | — | 323 | 580 |
| Survived | 248 | 233 | 0 |
| NoCoverage | 76 | 81 | 0 |
| Timeout | 2 | 0 | 0 |
| Ignored (directives) | — | 3 | 18 |
| Total mutants | ~640 | ~637 | 598 |

**Test count:** `check-private-leak.test.ts` grew from 113 tests (on `main`) to 183 (+70 across both passes; this pass alone added roughly 45 on top of the WIP commit's baseline). `pnpm test`: 75 files, 3176 tests + 3 todo (up from the WIP pass's 3109+3, and from 3102+3 before Unit 5A-2 started).

**Directive count by reason class (18 total, each covering every variant its named mutator(s) generate):**
1. **Unobservable fallback/seed value (4):** `isGh404Error`'s two `''` string fallbacks (`stdout`/`stderr` defaults, each observed only through two fixed regexes that never match the Stryker placeholder text) plus two `ArrayDeclaration` seed-pollution guards — `privateTokens: string[] = []` at both its independent construction sites (`main()` and `runPromotionCli()`) — seeded with `['Stryker was here']` contributes zero scan tokens because `buildPrivateNameTokens` requires a `/` the seed text lacks, proven with a dedicated `"Stryker was here"`-containing diff-line test at each call site.
2. **Unreachable-by-construction defensive invariant (5):** `OPERATOR_LOGIN`'s literal value (compared only after `author` is already provably non-empty); `validCandidates[0] === undefined` after `validCandidates.length !== 1` already threw; `r.node_id.length === 0` after `assertRepoEntry`'s schema layer already rejects empty-string `node_id` (both the pure-core and the `runPromotionCli` logging-path copies of this check); `'matchedFiles' in result` after both other `PromotionScanResult` variants were already returned above.
2b. **Type-contract exhaustiveness (2):** `GuardResult`'s `!scanResult.ok && 'matchedFiles' in scanResult` (the two clauses always agree per the type's own shape) and `PromotionScanResult`'s equivalent `resolutionFailed` branch.
3. **Deterministic-hang prevention (2):** the sentinel loop's `UpdateOperator`/`EqualityOperator` pair carried over from the WIP pass, re-verified against the variant-enumeration rule.
4. **Redundant downstream duplicate-check (1):** `fetchPrsByHeadSha`'s `.filter(isRecord)` — kept for the exported `PrApiResolver` interface's type contract (not deleted, since other implementations/callers may rely on the declared return type), but unobservable through this repo's own call site because `extractNumbers` re-applies the identical `isRecord` predicate.
5. **Single-variant/no-override boolean (1):** the promotion path's `checkPrivateLeak(privateTokens, diff, {titlePrefixed: false, isOperator: false})` call — no override is ever offered on the operator-supervised scheduled path, so flipping either boolean alone, or emptying the object, cannot change the no-override outcome.
6. **Operator-facing diagnostic prose (3):** the three remediation-instruction `process.stderr.write` calls at the end of the PR-diff detection path (`GH_TOKEN=<operator-PAT> node scripts/resolve-private.ts ...`, the node_id table note, the bypass instructions) — human-readable only, not parsed, not asserted, does not affect classification/exit status/redaction.
...
**Restructures + neutrality proofs (2):**
1. `extractPrivateNodeIds`'s inline `r.node_id.length > 0` filter clause extracted to a named `hasNonEmptyNodeId` predicate, reused at both of the module's two independent `.filter(r => hasNonEmptyNodeId(r))` call sites (the pure `extractPrivateNodeIds` core and `runPromotionCli`'s parse-for-logging path) — neutral because the extraction changes no control flow, only names an already-duplicated boolean expression once.
2. `runPromotionCli`'s `privateNodeIds: string[]` (mapped `.node_id` values) renamed/narrowed to `privateNodeIdCount: number` (`.length` only) — neutral because every read of the original variable in this scope was already count-only (`missingCount` subtraction, the final "ok (scanned N ...)" log); no code path in `runPromotionCli` ever read a resolved node_id *value* off it. This also closed an `ArrowFunction` survivor: the mapped-string variant made `.map(r => r.node_id as string)` a killable-in-principle but practically-unobservable arrow (nothing downstream read the mapped values), which the count-only refactor eliminates by construction rather than directiving away.

**Line-1206 self-invoke-guard root cause (resolved, no longer a survivor):** the guard (`import.meta.url === \`file://${process.argv[1]}\``) was fully covered by three dedicated tests already present from the WIP pass's later work, not written in this session: each dynamically re-imports the module with a cache-busting query string (`?guard-test-main`, `?guard-test-promotion`, `?guard-test-noop`) after setting `process.argv = [node, modulePath.pathname, ...]` to force the guard's condition to actually evaluate `true` for the *real* code (every other test in the file imports the module without ever matching `process.argv[1]`, which only trivially covers the `false` branch identically for every mutant). The `--promotion` branch and the plain-`main()` branch are each asserted via their own unmistakable failure-message signature (`FRO_BOT_POLL_PAT not set. This is required for promotion mode` vs `GITHUB_EVENT_PATH not set`), and a fourth non-matching-argv test proves the guard does NOT fire when `process.argv[1]` differs. `check-wiki-authority.ts`'s structurally identical guard was never actually compared line-by-line in this pass because it was unnecessary: this file's guard is now itself 100% killed (`ConditionalExpression` true/false, `EqualityOperator` swap, and the template-literal `StringLiteral` variant are all `Killed`, confirmed in the final scoped Stryker run) via the mechanism above, not via a directive.

Gate for this final pass: `pnpm exec eslint --fix scripts/check-private-leak.ts scripts/check-private-leak.test.ts` (4 pre-existing lint errors surfaced and fixed — two `expect.any(Object)` unsafe-assignment casts, one function hoisted out of a `describe` block per `unicorn/consistent-function-scoping`, one `no-template-curly-in-string` describe title); `pnpm check-types` (clean); `pnpm lint` (clean); `pnpm test` (75 files, 3176 tests + 3 todo, up from 3109+3); `pnpm exec vitest run scripts/mutation-guards-config.test.ts` (26/26). Live `pnpm check:mutation-guards` full run confirms `check-private-leak.ts` at 100.00%/100.00%, 580/0/0/0/0 (Killed/Survived/NoCoverage/Timeout/Errors) in the per-file table. Overall verdict remains `mutant-timeout` as expected (`corrections-survival.ts`'s timeouts and the other unfinished modules — `wiki-context-safety.ts`, `wiki-lockfile-gates.ts`, `check-wiki-private-presence.ts` — are untouched by this PR and still report Survived/NoCoverage mutants; Unit 5A-2 scoped this pass to `check-private-leak.ts` only).
