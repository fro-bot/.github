...
**Result:** `mutation-guards.json` created at repo root holding `not-mutated: [{path, reason}]`; `scripts/mutation-guards-config.test.ts` created with the five tests. `stryker.config.json`'s `mutate` grew from 10 to 15 entries and `testFiles` from 10 to 12 after reading every module the initial disposition deferred: `packages/wiki-write-core/src/schemas.ts`, `wiki-ingest.ts`, and `wiki-slug.ts` promoted to `mutate` (each has a real throw/reject path — `assertReposFile`, `WikiIngestError`, `computeRepoSlug` — consumed by an already-mutated gate: `private-leak-adapter.ts`'s metadata validation, `wiki-ingest.ts`'s own INVALID_WIKILINK/CORRECTION_ERODED gate, and `wiki-ingest.ts`'s slug resolution respectively); each is already same-tree covered by existing `testFiles` entries (`wiki-write-core.test.ts` via the `index.ts` barrel, `private-leak-adapter.test.ts` via a direct import), so no new `testFiles` entries were needed for those three. Two `scripts/check-*.ts` files the origin research missed — `check-md-links.ts` and `check-wiki-private-presence.ts` — read as genuine gates structurally identical to the already-mutated `check-private-leak.ts` (classification logic + `main()` with `exitCode`), promoted to `mutate` with their existing same-tree tests (`check-md-links.test.ts`, `check-wiki-private-presence.test.ts`) added to `testFiles`. `wiki-lint.ts` moved to `not-mutated` with the relocation reason as directed (Unit 5 still owns the flip to `mutate`). Final disposition: 7 `not-mutated`-with-reason package modules with no gate consuming their rejection path (`frontmatter.ts`, `rendering-policy.ts`, `wiki-utils.ts`, `markdown-links.ts`, `correction-text.ts`, `data-branch-bootstrap.ts`, `gate-contract.ts`) plus `index.ts` (pure barrel) and `wiki-lint.ts` (relocation pending) — 9 package `not-mutated` entries total; 1 scripts `not-mutated` entry (`check-mutation-guards.ts`, per its Unit 2 disposition). `readStrykerConfig`, `isLiteralPath`, and `MINIMATCH_METACHARACTER_PATTERN` exported from `scripts/check-mutation-guards.ts` for reuse (previously module-private); `readStrykerConfig` extended to also parse and return `testFiles` (previously omitted, needed for Tests 3 and 4). All five discrimination proofs ran red before the corresponding fix and green after, with every temp edit (a scratch `packages/wiki-write-core/src/new-gate.ts`, a scratch `scripts/check-new-thing.ts`, a scratch `scripts/helper.ts` pattern-miss check, a scratch `scripts/corrections.test.ts` cross-tree pairing exercised through the exported pure `findCrossTreePairingViolations` helper rather than a real cross-tree file move, a temporary `testFiles` entry, and a temporarily blanked `not-mutated` reason) reverted before landing. Gate: `pnpm check-types`, `pnpm lint`, and `pnpm test` (73 files, 2940 tests, 3 todo) all pass; `node -e "import('./scripts/check-mutation-guards.ts')"` remains inert.
