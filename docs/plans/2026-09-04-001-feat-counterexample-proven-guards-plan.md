...
- Line 77 (`if (destination !== '')`): `ConditionalExpression,StringLiteral` — `checkPath('')` is a no-op given `privateNames` never contains an empty string in real usage.
...
- Line 100 (`line.slice(1).toLowerCase()`): `MethodExpression` — dropping `.slice(1)` only leaves the leading `+` character in `content`, which can never create or hide a substring match since no real private name contains `+`; this directive also incidentally suppresses a second, already-killed `MethodExpression` variant on the same line (`.toLowerCase()` → `.toUpperCase()`) since Stryker's `next-line` scoping is per-line, not per-sub-expression — that variant stays protected by the case-insensitivity test under ordinary `pnpm test` regardless of whether Stryker still generates it.

No mutants could not be resolved. Live `pnpm check:mutation-guards` run confirms the module's own per-mutant counts above; `pnpm test`: 75 files, 3034 tests + 3 todo (up from 3002+3).
