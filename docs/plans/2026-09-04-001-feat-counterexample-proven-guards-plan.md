...
**Result (Unit 5A-1 — `private-leak.ts`):** Baseline 88 non-clean (52 Survived, 36 NoCoverage, 0 Timeout, CI run 34007429970). This module had no dedicated same-tree source exerciser under Stryker: `private-leak-adapter.test.ts` only covers it through the adapter's one fixed diff shape, `wiki-write-core.test.ts` has one `it()`, and `regex-redos-regressions.test.ts`'s richer direct calls are deliberately excluded from `testFiles` (Unit 3's Test 4). Added `packages/wiki-write-core/src/private-leak.test.ts` (34 tests: override-bypass AND-not-OR, empty-input early return, `diff --git a/X b/Y` path extraction including the separator-index boundary and fallback cases, `rename to `/`copy to ` destinations, `--- `/`+++` new-file detection including a stale-state-across-a-malformed-diff-git-line case, `+`-added-line content scanning, `checkPath` multi-name and de-duplication behavior, case-insensitivity, and `checkPrivateLeakWithAdapter`'s param forwarding), registered in `stryker.config.json`'s `testFiles`. Result: **122 Killed, 0 Survived, 0 NoCoverage, 0 Timeout, 14 Ignored (directives)** — module fully clean. Overall verdict stays `mutant-timeout` (other 8 modules untouched).

Mutant locations killed by the new tests, grouped by what they prove: override bypass requires both `titlePrefixed` AND `isOperator` (line 30 conditional/logical); `diff --git a/X b/Y` separator-index parsing including the exact `separatorIndex === diffPrefix.length` boundary (lines 57-63); `rename to `/`copy to ` OR-not-AND and empty-destination handling, INCLUDING an empty rename destination when `privateNames` itself contains `''` (lines 73-83, 2nd pass); `--- /dev/null` vs `--- a/X` new-file detection, including the case where a malformed subsequent `diff --git` line must reset stale `currentFile`/`checkPathAsNew` state (lines 61-92); `checkPath`'s `.some` (not `.every`) multi-name matching and its own de-duplication guard (lines 43-48); content-scan case-insensitivity AND the leading-`+`-stripping itself, given a private name starting with `+` (line 100, 2nd pass); `checkPrivateLeakWithAdapter`'s exact `{content, snapshotSha}` forwarding (line 117).

**Review follow-up (2nd pass):** two of the original ten directives were not equivalence claims but unstated assumptions about `privateNames`'s contents — read `schemas.ts`'s `assertRepoEntry`/`isRepoEntry` directly to check: neither `owner` nor `name` (nor any derived private-name string reaching this module) carries a character-class or non-empty constraint comparable to `node_id`'s `NODE_ID_PATTERN` or the allowlist's `CONTRIB_REPO_PATTERN` — both fields are only `typeof === 'string'`-checked. So the assumptions ("`privateNames` never contains `''`", "no real repo/org name contains `+`") are NOT enforced upstream and were not legitimate directives. Both killed with real tests instead: `''` in `privateNames` against an empty `rename to ` destination (would spuriously match via `''.includes('')`), and a `'+leaked'` private name against `+`-prefixed added-line content (would spuriously match only if the leading `+` were left in). Final directive count: **8 `next-line` directives covering 14 distinct mutants** (unchanged from before except the two removed entries below).
- Line 34 (`privateNames.length === 0 || diff.length === 0`): `ConditionalExpression,LogicalOperator,BlockStatement` — whichever side triggers the early return, `privateNames.length === 0` forces `lowerNames = []`, so no downstream `.some(...)` can ever match; falling through instead of returning early converges to the same `{ok: true}`.
- Line 41 (`let checkPathAsNew = false`): `BooleanLiteral` — currentFile only becomes non-null via a branch that also resets `checkPathAsNew`, so this initial value is never read while `currentFile !== null`.
- Line 58 (extracted to its own `const hasRoomAfterSeparator = separatorIndex + separator.length < line.length` line specifically so this directive cannot also suppress the left clause's mutants, which are genuinely killed): `ArithmeticOperator,EqualityOperator,ConditionalExpression` — structurally redundant; `lastIndexOf`'s own bound argument already guarantees this clause whenever `separatorIndex !== -1`.
- Line 68 (else-branch `checkPathAsNew = false`): `BooleanLiteral` — `currentFile` is reset to null in the same branch, blocking any read of this value until the next `diff --git a/` line resets it again anyway.
- ~~Line 77 (`if (destination !== '')`)~~ — **removed**; killed with a real test (`privateNames: ['']` against an empty rename destination).
- Line 92 (post-`+++`-handler `checkPathAsNew = false`): `BooleanLiteral` — same reasoning as line 68; `currentFile` can only change via a `diff --git a/` line, which always resets this too.
- ~~Line 100 (`line.slice(1).toLowerCase()`, `MethodExpression` dropping `.slice(1)`)~~ — **removed**; killed with a real test (`privateNames: ['+leaked']` against `+`-prefixed added-line content). The line still carries an already-killed sibling `MethodExpression` mutant (`.toLowerCase()` → `.toUpperCase()`), now independently re-verified by Stryker on every run rather than incidentally suppressed by a directive scoped to the same line.

No mutants could not be resolved. Live `pnpm check:mutation-guards` run confirms the module's own per-mutant counts above (122 Killed / 0 non-clean / 14 Ignored); `pnpm test`: 75 files, 3036 tests + 3 todo (up from 3002+3, +2 from the 2nd-pass killing tests).
