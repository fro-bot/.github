...
| A regex literal's Regex-mutator variant is itself syntactically invalid, and Stryker embeds every mutant of a regex literal as a literal alternative in a ternary chain (not a string compiled later) — one invalid variant fails to *parse*, at load time, independent of which mutant is active. Confirmed on `packages/wiki-write-core/src/wiki-ingest.ts:706`: the instrumented sandbox turns `\v` into `\V` inside a character class (`Invalid regular expression: ... Invalid escape`), which is not a valid RegExp escape. Because Vite's dependency scan processes every requested mutate-set file together, this silently zeroes test collection (`DryRunExecutor` logs "No tests were found") for *every* test file in that run, not only ones that import the broken module — this is what produced Unit 3's corrections.ts/corrections-survival.ts coverage loss once `wiki-ingest.ts` entered `mutate`. Observable signature: zero tests collected and a thrown `ConfigError('No tests were executed...')` with **no report file written at all**, not a `NoCoverage` mutant inside a produced report — the wrapper's existing "report file missing → `instrumentation-failed`, never `clean`" rule (Unit 2) already covers this case correctly; no wrapper change is needed. | Not a config error — confirmed by reproducing with a scratch `mutate: [corrections.ts, wiki-ingest.ts]` / `testFiles: [corrections.test.ts]` config: 0 tests collected; adding a temporary `// Stryker disable next-line Regex` directive on wiki-ingest.ts:706 restored normal collection (15 tests). This is why `wiki-ingest.ts` stays `not-mutated`, not merely for the dist/relocation reason: Unit 5, when relocating its test and flipping it to `mutate`, must also add a permanent `// Stryker disable next-line Regex` directive (with a reason) on this line, or Stryker must ship a fix upstream, before this file can safely enter `mutate`. |
