...
| Writer key leaks or requires rotation | Routine: create the new key, deploy consumers, verify minting, then revoke the old key; GitHub permits up to 25 non-expiring private keys per App, so overlap avoids an outage. Leak: revoke immediately, explicitly revoke active installation tokens with `DELETE /installation/token` or uninstall the App installation, remove the old mount, deploy the replacement, and rerun canary/tip-identity checks. The private key signs the JWT used to mint tokens; key revocation alone stops new minting but does not invalidate already-issued bearer installation tokens, valid for up to 1 hour. Pulling the writer key pauses operator saves but does not interrupt autonomous survey writes, which use their separate credential |
