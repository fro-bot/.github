  ...
  "candidates": [
    {
      ...
      "description": "Validated wiki change building, node_id identity/migration, index/log regeneration, Git Data API commit with conflict retry for autonomous survey writes and data-branch bootstrap. The operator write path must use a non-retrying variant or re-validate the edit snapshot on every retry attempt; the autonomous path keeps its existing retry semantics.",
      "disposition": "extend"
    },
    {
...
- **Page snapshots, not blob SHAs, are the concurrency contract**: `GET` returns an opaque edit-snapshot `ETag` covering page version and page-relevant correction state; the blob SHA is an input to that snapshot, not the contract itself. The edit-snapshot `ETag` is a strong validator — never `W/"..."` — because `If-Match` requires strong comparison. Writes require `If-Match`: a missing precondition returns 428; a supplied but stale or nonmatching validator returns 412 with the draft preserved; and a representation change before commit is still a lost validator and returns 412 with the draft preserved. 409 is reserved for genuine semantic conflicts, such as the page being deleted between read and write or page identity migrating (`node_id` moved). Compare-and-write must be atomic: build the commit object against the observed head with `parents: [observed head SHA]`, then call the Git Data API ref update with `force: false`; fast-forward-only enforcement supplies the compare-and-write property. A non-fast-forward `updateRef` returns 422, and that failure must map to the plan's 412 response, while a separate check-then-write sequence would recreate the race the validator prevents. `node_id` is identity, not version. Ambiguous GitHub outcomes reconcile through the idempotency key, current blob, and commit correlation metadata; classify each outcome as succeeded, failed, or indeterminate, and persist and surface indeterminate outcomes for resolution rather than silently discarding them as failures. A matching current state is not proof that this write landed because another actor can produce identical content; correlation metadata is load-bearing. Never blind retry.
...
- **No session step-up (supersedes the original requirement)**: this plan originally required that writes carry authentication issued within the last 30 minutes, described against the dashboard's 24-hour signed cookie. Both halves were wrong about the deployed system. Production runs the dashboard in gateway mode (`DASHBOARD_GATEWAY_OPERATOR_SESSION_ENABLED=true`), where the gateway operator session is the single auth authority and the dashboard's own signed cookie is never minted. In that mode the requirement is not implementable: `OperatorSessionInfo` is a frozen v1.0.0 contract carrying `{operatorId, login, expiresAt}`, and `expiresAt` is defined as the sooner of absolute or idle expiry — so an idle-refreshed session is indistinguishable from a freshly authenticated one, and auth age cannot be derived. Step-up is therefore dropped rather than approximated. The gateway's 30-minute idle expiry stands as the freshness bound: any accepted write comes from a session active within that window. This is acceptable because a wiki edit is a git commit — reversible, path-allowlisted, gate-validated, attributed, and restricted to a single allowlisted operator — with CSRF, origin binding, and rate limiting already covering session abuse. Rejected alternatives: bumping the frozen contract to add `issuedAt`, which buys a marginal control at the cost of a cross-repo release dependency; and deriving auth age from `expiresAt` minus the gateway's absolute TTL, which hardcodes another repository's constant into a security check that would then degrade silently if that constant ever changed. Wiki CSRF tokens still bind to session identifier, action, and a bounded time window.
...
- **Draft persistence is required**: drafts survive session expiry, 412 responses, and network failures.
...
**Planning note:** `fro-bot/dashboard` follows its existing React-state view switching in `web/src/App.tsx`; it does not introduce a router. The API revalidates the session per request, enforces origin/CSRF, request/body limits, authenticated and page-scoped rate limits, strong opaque edit-snapshot `ETag`/`If-Match` semantics (428 when missing, 412 for stale/nonmatching validators or a lost-validator race, 409 only for semantic deletion or identity-migration conflicts), server-derived attribution, draft persistence, and pending-state UX. Established mutation posture from `web/src/push/subscribe.ts` is mandatory: `credentials: 'include'`, `redirect: 'error'`, CSRF fetched before mutations, and `idempotency-key`. Do not copy its bounded POST retry; GitHub commits are not safely retryable. The dashboard performs no GitHub writes and never receives the Fro Bot App key; it authenticates and calls Unit 7b. The `AGENTS.md` write-path invariant must be updated before this unit starts.

**Test scenarios:** authenticated editor load and save; unauthenticated invisibility; missing/invalid CSRF and origin; missing `If-Match` → 428, stale/nonmatching `If-Match` and lost-validator race → 412 with draft preservation, semantic deletion or identity-migration conflict → 409; body and envelope limits; rate-limit boundaries; session-expiry response with draft preservation; network failure with draft preservation; idempotency key propagation; no retry after an ambiguous writer response; pending state after accepted commit; route contract matches Unit 6.
...
**Planning note:** `marcusrbrown/infra` adds the writer service to `apps/dashboard/docker-compose.yaml`, which currently runs `caddy` and `dashboard` pulling `ghcr.io/fro-bot/dashboard`. The writer remains on the dedicated private network, has no public port or Caddy route, mounts the separately managed Fro Bot App key only into the writer, and receives the dashboard image/deploy updates. No new droplet and no new public origin. `marcusrbrown/infra` owns operational rotation and revocation of the writer key and maintains an inventory recording which key is deployed where. Rotation and revocation are a two-party operation with `fro-bot/dashboard`: dashboard owns the writer image/config changes and infra owns the deployed secret plus GitHub App revocation. **Routine rotation** is create the new key, deploy consumers, verify minting works, then revoke the old key; GitHub permits up to 25 private keys per App and keys do not expire, so overlapping keys are the supported path and avoid an unnecessary minting outage. **Leak response** is different: revoke the compromised key immediately and suspend the App installation, which is the preferred containment step over uninstall. A suspended installation's tokens fail immediately, and suspension is reversible without re-consenting permissions or re-establishing the installation ID that `secrets.APPLICATION_PRIVATE_KEY` consumers depend on; uninstall achieves the same containment with a much worse restore path. `DELETE /installation/token` only revokes the token presented in that request's own `Authorization` header, so it is routine-rotation hygiene, not leak containment. The private key signs the JWT used to mint tokens; revoking or deleting it stops new minting but does not invalidate already-issued bearer installation tokens, which remain valid for up to one hour from issue. Remove the old key from the infra secret mount and writer deployment, deploy the replacement, unsuspend the installation, and keep operator saves disabled until the canary and tip-identity checks pass — the canary authenticates with an installation token, so it cannot run before the unsuspend. Pulling the writer key during routine rotation does not interrupt autonomous survey writes: those 16 workflows authenticate with `secrets.APPLICATION_PRIVATE_KEY`, a distinct key object on the same App, so revoking the writer's key pauses only operator wiki writes until replacement validation completes. That routine-path reassurance does not apply to a leak response. A leaked writer key forces a choice between a one-hour uncontained window and a control-plane outage: installation suspension takes the entire control plane dark — survey, promotion, reconcile, and every other autonomous loop — while revoking the key alone does not invalidate already-issued bearer installation tokens. That containment holds at the installation tier, not the key tier. The runbook must name installation suspension as this escalation tier and state that it takes the entire control plane dark. Verify the save path after deployment, including the Fro Bot tip-identity check.
...
- **State lifecycle risks:** corrections store must survive page migrations (node_id-keyed, tested in Unit 3); ambiguous commit outcomes (timeout after ref update) are reconciled through the idempotency key, current blob, and commit correlation metadata, then classified as succeeded, failed, or indeterminate. An indeterminate outcome is persisted and surfaced for resolution, never silently reported as failure and discarded; matching the desired current state alone is not proof that this write landed because another actor can produce identical content.
...
| Writer key leaks or requires rotation | Routine: create the new key, deploy consumers, verify minting, then revoke the old key; GitHub permits up to 25 non-expiring private keys per App, so overlap avoids an outage. `DELETE /installation/token` is clean-rotation hygiene only: it revokes the token presented in that request's `Authorization` header and cannot enumerate or revoke tokens minted by an adversary. Leak: revoke immediately and suspend the App installation, preferred over uninstall; suspended-installation tokens fail immediately, suspension is reversible without re-consenting permissions or re-establishing the installation ID, and uninstall has a much worse restore path. A leaked writer key forces a choice between a one-hour uncontained window and a control-plane outage: installation suspension takes survey, promotion, reconcile, and every other autonomous loop dark, while key revocation alone stops new minting but does not invalidate already-issued bearer installation tokens, valid for up to 1 hour. Remove the old mount, deploy the replacement, unsuspend, then rerun canary/tip-identity checks before re-enabling operator saves — the canary authenticates with an installation token, so it cannot run while the installation is suspended. Pulling the writer key pauses operator saves but does not interrupt autonomous survey writes only during routine rotation; that reassurance does not apply to leak containment |
...
| Race/ambiguous GitHub outcomes on save | Strong opaque edit-snapshot `ETag` + required `If-Match`; 428 when missing, 412 when supplied but stale/nonmatching or when the representation changes before commit (lost validator), 409 only for genuine semantic conflict such as deletion or `node_id` identity migration. Compare-and-write is atomic by building the commit object against the observed head with `parents: [observed head SHA]` and updating the ref with `force: false`; the non-fast-forward `updateRef` failure is 422 and must map to 412. Never recreate this with a separate check-then-write. Reconcile with idempotency key, current blob, and commit correlation metadata; classify succeeded/failed/indeterminate, persist and surface indeterminate outcomes for resolution, and do not treat matching current state as proof the write landed. Preserve the draft and never blind retry |
