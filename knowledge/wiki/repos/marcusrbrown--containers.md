...
sources:
  - url: https://github.com/marcusrbrown/containers
    ...
    accessed: 2026-04-18
tags: [docker, containers, dockerfiles, multi-arch, python, github-actions, ci-cd, security-scanning]
aliases: [containers]
related:
...
Both variants use multi-architecture build args (`TARGETPLATFORM`, `TARGETARCH`), run as non-root (`node` user), use `tini` as PID 1, and include health checks on `:3000/health`.
...
A `templates/` directory provides starter Dockerfiles for multiple stacks:

- `templates/base/alpine/` — Alpine base with docker-compose
- `templates/apps/nodejs/express/` — Express.js application
- `templates/apps/python/fastapi/` — FastAPI with docker-compose and requirements.txt
- `templates/databases/postgresql/` — PostgreSQL
- `templates/infrastructure/nginx/` — Nginx reverse proxy
- `templates/microservices/go/` — Go microservice with go.mod

Corresponding documentation lives in `docs/templates/` with per-template README files.
...
| --- | --- | --- |
| `containers` | `scripts.containers_cli:main` | Main CLI interface |
...
| `template-engine` | `scripts.template_engine:main` | Template rendering engine |
| `template-testing` | `scripts.template_testing:main` | Template validation |
| `generate-docs` | `scripts.template_documentation:main` | Documentation generation |
...
Dependencies include `openai` (>=2.32.0,<2.33.0), `anthropic` (>=0.30.0,<1.0.0), `jinja2`, `pyyaml`, `jsonschema`, `requests`.

### Node.js Layer

Minimal — only Prettier formatting via `@bfra.me/prettier-config/120-proof`. Managed by pnpm.

## Dockerfile Patterns

Both active Dockerfiles follow consistent best practices:

- **Syntax directive:** `# syntax=docker/dockerfile:1.23` with digest pin
- **Base image pinning:** Full `@sha256:...` digest pins on base images
- **OCI labels:** Follows OCI Image Spec annotations (title, description, version, vendor, source, licenses, base.name, base.digest). CI-injected labels (created, revision, version) are not hardcoded.
- **Build cache mounts:** `RUN --mount=type=cache` for package manager caches (`/var/cache/apk` or `/var/cache/apt`)
...
| Metrics Collector | `metrics_collector.yaml` | — | Container metrics collection |
...
All GitHub Actions are SHA-pinned with version comments. Key actions:
...
- `fro-bot/agent` — `v0.40.0` (SHA `4247f5e4...`)

## Fro Bot Integration

**Fro Bot workflow present** (`fro-bot.yaml`). Uses `fro-bot/agent@v0.40.0` with:

- **PR Review:** Container-specific review prompt focusing on Dockerfile best practices, multi-arch correctness, Python quality, Actions security, and breaking changes. Structured verdict format (PASS/CONDITIONAL/REJECT).
...
- **Auth:** `FRO_BOT_PAT` secret, `OPENCODE_AUTH_JSON`, model from `FRO_BOT_MODEL` var, plus `OMO_PROVIDERS` and `OPENCODE_CONFIG` secrets.
...
- **Poetry:** Build system (`poetry-core>=2.0.0`), dev deps include `pytest`, `pytest-cov`, `pytest-mock`, `black`, `isort`, `pylint`, `yamllint`.
- **Black:** line-length 88, target Python 3.13, excludes `templates/`.
- **isort:** Black-compatible profile.
- **Prettier:** `@bfra.me/prettier-config/120-proof` via pnpm.
...
- **mise:** Polyglot tool version manager for development environment.
- **Tests:** `tests/test_dockerfile_policy.py` — Dockerfile policy validation.
...
- **Template system:** A dual `templates/` + `docs/templates/` structure provides both runnable Dockerfiles and documentation for each stack template.
- **AI-powered tooling:** CLI includes LLM-backed commands (OpenAI and Anthropic) for template recommendations, project analysis, and interactive chat.
