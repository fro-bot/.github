...
- **Latest release:** `9.90.0` (2026-05-20)
- **Stars / Forks / Watchers:** 2 / 1 / 1
- **Open issues:** 60 (consistent with a long-lived autoheal / Renovate dependency dashboard)
...
2. **`configure`** — Bash step (`bash -Eeuo pipefail`) that:
   - Pins `RENOVATE_VERSION=43.186.2` (Renovate v43) with a `# renovate: datasource=docker depName=renovate packageName=ghcr.io/renovatebot/renovate versioning=semver` comment so Renovate self-bumps it.
...
- **Mode resolution:** Inline Bash in the `Determine mode and prompt` step maps event type → mode (schedule cron `15` → maintenance, schedule cron `03` → autoheal, `pull_request` → review, `workflow_dispatch` → user-selected). Mode controls which inline `env`-block prompt is used.
...
Extends:
- `github>bfra-me/.github:internal.json5#v4.16.18` — bfra-me org's internal Renovate preset
...
| Tool | Version |
| --- | --- |
| Node.js | 24.15.0 (`engines.node` in package.json) |
| pnpm | 10.33.4 |
| TypeScript | 6.0.3 |
...
| Fro Bot agent | v0.44.2 (newest in ecosystem at survey) | v0.42.2 | v0.43.1 |
