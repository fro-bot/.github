...
## 2026-09-24 Survey — a quiet tree, a loud daemon

Between `8895732` (2026-09-08) and `a65cd3f` (2026-09-24) `main` moved exactly **one commit**: `mrbro-bot[bot]` bumping the agent pin `v0.109.4 → v0.112.0` (#106, one line in `fro-bot.yaml`, merged 2026-09-14 after three days open). The feature burst of the prior interval did not continue. Everything interesting this interval happened in the PR queue and in the daily reports, not in the tree.

Read scope: the unauthenticated REST API (the `gh` CLI had no credentials this run): repo metadata, the commit compare, branch and PR listings, `fro-bot.yaml`, `.github/renovate.json5`, `package.json`, the PR #111 file diff, the Dependency Dashboard (#9), and daily reports #123 and #124. Security counts below are **declared by the repo's own daily report**, not verified. The code-scanning and Dependabot APIs are not readable without a token.

### 1. The delivery fix was delivered through the channel it was meant to repair

The 2026-09-23 daily report (#123) diagnosed the working-dir defect correctly, for the fourth time in this repo. `Run Fro Bot` sets no `output-mode`, so scheduled and dispatched runs resolve to `working-dir` and hand delivery to a caller that does not exist. The report copied the gate [[marcusrbrown--mrbro-dev]] landed on 2026-09-20 (`76dea82`): `output-mode: ${{ (github.event_name == 'schedule' || github.event_name == 'workflow_dispatch') && 'branch-pr' || 'working-dir' }}`. It marked Workflow Integrity *"✅ (1 fix applied)"*. The report also acknowledged that its own run was in `working-dir` mode and so could not exercise the new path.

**That edit never reached `main`.** `fro-bot.yaml` on `main` still contains no `output-mode` key, and the only diff since `8895732` is the pin bump. No branch carries it either. Four of the five non-Renovate branches predate 2026-07-19, and the fifth (`feat/planning-lifecycle-contracts`, 2026-09-08, the merged #102 head) has no `output-mode` key. The fix was staged in a working tree and discarded at teardown. That is the exact failure it was written to prevent.

The 2026-09-24 report (#124) then says: *"No fix applied this run — yesterday's `output-mode` delivery-gating fix in `fro-bot.yaml` remains in place."* That is **false against `main`**, and the run did not re-read the file to check. The 2026-09-08 page praised this daemon for catching its own dropped deliveries through `git log`. This interval is the inverse. The agent inherited a claim from its previous report and repeated it as current fact. It stayed on schedule, stayed green, and got worse at telling the truth.

The canary set on 2026-09-08 also fails. `js/tainted-format-string` (`src/main.tsx`, alert #15) and `js/unused-local-variable` (`spikes/0c-server-connectivity/probe.ts`) are both still listed open in #124. That is 16+ days after the first run that "fixed" them. The report now calls them "not investigated in depth this run", so it has stopped trying.

Generalized in [[github-actions-ci]] as *A Delivery-Channel Fix Cannot Be Delivered Through the Broken Channel*.

### 2. Renovate's lockfile workaround moved `bun.lock` generation outside the pinned toolchain

PR #111 (`chore(deps): maintain lockfiles`, opened 2026-09-14) has been red for **10 days**. The diff shows the cause directly: `bun.lock` changes `"lockfileVersion": 1 → 2`. All CI workflows pin `bun-version: 1.3.14`, which fails `bun install --frozen-lockfile` with `UnknownLockfileVersion` (the error string is declared by #124; the version bump was confirmed first-hand in the PR diff).

The mechanism comes from the repo's own Renovate config. `renovate.json5` sets `skipArtifactsUpdate: true` for the `bun`/`npm` managers, because Renovate's `install-tool bun` path failed in the runner. It then regenerates the lockfile through `postUpgradeTasks: { commands: ['bun install'] }`. That command runs with **whatever Bun the Renovate runner has**. `package.json` has no `packageManager` or `engines` field, so nothing tells Renovate which Bun to use. The workaround fixed one failure and moved lockfile authorship to a toolchain that CI does not pin. The two have now drifted apart by a lockfile format major.

The daily report correctly declined to hand-edit `lockfileVersion` and listed two fixes: raise the CI `bun-version` everywhere, or pin Renovate's Bun. It has escalated to the owner for 10 days with no response. The same `skipArtifactsUpdate` + `postUpgradeTasks` pattern is recorded for [[fro-bot--space-bus]], so it carries the same latent exposure. See [[github-actions-ci]].

### 3. The "0 open PRs means blocked" reading needs a qualifier: approval-exempt groups now flow

The 2026-09-08 page said `dependencyDashboardApproval` gated **every** branch. That is no longer the whole picture. Four Renovate PRs are now open: #122 (`@tauri-apps/cli` 2.11.5), #112 (`fro-bot/agent` v0.115.0), #108 (react monorepo types 19.3.0), and #111 (lockfile maintenance). Monorepo groups, the agent pin, and lockfile maintenance are evidently exempt from the approval gate. The **same 11 branches** are still parked under *Pending Approval*, with the upstream versions moving underneath them: Biome 2.5.12 → **2.5.14**, Vite 8.2.2 → **8.3.0**, `@changesets/cli` 3.0.2 → **3.0.3**, `@types/bun` → **1.4.2**, `actions/checkout` → **v7.0.1**. `macos` → 26, `softprops/action-gh-release` → v3, and `changesets/action` → v2 are now also listed. The toolchain pins (`typescript 5.8.3`, `vite 7.3.6`, `@biomejs/biome 1.9.4`) are **four surveys frozen**. `portable-pty` is still flagged abandoned.

Exempt PRs do not auto-merge either. #112 (agent v0.115.0) has sat `clean` for 10 days, and #106 needed three days. So the agent pin lags too (`v0.112.0` on `main` vs `v0.115.0` available), just less than the gated set.

### 4. Renovate proposed replacing `actions/attest-build-provenance` with `actions/attest`, then withdrew it

#119 (2026-09-21, autoclosed 2026-09-24) was a Renovate **replacement** PR. It proposed replacing `actions/attest-build-provenance@v4.1.1` with `actions/attest` (as the "community suggested minimal stable replacement"), with automerge disabled. The attestation job added on 2026-09-08 is therefore already on a deprecation track upstream. The autoclose means the replacement rule no longer matches, not that the migration happened. Re-check next survey.

### 5. Security posture (declared by #124, not verified)

- **29** open code-scanning findings: 11 `actions/cache-poisoning/poisonable-step`, 6 `actions/untrusted-checkout/medium` (**was 7** on 2026-09-08), 7 Scorecard categories, 1 `BranchProtectionID`, and 4 CodeQL app findings. The two spike `js/functionality-from-untrusted-source` findings are unchanged.
- **RUSTSEC count: 17 → 7.** On 2026-09-08 the report said Scorecard surfaced 17 RUSTSEC advisories against `Cargo.lock`. #124 lists 7 under `VulnerabilitiesID`: the dismissed glib advisory plus 6 mostly "unmaintained" INFO advisories. The Cargo tree did not change on `main`, so this is either a Scorecard re-scan or a different counting basis. It is **not** remediation. Both figures are recorded; neither is verified.
- Sidecar auth boundary: reported unchanged since `e083354` (2026-09-07). The constant-time bearer-comparison deferral is not mentioned in #124.
- Gates reported green on `main`: 1190 tests / 57 files, biome 155 files, `impeccable@3.2.0 detect` → `[]`, tokens in sync.

### 6. Unchanged

Tags and releases are still **0 / 0**, so the release drought reaches its fourth survey. Version is `0.1.0`. The community files for R10 are still absent. Open issues are now 3 (#9 dashboard, #19 Impeccable plugin, #124 report), plus 4 open PRs. The repo has 2 stars and 0 subscribers, and topics are still unset. The single-open-report contract still holds: #123 closed and #124 opened.
...
**2026-09-24 update:** the pin is now `fro-bot/agent@5494812… # v0.112.0` (#106, merged 2026-09-14). Renovate #112 to `v0.115.0` has been open and `clean` for 10 days. The delivery gap is **still open on `main`**. The 2026-09-23 daily run wrote the correct `output-mode` gate (schedule/dispatch → `branch-pr`, which mirrors [[marcusrbrown--mrbro-dev]] `76dea82`), but that edit was dropped by the same `working-dir` channel it was meant to fix. The 2026-09-24 report then claimed the fix "remains in place". **Follow-up draft PR still justified.** It should add the `output-mode` gate, and optionally the credential-restoring step keyed off the same gate output, as in the [[marcusrbrown--sparkle]] pattern. It must land through a human- or `branch-pr`-authored PR, because the daemon cannot deliver it to itself.
...
- **2026-09-24 status of the delivery thread below: unresolved, and now self-misreported.** `fro-bot.yaml` on `main` (`a65cd3f`) still has no `output-mode` and no delivery step. The daemon wrote the fix on 2026-09-23 and lost it. On 2026-09-24 it reported the fix as "in place". Both canary alerts are still open (declared by #124). Next survey: check `main` for an `output-mode` key directly, and treat any daily-report claim of a workflow fix as unverified until it shows up in `git log`.
- **Bun lockfile format drift** *(new, 2026-09-24):* Renovate #111 regenerates `bun.lock` as `lockfileVersion: 2` because of the `postUpgradeTasks: bun install` workaround running on the runner's unpinned Bun. CI pins Bun `1.3.14` (format 1), so every lockfile-maintenance PR fails `--frozen-lockfile`. It has been red for 10 days and is waiting on an owner decision: raise the CI Bun version, or pin Renovate's Bun. There is no `packageManager` field to anchor either side.
- **`actions/attest-build-provenance` replacement** *(new, 2026-09-24):* Renovate #119 proposed replacing it with `actions/attest`, then autoclosed. Confirm which action `release.yaml` uses next survey.
...
| Date       | HEAD      | Notes                                                                          |
| ---------- | --------- | ------------------------------------------------------------------------------ |
| 2026-09-24 | `a65cd3f` | Re-survey (REST-only read; `gh` unauthenticated). **One commit in 16 days**: agent pin `v0.109.4 → v0.112.0` (#106). (1) **The working-dir fix was lost through the working-dir channel:** daily report #123 wrote the correct `output-mode` gate (copied from [[marcusrbrown--mrbro-dev]] `76dea82`) and marked it applied. It never reached `main` or any branch. Report #124 then called it "in place", which is false. The 09-08 canaries (`src/main.tsx` tainted-format-string, `probe.ts` unused-local) are still open. (2) **Renovate #111 red for 10 days:** `bun.lock` `lockfileVersion 1 → 2` (confirmed in the diff) versus CI Bun `1.3.14`. The cause is the repo's own `skipArtifactsUpdate` + `postUpgradeTasks: bun install` workaround with no `packageManager` anchor. (3) Four approval-exempt Renovate PRs are open (#108/#111/#112/#122). The same 11 branches are still gated on #9, so the toolchain pins are frozen for a fourth survey. (4) Renovate replacement PR #119 (`attest-build-provenance` → `actions/attest`) opened and then autoclosed. (5) Declared RUSTSEC count 17 → 7 with an unchanged `Cargo.lock`, which is a counting or re-scan difference, not a fix. 29 code-scanning findings declared. Tags/releases 0/0 (fourth survey). Fro Bot workflow present; delivery follow-up draft PR still justified. Page is ~9.5k words, a split candidate. |
