...
sources:
  - url: https://github.com/marcusrbrown/gpt
    sha: c7980be10467836397381e389720122562da3c33
    accessed: 2026-04-26
  - url: https://github.com/marcusrbrown/gpt
...
| Build | Vite 8.0.10, `@vitejs/plugin-react-swc` | `tsgo` (`@typescript/native-preview` 7.0.0-dev.20260422.1) for type-checking |
...
| AI | LangChain 1.3.4, `@langchain/openai` 1.4.4, `@langchain/anthropic` 1.3.27, `@langchain/core` 1.1.41, `@langchain/langgraph` 1.2.9 | Provider-abstracted via `BaseLLMProvider` |
...
Both workflows use `fro-bot/agent@v0.42.1` (SHA `6c45d8ce66b0b69f1b80b23f283ed455deb59517`) with `OPENCODE_AUTH_JSON`, `FRO_BOT_PAT`, `FRO_BOT_MODEL`, and `OMO_PROVIDERS` secrets/vars.
...
- **PR #2345** — Adopt `error_` naming convention and `@/` import alias (authored by `fro-bot`, code style enforcement)
- **30 open issues** (as of 2026-04-26)
...
| 2026-04-26 | `c7980be` | Dependency-only delta (8 commits): `fro-bot/agent` v0.41.4→v0.42.1, `vite` 8.0.9→8.0.10, `vitest` 4.1.4→4.1.5, `tailwindcss` 4.2.2→4.2.4, `pnpm` 10.33.0→10.33.1, `react-router-dom` 7.14.1→7.14.2, LangChain.js monorepo bump (`langchain` 1.3.3→1.3.4, `@langchain/anthropic` 1.3.26→1.3.27, `@langchain/core` 1.1.41). New Fro Bot PR #2345 (code style). No structural changes. |
