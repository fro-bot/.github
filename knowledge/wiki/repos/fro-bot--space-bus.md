...
> **2026-08-19 no-op re-survey (HEAD `fd8a746`, npm `0.15.0`) — frozen, no durable delta.** `main` HEAD is **still `fd8a746`** (last merged commit #128 "chore(deps): update bfra-me/.github to v4.16.44", dated 2026-08-03), unchanged since the 2026-08-04 survey — now **16 days frozen**. `pushed_at` reports 2026-08-16 but that is **open-PR-branch activity only** (deps PRs #118 GitHub Actions / #115 `@opencode-ai/plugin` v1.18.18), not a tree change; `updated_at` 2026-08-19 is the daily-report issue churn. Re-verified against this repo's own manifest at HEAD: npm `latest` still **`0.15.0`** (still 22 versions, `latest_time` 2026-07-19 — no publish in a month); `package.json` deps byte-identical (`@modelcontextprotocol/sdk` 1.29.0, zod `^4.4.3`, peer `@opencode-ai/plugin >=1.17.13 <2`, dev-pin `1.18.2`, `@changesets/cli` 2.31.1, Biome 2.5.2, TS 5.9.3); 7 workflows unchanged; `fro-bot.yaml` re-read at HEAD — agent still `fro-bot/agent@v0.93.1` (SHA `a4976f4`), single `0 0 * * *` cron, `bfra-me/.github` reusable pin unchanged. Open issues 9 / stars 1 (flat). Queue is the same carried set, only the daily-report issue renumbered (#147); backlog PRs #135 (npm v11.19.0) / #130 (`@modelcontextprotocol/sdk` v1.30.0) / #118 / #115 / #72 (Biome v2.5.8, failing since ~2026-07-11 per prior oversight) all still open and unmerged, plus deferred-hardening issues #81 / #63 / #6. The plugin remains in steady-state; the 2026-08-04 record below is unchanged and canonical.
...
| Attribute        | Value (2026-08-19 no-op re-survey, HEAD `fd8a746` — unchanged since 2026-08-04) |
| ---------------- | ------------------------------------------------------------------------------ |
| Created          | 2026-07-03                                                                     |
| Last push        | `pushed_at` 2026-08-16 (open-PR-branch pushes only; HEAD commit `fd8a746` dated 2026-08-03, "chore(deps): update bfra-me/.github to v4.16.44" #128 — `main` tree frozen 16 days) |
...
**Published-`latest` confirmation (2026-08-04, this repo's manifest):** `latest` is now **`0.15.0`** (22 versions). Mothership's last-known pin `0.14.0` is one minor behind — the `0.15.0` message-correlation additions (`createDispatchMessageId`, `messageId` on `dispatch`, `DispatchFailure`) are strictly additive on the browser-safe `/core` lane, so a mothership bump to `0.15.0` picks them up without contract breakage. Verify mothership's pin on its next survey. **Re-confirmed 2026-08-19:** `latest` still `0.15.0` (22 versions, `latest_time` 2026-07-19 — no publish in a month; the browser-safe `/core` surface is unchanged for Mothership).
...
| Date       | HEAD      | Notes                                                                          |
| ---------- | --------- | ------------------------------------------------------------------------------ |
| 2026-08-19 | `fd8a746` | **No-op re-survey.** `main` HEAD **still frozen at `fd8a746`** (last merged #128, 2026-08-03) — unchanged since 2026-08-04, now 16 days. `pushed_at` 2026-08-16 = open-PR-branch pushes only (#118/#115), no tree drift; `updated_at` 2026-08-19 = daily-report churn. npm `latest` still **`0.15.0`** (22 versions, no publish since 2026-07-19); `package.json` deps byte-identical; 7 workflows unchanged; `fro-bot.yaml` re-read (agent `v0.93.1` `a4976f4`, single `0 0` cron). Open issues 9 / stars 1 (flat); queue same carried set (daily-report renumbered #147), backlog PRs #135/#130/#118/#115/#72 + issues #81/#63/#6 all unmerged. Steady-state. |
