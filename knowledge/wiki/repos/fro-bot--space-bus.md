...
> **2026-09-19 survey (HEAD `47e32358`, npm `0.15.0`) — the freeze held, the delivery gap escalated from documentation to security, and the prior survey's root cause is corrected.** Four commits since `6c32dec`, touching **7 files**: five `.github/workflows/*.yaml`, plus `package.json`/`bun.lock` for one dev-pin. Recursive tree still **117 blobs**; `src/` is untouched, so the last `src/` commit remains `fe0cc42` (2026-07-19) — now **62 days**. npm `latest` is still `0.15.0`, still 22 versions, registry `modified` still `2026-07-19T08:38:39.752Z` — a **62-day publish drought**, `.changeset/` still holding only `config.json`. **(1) CORRECTION — the delivery blocker is the harness delivery contract, not the prompt.** The 2026-09-04 page attributed the undelivered docs fix to a HARD BOUNDARIES/category-4 conflict. Superseded by the agent's own reports: from **#166 (2026-09-05)** — *one day after that survey was written* — every daily report carries an explicit delivery note ("this run operates in `working-dir` mode (no branch/PR creation permitted)… the caller workflow owns diffing, committing, and opening the PR"). `fro-bot.yaml` still has exactly four steps and no commit/push/PR half. Same root cause as [[marcusrbrown--tokentoilet]]; the prompt conflict is real but not the binding constraint. **(2) ESCALATION — the discarded work is now security remediation.** #170 (09-08) left a `bun.lock` fix taking `bun audit` **19 → 0** in the working tree; #183 (09-19) left `package.json` `overrides` closing **three confirmed HIGH advisories** (`fast-uri` 3.1.3→3.1.8, `ip-address` 10.2.0→10.7.2, `toml` 4.1.2→4.3.0) in the working tree. Neither reached `main`. The gap stopped costing documentation accuracy and started costing patches. **(3) A human edited `fro-bot.yaml` and shipped the other half.** #174 (09-11, `marcusrbrown`, first human commit in 54 days) added conditional `persist-credentials` — correct hardening, and it makes credentials *persist* on exactly the `schedule` runs that need delivery, so the credential precondition now exists while the delivery step still does not. Compare [[marcusrbrown--sparkle]] #2001/#2003, where one computed gate feeds **both** the credential restore and the agent's `output-mode`; space-bus has the credential half only. **(4) PR #72's red is a coupled-constant defect, not the lockfile theory** — `biome.json`'s `$schema` is pinned to `https://biomejs.dev/schemas/2.5.2/schema.json`, invisible to every Renovate manager, so each `@biomejs/biome` bump is red by construction; **70 days** open, second fleet instance after [[marcusrbrown--opencode-copilot-delegate]]. **(5) Immune by lag** — the `bfra-me/.github` pin sat at v4.16.44 through the 2026-09-04 `tar` incident and jumped straight to **v4.29.0** on 09-14, stepping over the poisoned v4.25.0 entirely. **(6) Grouped Renovate PRs land content their titles do not name** — #115 titled `@opencode-ai/plugin to v1.18.3` landed `1.18.2 → 1.18.26`; #167 titled `fro-bot/agent to v0.108.1` landed `v0.106.0 → v0.112.0` plus four `github/codeql-action` SHA bumps. Carried forward unchanged: the `actions/checkout` two-major split, the inert `fro-bot/agent` automerge carve-out, and a ~100% bot-authored trigger surface. Agent pin **v0.106.0 → v0.112.0**; `@opencode-ai/plugin` dev-pin **1.18.2 → 1.18.26**. Open issues 4, open PRs 5.
...
| ------------------ | ----------------------------------------------------------------------------- |
| Repo id            | `1287751069` (`node_id` `R_kgDOTMGFnQ`) — **public**                          |
| Created            | 2026-07-03                                                                    |
| HEAD               | `47e32358` (2026-09-14, "chore(deps): update bfra-me/.github to v4.29.0" #158) |
| Last push          | 2026-09-17 (`pushed_at` — PR-branch pushes only)                              |
| Last `src/` change | **2026-07-19** (`fe0cc42`, #113) — **62 days**                                |
| Last human commit  | **2026-09-11** (`068a4d47`, `marcusrbrown`, #174) — a workflow-only CI hardening; the last human commit **to `src/`** is still `fe0cc42` |
| Tree               | **117 blobs** — identical count to `6c32dec`; `src/` 47 blobs (22 modules + 25 colocated tests), `docs/` 37, `test/` 4 |
| Language / runtime | TypeScript (strict, ESM); Bun for dev/build/test, published dist targets Node ESM |
| Package            | `@fro.bot/space-bus` — npm `latest` **`0.15.0`**, 22 versions, published `2026-07-19T08:38:39.441Z`, registry `modified` `…:39.752Z` (unmoved) ⇒ **62-day publish drought**; `.changeset/` holds only `config.json` |
| License            | MIT                                                                           |
| Stars / forks      | 1 / 0                                                                         |
| Open issues        | 4 (`#183` today's daily report, `#81`, `#63`, `#6` Dependency Dashboard)       |
| Open PRs           | 5, **all `fro-bot[bot]` Renovate** — `#178` (5d), `#177` (5d), `#135` (47d), `#130` (51d), **`#72` (70d, red)** |
| Workflows          | 7, all `active`: `ci`, `codeql-analysis`, `fro-bot`, `release`, `renovate`, `scorecard`, `update-repo-settings` |
| Agent pin          | `fro-bot/agent@5494812a` **# v0.112.0** (was v0.106.0); next bump **v0.113.2** rate-limited |
| Reusable base      | `bfra-me/.github` **v4.29.0** (`0e881c39`) in both `renovate.yaml` and `update-repo-settings.yaml`; v4.30.0 rate-limited |
| Default-branch CI  | `Check` **green** — 30/30 most recent `main` runs `success`                    |
| Status             | **Shipped OpenCode plugin, still code-frozen.** Six-tool bus + managed-server + CLI + launchd + browser-safe library surface all durable and unchanged since 2026-07-19. Automation runs daily and green; its output does not reach the repository (see the delivery sections below) |

### Prior overview (2026-09-04 survey, HEAD `6c32dec` — historical)
...
## CORRECTION — The Delivery Blocker Is the Harness Contract, Not the Prompt (2026-09-19)

The 2026-09-04 section above diagnosed the undelivered docs fix as a prompt defect: category 4 asks for a PR branch, HARD BOUNDARIES whitelists direct pushes only for categories 1–2, so no permitted path exists. That reading was **superseded within 24 hours of being written**, by the agent itself.

Every daily report from **#166 (2026-09-05)** onward carries an explicit delivery note. Verbatim from three of them:

| Issue | Date       | Delivery note                                                                                              |
| ----- | ---------- | ---------------------------------------------------------------------------------------------------------- |
| #166  | 2026-09-05 | "this run operates in `working-dir` mode (no branch/PR creation permitted), so this fix … lands in the same working-tree" |
| #179  | 2026-09-15 | "this run's Delivery Mode is `working-dir` (schedule trigger) — all fixes were written directly to the working tree for the caller workflow to diff/commit/push/PR" |
| #183  | 2026-09-19 | "no branch/commit/push/PR was performed by this agent. The caller workflow owns diffing, committing, and opening the PR for these changes." |

So the operative mechanism is the harness's **`working-dir` delivery contract**: the agent is instructed that delivery belongs to the caller workflow, and `fro-bot.yaml` still ends at `Run Fro Bot` — four steps, no commit, no push, no `gh pr create`, no `output-mode` input. This is the same shape recorded at [[marcusrbrown--tokentoilet]] (a fully-permissioned daemon whose output channel was cut when the harness moved delivery to the caller) and the class generalized in [[github-actions-ci]].

The prompt's category-4/boundary conflict is still present and still worth fixing, but it is **not load-bearing** — widening the whitelist would change nothing while the run is under a `working-dir` contract. Recording both readings: the 2026-09-04 diagnosis was a correct observation of the prompt and a wrong attribution of cause. The generalizable lesson is narrow and useful: **when an agent explains its own inaction, read the delivery-mode note before reading the prompt** — the harness contract sits above the prompt in the resolution order, and a boundary clause cannot grant a capability the contract has already withdrawn.

## The Gap Now Discards Security Patches, Not Documentation (2026-09-19)

The 2026-09-04 framing — an autoheal loop burning itself on a two-file docs diff — undersold what was at stake. In the fifteen days since, the same discarded-working-tree path has swallowed two rounds of security remediation:

| Issue | Date       | Work written to the working tree and lost                                                                 |
| ----- | ---------- | ----------------------------------------------------------------------------------------------------------- |
| #170  | 2026-09-08 | A `bun.lock` fix taking `bun audit` from **19 vulnerabilities → 0**                                          |
| #183  | 2026-09-19 | `package.json` `overrides` closing **three confirmed HIGH advisories**: `fast-uri` 3.1.3 → 3.1.8 (6 HIGH, host-confusion/SSRF in `ajv`'s URI parser), `ip-address` 10.2.0 → 10.7.2 (1 HIGH, octal/leading-zero parsing), `toml` 4.1.2 → 4.3.0 (1 HIGH, uncontrolled recursion) |

Both runs verified their own fix (`bun install` + the full `typecheck`/`lint`/`build`/`test` gate, green) and both reported ✅/⚠️ in a public issue. `package.json` at HEAD has **no `overrides` block**, and `bun.lock` is untouched by any commit since 09-05. The repository has neither patch.

This is the part worth carrying to other repos: a delivery gap is usually discovered through its most visible symptom (a docs diff that keeps reappearing), and the symptom is chosen by what the agent happens to find, not by severity. The same broken channel carries whatever the daemon produces. **The cost of an undelivered write path is bounded by the most important thing the agent is capable of fixing, not by the thing it fixed yesterday.** A monitor that watches only "did the docs drift close?" will report the cheap failure and miss the expensive one.

### The report series continues

The docs drift is **independently confirmed still present at HEAD `47e32358`**, not taken on the agent's word:

- `README.md`'s "Library surface" list (lines 145–150) enumerates `/core`, `/config`, `/contract`, `/format`, `/managed-server`, `/attach` — still **no `@fro.bot/space-bus/registry`**, which is a real `exports` entry (`"./registry" → dist/registry-entry.js`).
- `AGENTS.md`'s "Project structure" list (lines 7–18) covers `index`/`tools`/`core`/`config`/`contract`/`attach`/`mcp`/`discovery`/`server`/`cli`/`launchd`/`service` — still **no `src/registry.ts`, `src/roster-edit.ts`, `src/registry-entry.ts`**, and still no `src/format.ts`.
- **A drift instance nobody has named yet:** `README.md:139` opens the Library surface section with "the functions the **four** tools run on." The four-tool constraint was superseded in 2026-07-18; the repo has shipped six tools for two months, and the README says so correctly seventy lines earlier.

Daily reports ran **08-28 → 09-19 with a single gap (2026-09-12, no issue between #173 and #175)**, claiming the same fix throughout — at least fifteen consecutive claimed-and-lost applications on top of the eight already recorded. #182 (09-18) is the first to classify it correctly in its own body: "Recurring docs-drift regression (process issue, not a code issue)."

## PR #72 Is a Coupled-Constant Defect, Not a Lockfile One (2026-09-19)

**Correction.** The 2026-09-04 section attributed #72's permanent red to `skipArtifactsUpdate: true` leaving `bun.lock` stale so `bun install --frozen-lockfile` fails. That was an inference, and it is wrong. The 2026-09-19 report diagnoses it exactly, and the first half is verifiable at HEAD:
...
`@biomejs/biome` is a devDependency pinned to `2.5.2`. The schema URL encodes the **same version a second time**, in a place no Renovate manager can see — it is a string inside a JSON config, not a dependency declaration. So every Biome bump Renovate opens is red the moment it lands: the CLI validates `biome.json` against a `$schema` naming a different release. Second cause, also real: a genuine new `lint/correctness/noUnsafeOptionalChaining` finding at `src/contract.test.ts:125`, introduced by a rule Biome added between 2.5.2 and 2.5.13.

Both fixes belong **on `main`, not on the Renovate branch** — bump the `$schema` URL and fix the lint site, and #72 goes green on its next rebase without anyone touching it. Which is why it has sat for **70 days**: the repair is outside the branch that is failing, and no category in the autoheal prompt owns "fix `main` so a dependency PR can pass."

This is the same class as the `repology` Alpine-branch footgun in [[docker-containers]] — a constant duplicated across two files that no tool links — and the second fleet instance of the specific Biome variant after [[marcusrbrown--opencode-copilot-delegate]]'s `$schema`-versus-CLI chase (#302 → #332 → #377). There the chase was noisy but self-clearing; here it has frozen a PR for ten weeks. Whether it self-clears depends entirely on whether anything else forces a `main` commit.

## Immune by Lag: How a Stale Pin Skipped the `tar` Incident (2026-09-19)

The 2026-09-04 survey flagged `bfra-me/.github` at v4.16.44 against a fleet at v4.24.0 as maintenance debt — "a lag, not the [[marcusrbrown--esphome-life]] mis-path defect." It was also, for one day, a quarantine.

On 2026-09-04 upstream v4.25.0 shipped `bfra-me/renovate-action` 10.34.0, whose bundle left `tar` a devDependency; Renovate exited before servicing any dependency in every repo that ingested it ([[marcusrbrown--github]], [[bfra-me--works]], [[marcusrbrown--esphome-life]], [[bfra-me--renovate-action]]). space-bus's pin never moved during the window — PR #158 was open from 08-30 and merged **2026-09-14**, and the diff it landed is `v4.16.44 → v4.29.0` in one hop, stepping over v4.25.0 entirely. Corroborating: Renovate here automerged #115 at **09-05T07:52**, hours after the fleet-wide outage ended and on a pin that was never poisoned.

Stated precisely, because it is easy to over-read: **during an upstream defect window, pin lag is isolation.** It is not a control — nobody chose it, it protects against nothing in particular, and the same lag would have delayed the antidote had the poison landed first (which is exactly what cost [[marcusrbrown--github]] its 6h28m inert window). The honest framing is that a fleet-wide "everyone on the newest tag" posture concentrates correlated risk, and the repos that carried debt were the repos that did not need a manual sweep. Generalized in [[github-actions-ci]].

## Grouped Renovate PRs Land Content Their Titles Do Not Name (2026-09-19)

The 2026-09-04 page recorded PR #118 as a one-off: open 41 days, rebased throughout, merged carrying `v0.93.1 → v0.106.0` under a title naming neither endpoint. It is not a one-off — it is a property of this repo's grouped branches, now with three instances:

| PR    | Title                                              | What actually landed                                                        |
| ----- | -------------------------------------------------- | ---------------------------------------------------------------------------- |
| #118  | `chore(deps): update GitHub Actions`               | agent `v0.93.1 → v0.106.0` (~13 minors) + 4 other pins, after 41 days open  |
| #115  | `build(dev): update dependency @opencode-ai/plugin to v1.18.3` | `@opencode-ai/plugin` **1.18.2 → 1.18.26** (23 patch releases), after ~47 days open |
| #167  | `chore(deps): update fro-bot/agent to v0.108.1`    | agent **v0.106.0 → v0.112.0** *plus* four `github/codeql-action` SHA bumps v4.37.9 → v4.38.0 |

The mechanism is ordinary — Renovate writes the title when it opens the branch and rebases the content without rewriting the title — but the consequence is specific and worth stating: **on a repo with a long merge queue, a Renovate PR title is a record of when the branch was created, not of what it delivers.** Any audit, changelog, or wiki survey that reads merged-PR titles as a version ledger will be wrong by however long the queue is. Read the merge commit's diff. Same failure mode as [[marcusrbrown--sparkle]]'s autoheal PR whose title went stale when the daemon widened its own override mid-flight; there the drift was authored, here it is emergent.

The dev-pin case is the one with teeth. `@opencode-ai/plugin` is the **dev-pin CI tests against**, while the published `peerDependencies` range promises `>=1.17.13 <2`. A 23-release jump landing under a title claiming a 1-patch bump means the tested surface moved a long way with no review signal proportional to the move.

## Automation Motion Without Repository Motion (2026-09-19)

Three measurements of a daemon fleet running hard over a static tree.

**The Fro Bot workflow's trigger surface is ~83% self-generated.** Of the 30 most recent `main` runs: **25 `skipped`** (15 `issues`, 10 `issue_comment`) and **5 `schedule success`**. The skipped events are the agent's own artifacts — it opens a daily report issue (`issues`), then posts closing comments on the prior ones (`issue_comment`), each of which fires a run that the `[bot]`-author guard immediately skips. 1,159 run numbers, 548 runs on `main`. Fourth instance of the no-op run storm after [[marcusrbrown--tokentoilet]], [[bfra-me--ha-addon-repository]], and [[marcusrbrown--dotfiles]] — and the first where the noise is **strictly self-inflicted** rather than incidental bot traffic.

**The Renovate workflow fires in near-simultaneous pairs.** 1,813 runs on a repo 78 days old (~23/day), and the recent ones arrive two at a time: `1813`/`1812` 9 s apart, `1811`/`1810` 13 s apart, `1808`/`1807` 12 s apart — every pair `workflow_dispatch`, actor `fro-bot[bot]`, both `success`, each 75–110 s. `renovate.yaml` has **no `schedule:` of its own** (triggers: `issues:edited`, `pull_request:edited`, `push` on non-`main`, `workflow_dispatch`, `workflow_run` on `Release`), so every periodic pass is an externally originated dispatch — the same coupling recorded at [[bfra-me--works]]. Stating the limit: the **source of the second dispatch is not established from public data**. What is established is that two full Renovate passes run concurrently against the same branches and the same Dependency Dashboard, roughly a dozen times a day. That is a race, not just waste.

**Dependabot cannot see `bun.lock`.** Per #183: 0 open Dependabot alerts (feature enabled, queried via API) against **22 OSV advisories** that the Scorecard code-scanning path finds reachable through transitive deps of `@modelcontextprotocol/sdk` and `@changesets/cli`. Renovate's own `Vulnerabilities` dashboard section likewise reports "has not found any CVEs on osv.dev." Three vulnerability surfaces disagree on the same tree, and the one wired to alerting is the one reporting zero. Agent-reported, not independently verified here (alert APIs need auth) — but the structural claim is checkable and matters for every Bun-toolchain repo in the fleet: **a green Dependabot on a `bun.lock` repo is an absence of coverage, not an absence of findings.**

## Carried Forward Unchanged (2026-09-19)

Re-verified against the Dependency Dashboard (#6, updated 2026-09-18) and the workflow files at HEAD:

- **`actions/checkout` is still split across two majors** — `v6.1.0` (`d23441a4…`) in `ci`/`release`/`fro-bot`, `v7.0.1` (`3d3c42e5…`) in `codeql-analysis`/`scorecard`. Fifteen more days, no motion. The reconciliation is still the same unchecked box under **Pending Approval** (`renovate/major-github-actions`, bundling `actions/checkout`, `actions/setup-node` v7, `changesets/action` v2).
- **The `fro-bot/agent` automerge carve-out is still inert** — third consecutive confirmation. v0.112.0 landed grouped with four `github/codeql-action` bumps (#167), and the next one (**v0.113.2**) is back under **Rate-Limited** on `renovate/github-actions`, again grouped with `github/codeql-action`. The `matchPackageNames: ['!fro-bot/agent'] + automerge: false` rule has never once produced a hands-free agent bump.
- **Pending Approval is the real backlog** — `npm` v12, the GitHub Actions majors, `@changesets/cli` v3, `typescript` v7. Four majors invisible to any audit that counts open PRs.
- **`bfra-me/.github` lag closed** — v4.16.44 → **v4.29.0** (see the immune-by-lag section); v4.30.0 now rate-limited. The prior "6 minor series behind" thread is resolved.
- **Probot Settings unchanged** — `main` requires `Analyze`/`CodeQL`/`Check`/`Fro Bot`/`Renovate / Renovate`, `enforce_admins: true`, linear history, 1 code-owner review with `require_last_push_approval: true`. The required `Fro Bot` context still skips on bot-authored PRs, so the review gate remains structurally unevaluable on 100% of this repo's open queue.
- **Default-branch health is genuinely green** — `Check` succeeded on all 30 most recent `main` runs. The freeze is not hiding a broken build.
...
**2026-09-19 re-confirmation:** `package.json` moved exactly one line since `6c32dec` — `@opencode-ai/plugin` dev-pin **1.18.2 → 1.18.26** (#115, merged 09-05 under a title naming v1.18.3). Everything else is unchanged: `@modelcontextprotocol/sdk` 1.29.0, `zod` ^4.4.3, peer `>=1.17.13 <2`, `@biomejs/biome` **2.5.2**, `@changesets/cli` 2.31.1, `@types/bun` 1.3.14, `typescript` 5.9.3. The dev-pin gap the prior survey flagged (23 patch releases of drift between the tested surface and the promised peer range) **closed and immediately reopened** — 1.18.31 is now queued in #177. Still no `overrides` block, despite two daily runs authoring one (see the discarded-security-patches section). Queued and unmerged at this survey: Biome → 2.5.13 (#72, red 70 days), `@opencode-ai/plugin` → 1.18.31 (#177), MCP SDK → 1.30.0 (#130, 51 days), `@types/bun` → 1.4.2 (#178), `npm` → 11.19.1 (#135, 47 days); `@changesets/cli` v3, `typescript` v7, and `npm` v12 parked in Pending Approval.
...
**2026-09-19 update to this section.** Three of the readings above are dated and now superseded in fact, kept here for provenance: the agent pin is **v0.112.0** (`5494812a`, #167, 09-14) not v0.106.0; `bfra-me/.github` is **v4.29.0** (`0e881c39`, #158, 09-14) not v4.16.44, so the "6 minor series behind" note is resolved; and the job gained a fifth configuration detail without gaining a fifth step. #174 (`068a4d47`, 09-11, `marcusrbrown`) added to the checkout step:
...
Correct hardening — it withholds the `FRO_BOT_PAT`-backed git credential on exactly the triggers an outside contributor can reach, complementing the existing "never check out PR-head code" comment block. It also inverts the delivery picture in a way worth naming: credentials now **persist** on `schedule` and `workflow_dispatch`, which are the runs that produce fixes. The git-level precondition for delivery exists; the steps that would use it do not. `permissions: contents: read` at workflow scope and the four-step job (checkout → setup-bun → `bun install` → `Run Fro Bot`) are both unchanged, and the agent action still receives no `output-mode` input.
...
**Freeze extended (2026-09-19, npm registry):** `latest` is **still `0.15.0`**, still 22 versions, `time.modified` still `2026-07-19T08:38:39.752Z`. The drought is now **62 days** and the consumer offset against [[marcusrbrown--mothership]]'s `0.14.0` pin is unchanged — a fixed gap, not a closing one. Re-reading the sentence above after another interval: the `@experimental` caveat has now had 62 days of no change, and in that time the only code the repository's own automation tried to write was rejected by its delivery path. Stability by immobility is the weakest kind available, and it is the kind this surface has.
...
- **CORRECTED 2026-09-19 — the delivery blocker is the `working-dir` harness contract, not the prompt.** The agent has said so in every report since #166 (09-05). Re-check next survey whether `fro-bot.yaml` grew the caller half (a `Resolve delivery mode` gate + `output-mode` input + commit/push/PR steps, per the [[marcusrbrown--sparkle]] #2001/#2003 shape) or whether the schedule run switched to `branch-pr`. The prompt's category-4/boundary conflict should still be fixed, but fixing it alone will not deliver anything.
- **NEW 2026-09-19 — the gap is now discarding security patches.** Two verified-green remediations written to the working tree and lost: `bun audit` 19 → 0 (#170, 09-08) and `overrides` for three HIGH advisories (#183, 09-19). Check `package.json` for an `overrides` block next survey; its absence is the cheapest possible proof the channel is still cut.
- **NEW 2026-09-19 — PR #72 needs a fix on `main`, not on its own branch.** `biome.json`'s `$schema` pins `2.5.2` while Renovate bumps the CLI; plus a real `noUnsafeOptionalChaining` finding at `src/contract.test.ts:125`. Track whether either lands. This supersedes the 2026-09-04 stale-`bun.lock` hypothesis.
- **NEW 2026-09-19 — Renovate runs in duplicate.** Pairs 9–13 s apart, ~23 runs/day, `renovate.yaml` has no cron of its own. Source of the second dispatch **unidentified**; establish it next survey (a `print-config: true` dispatch or the org dispatcher's own run log should name it).
- **NEW 2026-09-19 — a new docs-drift instance nobody has flagged:** `README.md:139` says "the functions the **four** tools run on" in a six-tool repo.
- **NEW 2026-09-19 — Dependabot reports 0 alerts while Scorecard's OSV path finds 22.** Structural claim: Dependabot does not parse `bun.lock`. Verify with authenticated alert APIs next survey; if it holds, it is a fleet-wide coverage gap for every Bun repo.
- **RESOLVED 2026-09-19 — the `bfra-me/.github` lag.** v4.16.44 → v4.29.0 (#158, merged 09-14), stepping over the poisoned v4.25.0. The lag was accidental quarantine during the 09-04 `tar` incident.
...
| 2026-09-19 | `47e32358` | Re-survey. **Freeze holds at 62 days; the delivery gap escalates; two prior diagnoses corrected.** 4 commits / 7 files since `6c32dec` — 5 workflow files + `package.json`/`bun.lock` for one dev-pin; tree still **117 blobs**, `src/` untouched since 2026-07-19; npm still `0.15.0`/22 versions, `modified` unmoved ⇒ **62-day publish drought**. **(1) CORRECTION:** the undelivered-fix root cause is the harness's **`working-dir` delivery contract** plus a caller workflow with no commit/PR half (same as [[marcusrbrown--tokentoilet]]), stated by the agent itself in every report from **#166 (09-05)** — one day after the prior survey's prompt-conflict attribution, which is superseded as *cause* (the conflict is real but not binding). **(2) ESCALATION:** the discarded working tree now carries security work — `bun audit` **19 → 0** (#170, 09-08) and `overrides` for three HIGH advisories `fast-uri`/`ip-address`/`toml` (#183, 09-19); `package.json` at HEAD has no `overrides` block. Docs drift **still present at HEAD**, plus an unflagged instance (`README.md:139` says "four tools" in a six-tool repo); reports ran 08-28 → 09-19 with one gap (09-12). **(3)** First human commit in 54 days — **#174** (`068a4d47`, 09-11) added conditional `persist-credentials` to `fro-bot.yaml`: correct hardening, and it makes credentials persist on exactly the `schedule` runs that need delivery, so the credential precondition now exists while the delivery step does not (contrast [[marcusrbrown--sparkle]] #2001/#2003, one gate feeding both halves). **(4) CORRECTION:** PR **#72** (red **70 days**) fails on `biome.json`'s `$schema` pinned to `2.5.2` — a coupled constant invisible to Renovate — plus a real `noUnsafeOptionalChaining` finding at `src/contract.test.ts:125`; the stale-`bun.lock` hypothesis is withdrawn. Fix belongs on `main`. **(5)** **Immune by lag:** `bfra-me/.github` v4.16.44 → **v4.29.0** in one hop (#158, 09-14), stepping over the poisoned v4.25.0 — this repo never ingested the 09-04 `tar` defect. **(6)** Three grouped Renovate PRs landed content their titles do not name (#118, #115 `1.18.2 → 1.18.26`, #167 `v0.106.0 → v0.112.0` + 4 codeql-action pins). **(7)** Renovate fires in **near-simultaneous pairs** (9–13 s apart, ~23 runs/day, no local cron; second dispatcher unidentified); Fro Bot's trigger surface is **~83% self-generated** (25 of 30 recent `main` runs `skipped`, fired by the agent's own report issue and closing comments). **(8)** Dependabot 0 alerts vs 22 OSV advisories via Scorecard — `bun.lock` coverage gap (agent-reported). Carried unchanged: `actions/checkout` two-major split, inert agent automerge carve-out (v0.113.2 rate-limited), Probot Settings, `Check` green on all 30 recent `main` runs. Agent pin v0.106.0 → **v0.112.0**. Open issues 4, open PRs 5. |
