...
sources:
  ...
  - url: https://github.com/fro-bot/dashboard
    ...
    accessed: 2026-07-09
tags: [typescript, hono, node24, native-typescript, strip-only, pnpm, github-app, oauth, octokit, arctic, monitoring, dashboard, read-only, ssr, spa, react, vite, tailwind, pwa, workbox, calver, ghcr, impeccable, fro-bot, redaction, fail-closed, security, operator-contract, sse, control-plane, fixture-harness]
related:
  - fro-bot--agent
  - marcusrbrown--infra
  - bfra-me--works
...
| --------------- | --------------------------------------------------------------- |
| Created         | 2026-06-14                                                      |
| Last push       | 2026-07-09 (survey 2026-07-09, HEAD `cb5190d`; prior `5c631a5` @ 2026-06-26, `2504939` @ 2026-06-15) |
| Language        | TypeScript (Node 24 native TS, **strip-only** for `src/`/`test/`/scripts, ESM) |
| Package manager | pnpm 11.10.0 (was 11.8.0 → 11.5.0)                              |
| Node.js         | `>=24` (engines)                                                |
...
| Backend framework | Hono ^4.7.11 + `@hono/node-server` ^1.14.4 (SSR shell + API + operator proxy) |
| Frontend stack  | React 19.2.7 + react-dom, Tailwind CSS 4.3.2 (`@tailwindcss/vite`), Vite 8.1.3, `@vitejs/plugin-react-swc` 4.3.1, PWA via `vite-plugin-pwa` 1.3.0 + Workbox 7.4.1 |
...
| Test framework  | Vitest 4.1.9; `@testing-library/react` 16.3.2 + `jest-dom` 6.9.1 + jsdom 29.1.1 for the SPA |
| Lint            | ESLint 10.6.0 (`@bfra.me/eslint-config` 0.51.1) + `eslint-plugin-erasable-syntax-only` 0.4.0; `impeccable` UI anti-pattern detector (Design Check) |
| TypeScript      | 6.0.3 (`@bfra.me/tsconfig` 0.13.1)                             |
| Visibility      | Public                                                          |
| Stars           | 1                                                              |
| License         | None declared                                                  |
| Releases        | **CalVer (`YYYY.MM.N`)** — 58 releases, latest `2026.07.12` (2026-07-08); GHCR image only, `package.json` version stays `0.0.0` |
...
**Three workflows** (`main.yaml`, `fro-bot.yaml`, `release.yaml`) — unchanged in count as of 2026-07-09. All third-party actions are SHA-pinned with version comments. The 2026-07-09 survey confirms `main.yaml` still runs six jobs and `release.yaml`'s four-job CalVer chain is intact; the notable churn is dependency/pin bumps plus a reworked `fro-bot.yaml` concurrency group.
...
Runtime deps are unchanged from 2026-06-15 through 2026-07-09. The frontend/dev toolchain is bumped as of 2026-07-09: `react`/`react-dom` 19.2.7, `tailwindcss` + `@tailwindcss/vite` **4.3.2** (was 4.3.1), `vite` **8.1.3** (was 8.0.16), `@vitejs/plugin-react-swc` **4.3.1**, `vite-plugin-pwa` 1.3.0 (now exact) + `workbox-*` 7.4.1 (exact-pinned across build/core/precaching/routing/strategies/expiration/cacheable-response/window), `@testing-library/react` 16.3.2 / `jest-dom` **6.9.1** / `jsdom` **29.1.1**, `@types/react` **19.2.17** / `@types/react-dom` **19.2.3**. Other dev: `@bfra.me/eslint-config` 0.51.1, `@bfra.me/tsconfig` 0.13.1, `eslint` **10.6.0** (was 10.5.0), `eslint-plugin-erasable-syntax-only` 0.4.0, `typescript` 6.0.3, `vitest` 4.1.9, `jiti` 2.7.0, `@types/node` **24.13.2**. The `@bfra.me/*` toolchain comes from [[bfra-me--works]].
...
**Present and self-hosted.** `fro-bot.yaml` pins `fro-bot/agent@99e7d85` (v0.84.2, ecosystem version leader as of 2026-07-09) with full three-mode coverage (PR review, mention/issue triage, daily midnight-UTC oversight+autohealing, manual dispatch). The daily report issue is live (#180, "Daily Fro Bot Report — 2026-07-09 (UTC)"). No follow-up workflow draft is needed.

Open issues as of 2026-07-09: #180 (daily report), **#179 (operator run cancellation + sanitized failure-reason, contract 1.6.0)**, #112 (dedicated infra-only dispatch App — carried), #108 (deferred PWA push/background sync), #8 (Renovate dependency dashboard).
