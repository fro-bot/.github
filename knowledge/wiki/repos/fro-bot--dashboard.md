...
| --------------- | --------------------------------------------------------------- |
| Created         | 2026-06-14                                                      |
| Last push       | 2026-08-08 (survey 2026-08-08, HEAD `a7bbb79`; prior `58f9634` @ 2026-07-23, `cb5190d` @ 2026-07-09, `5c631a5` @ 2026-06-26, `2504939` @ 2026-06-15) |
| Language        | TypeScript (Node 24 native TS, **strip-only** for `src/`/`test/`/scripts, ESM) |
| Package manager | pnpm 11.20.0 (was 11.15.1 → 11.10.0 → 11.8.0 → 11.5.0)          |
| Node.js         | `>=24` (engines)                                                |
...
| Test framework  | Vitest 4.1.10 (server + `web/vitest.config.ts` split runs); `@testing-library/react` 16.3.2 + `jest-dom` 6.9.1 + jsdom 29.1.1 for the SPA |
| Lint            | ESLint 10.8.0 (`@bfra.me/eslint-config` 0.51.1) + `eslint-plugin-erasable-syntax-only` 0.4.2; `impeccable` UI anti-pattern detector (Design Check, CI-pinned `impeccable@3.2.1`) |
| TypeScript      | 6.0.3 (`@bfra.me/tsconfig` 0.13.1) — Renovate flags v7 available (held) |
| OpenCode plugin | `@opencode-ai/plugin` 1.18.14 (drives the in-repo Impeccable design-lint plugin at `.opencode/impeccable/`) |
| Visibility      | Public                                                          |
| Stars           | 1                                                              |
| License         | None declared                                                  |
| Security        | **CodeQL + OpenSSF Scorecard + Dependency Review + self-hosted Renovate** (all new 2026-08-08) — CI parity gap closed |
| Releases        | **CalVer (`YYYY.MM.N`)** — latest `2026.08.14` (2026-08-07); 104 total (was 58 @ 2026-07-09); GHCR image only, `package.json` version stays `0.0.0` |
...
**Present and self-hosted.** `fro-bot.yaml` pins `fro-bot/agent@3f19f02` (v0.97.0, ecosystem version co-leader as of 2026-08-08) with full three-mode coverage (PR review, mention/issue triage, daily midnight-UTC oversight+autohealing, manual dispatch). The daily report issue is live (#320, "Daily Fro Bot Report — 2026-08-09 (UTC)"). No follow-up workflow draft is needed.
