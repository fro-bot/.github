...
By the **2026-08-24 survey** (HEAD `008d10b`) there is **no structural change** — this is a steady-state maintenance interval. The security/supply-chain suite (7 workflows), multi-stage Dockerfile (digest `sha256:3638d9a6…` byte-identical), operator control surface, vendored contract (`1.6.0`), fixture harness, and in-repo Impeccable skill+plugin all persist unchanged in shape. The delta is **fleet upkeep**: agent pin advanced **v0.97.0 → v0.105.0** (`335e4f8`, +8 minors, still ecosystem front-of-fleet); CalVer releases **104 → ~114** (latest `2026.08.24`); routine dep bumps (eslint 10.8.0 → 10.8.1, vite 8.2.0 → 8.2.2, vitest 4.1.10 → 4.1.11, `@opencode-ai/plugin` 1.18.14 → 1.18.19, `@bfra.me/eslint-config` 0.51.1 → 0.51.4, `@bfra.me/tsconfig` 0.13.1 → 0.13.2, pnpm 11.20.0 → 11.22.0). Two small **additive config surfaces** appeared: a root **`opencode.json`** (`$schema` + `plugin: ["./.opencode/impeccable/plugin.ts"]`) that registers the in-repo Impeccable plugin at the OpenCode config level rather than relying solely on CI wiring, and a root **`.ignore`** managing the `.slim/clonedeps/` vendoring (un-ignoring the clone tree while excluding nested `.git/`). `pnpm-workspace.yaml` gained a **`minimumReleaseAgeExclude`** block (allowlisting `@bfra.me/eslint-config@0.51.2||0.51.3||0.51.4` + `@bfra.me/tsconfig@0.13.2` past the org-wide minimum-release-age quarantine so trusted first-party bumps land immediately). **Carried drift, sixth consecutive survey:** the operator-contract `README.md` header still reads `OPERATOR_CONTRACT_VERSION = 1.5.0` while `version.ts` pins `1.6.0`; clonedeps + vendored contract stay frozen at agent **v0.78.0** while the action now runs **v0.105.0** (by-design inspection-source skew, now ~27 agent-minors wide). Open issues flat at 5 (#370 daily report, #238, #193, #112, #8) — no new issues since 2026-07-23.
...
| --------------- | --------------------------------------------------------------- |
| Created         | 2026-06-14                                                      |
| Last push       | 2026-08-24 (survey 2026-08-24, HEAD `008d10b`; prior `a7bbb79` @ 2026-08-08, `58f9634` @ 2026-07-23, `cb5190d` @ 2026-07-09, `5c631a5` @ 2026-06-26, `2504939` @ 2026-06-15) |
| Language        | TypeScript (Node 24 native TS, **strip-only** for `src/`/`test/`/scripts, ESM) |
| Package manager | pnpm 11.22.0 (was 11.20.0 → 11.15.1 → 11.10.0 → 11.8.0 → 11.5.0) |
| Node.js         | `>=24` (engines)                                                |
...
| Frontend stack  | React 19.2.8 + react-dom, Tailwind CSS 4.3.3 (`@tailwindcss/vite`), Vite 8.2.2, `@vitejs/plugin-react-swc` 4.3.3, PWA via `vite-plugin-pwa` 1.3.0 + Workbox 7.4.1 |
| Auth library    | Arctic 3.7.0 (GitHub OAuth) — flagged abandoned by Renovate (last release 2025-05-21) |
| GitHub client   | `@octokit/core` 7.0.7, `@octokit/auth-app` 8.3.0, `@octokit/graphql` 9.0.4, retry (8.1.1) + throttling (11.0.5) plugins |
| Test framework  | Vitest 4.1.11 (server + `web/vitest.config.ts` split runs); `@testing-library/react` 16.3.2 + `jest-dom` 6.9.1 + jsdom 29.1.1 for the SPA |
| Lint            | ESLint 10.8.1 (`@bfra.me/eslint-config` 0.51.4) + `eslint-plugin-erasable-syntax-only` 0.4.2; `impeccable` UI anti-pattern detector (Design Check, CI-pinned `impeccable@3.2.1`) |
| TypeScript      | 6.0.3 (`@bfra.me/tsconfig` 0.13.2) — Renovate flags v7 available (held) |
| OpenCode plugin | `@opencode-ai/plugin` 1.18.19 (drives the in-repo Impeccable design-lint plugin at `.opencode/impeccable/`); registered at root via new `opencode.json` `plugin: ["./.opencode/impeccable/plugin.ts"]` |
| Visibility      | Public                                                          |
| Stars           | 1                                                              |
| License         | None declared                                                  |
| Security        | **CodeQL + OpenSSF Scorecard + Dependency Review + self-hosted Renovate** (all new 2026-08-08) — CI parity gap closed |
| Releases        | **CalVer (`YYYY.MM.N`)** — latest `2026.08.24` (2026-08-23); ~114 total (was 104 @ 2026-08-08, 58 @ 2026-07-09); GHCR image only, `package.json` version stays `0.0.0` |
...
The `buildDashboardApp(opts?)` / `createDashboardServer()` split, the `Result<T,E>` shape, the `Logger` + `redactSensitiveFields` helpers, and the `readSecret` pattern are all deliberately mirrored from [[fro-bot--agent]]'s `packages/gateway` and `packages/runtime` for a **deferred `@fro.bot/runtime` extraction seam**. The clone-dep manifest (`.slim/clonedeps.json`) pins both `@fro-bot/gateway` and `@fro-bot/runtime` at **`fro-bot/agent@v0.78.0`** (was `@main`) as read-only inspection sources under `.slim/clonedeps/repos/fro-bot__agent/`. Note the widening pin-vs-runtime skew: the clonedeps + vendored contract stay frozen at agent **v0.78.0** (contract `1.6.0`, unchanged since 2026-07-09), while `fro-bot.yaml` runs the agent action at **v0.105.0** as of 2026-08-24 (was v0.97.0 @ 2026-08-08, v0.94.2 @ 2026-07-23). The inspection source is intentionally frozen at the last contract-refresh point, not chased to every action bump — the skew (~27 agent-minors wide now) is by design, not drift.
...
New top-level docs/config accompany the SPA: `DESIGN.md` and `PRODUCT.md` (product/design intent), and `.impeccable/config.json` driving the `impeccable` UI anti-pattern detector wired into CI. As of 2026-08-24 a root **`opencode.json`** registers the in-repo Impeccable plugin (`plugin: ["./.opencode/impeccable/plugin.ts"]`) at the OpenCode config level — the design gate is now loadable by any OpenCode session in the repo, not only the CI Design Check job. A root **`.ignore`** (oh-my-opencode-slim managed) un-ignores the `.slim/clonedeps/` vendored tree while excluding nested `.git/` dirs, so the agent's clone-dep inspection sources stay tracked without leaking upstream git metadata.
...
Present and self-hosted, pinned to **`fro-bot/agent@335e4f8` (v0.105.0)** as of 2026-08-24 (was `3f19f02` / v0.97.0 @ 2026-08-08, `64029d5` / v0.94.2 @ 2026-07-23, `99e7d85` / v0.84.2 @ 2026-07-09, `f51adbd` / v0.77.0 @ 2026-06-26, `854072f` / v0.64.0 @ 2026-06-15) — again front-of-fleet in the surveyed ecosystem. Single three-mode job: PR review (`pull_request`), mention/issue triage (`issue_comment`, `pull_request_review_comment`, `issues`), and a daily schedule (`0 0 * * *`, midnight UTC) + manual dispatch (with an optional custom-`prompt` dispatch input).
...
Runtime deps are stable in shape from 2026-06-15 through 2026-08-08; `@hono/node-server` holds at the **^2.0.0** major (bumped 2026-07-23 from ^1.14.4), and the Octokit stack took patch bumps as of 2026-08-08: `@octokit/core` 7.0.7, `@octokit/auth-app` 8.3.0, `@octokit/graphql` 9.0.4, `plugin-retry` 8.1.1, `plugin-throttling` 11.0.5. The frontend/dev toolchain as of 2026-08-08: `react`/`react-dom` **19.2.8** (was 19.2.7), `tailwindcss` + `@tailwindcss/vite` 4.3.3, `vite` **8.2.0** (was 8.1.5), `@vitejs/plugin-react-swc` **4.3.3** (was 4.3.1), `vite-plugin-pwa` 1.3.0 + `workbox-*` 7.4.1 (exact-pinned across build/core/precaching/routing/strategies/expiration/cacheable-response/window), `@testing-library/react` 16.3.2 / `jest-dom` 6.9.1 / `jsdom` 29.1.1, `@types/react` 19.2.18 / `@types/react-dom` 19.2.4. Other dev: `@bfra.me/eslint-config` **0.51.4** (was 0.51.1), `@bfra.me/tsconfig` **0.13.2** (was 0.13.1), `eslint` **10.8.1** (was 10.8.0), `eslint-plugin-erasable-syntax-only` 0.4.2, `typescript` 6.0.3 (Renovate flags v7; held), `vitest` **4.1.11** (was 4.1.10), `jiti` 2.7.0, `@types/node` 24.13.3, and **`@opencode-ai/plugin` 1.18.19** (was 1.18.14) for the in-repo Impeccable OpenCode plugin. As of 2026-08-24 `vite` is **8.2.2** (was 8.2.0). The `@bfra.me/*` toolchain comes from [[bfra-me--works]].

`pnpm-workspace.yaml` as of 2026-08-24: `shamefullyHoist: true`, `allowBuilds` lists `@swc/core: true` + `unrs-resolver: true` + **`msgpackr-extract: false`** (comment: `@opencode-ai/plugin` is a type-only devDep whose transitive native accelerator never needs to build). New as of 2026-08-24: a **`minimumReleaseAgeExclude`** block allowlisting `@bfra.me/eslint-config@0.51.2 || 0.51.3 || 0.51.4` and `@bfra.me/tsconfig@0.13.2` — trusted first-party `@bfra.me/*` bumps skip the org-wide minimum-release-age quarantine so they land immediately rather than waiting out the cooldown window. The prior daily-pass security `overrides` (`brace-expansion`, `fast-uri`) remain **gone** — the 2026-07-23 working-dir-delivery gap that stranded them stays superseded now that Renovate + Dependency Review own the transitive-advisory path in-repo (see CI Pipeline → Security & supply-chain workflows).
...
**Present and self-hosted.** `fro-bot.yaml` pins `fro-bot/agent@335e4f8` (v0.105.0, ecosystem front-of-fleet as of 2026-08-24) with full three-mode coverage (PR review, mention/issue triage, daily midnight-UTC oversight+autohealing, manual dispatch). The daily report issue is live (#370, "Daily Fro Bot Report — 2026-08-24 (UTC)"). No follow-up workflow draft is needed.
...
| 2026-08-24 | `008d10b` | Sixth survey. **No structural change — steady-state maintenance interval.** 7 workflows, multi-stage Dockerfile (digest `sha256:3638d9a6…` byte-identical), operator control surface, vendored contract (`1.6.0`), fixture harness, in-repo Impeccable skill+plugin all durable. Delta is fleet upkeep: agent pin **v0.97.0 → v0.105.0** (`335e4f8`, +8 minors, front-of-fleet); CalVer releases 104 → ~114 (latest `2026.08.24`); dep bumps (eslint 10.8.0 → 10.8.1, vite 8.2.0 → 8.2.2, vitest 4.1.10 → 4.1.11, `@opencode-ai/plugin` 1.18.14 → 1.18.19, `@bfra.me/eslint-config` 0.51.1 → 0.51.4, `@bfra.me/tsconfig` 0.13.1 → 0.13.2, pnpm 11.20.0 → 11.22.0). Two additive config surfaces: root **`opencode.json`** (registers Impeccable plugin at OpenCode config level) + root **`.ignore`** (manages `.slim/clonedeps/` vendoring). `pnpm-workspace.yaml` gained **`minimumReleaseAgeExclude`** (allowlists trusted `@bfra.me/*` bumps past the org min-release-age quarantine). Carried drift (6th survey): contract `README.md` header still `1.5.0` vs `version.ts` `1.6.0`; clonedeps frozen at agent v0.78.0 vs action v0.105.0 (by-design, ~27-minor skew). Open issues flat at 5 (#370 daily report, #238, #193, #112, #8) — no new issues since 2026-07-23 (3rd survey running). |
