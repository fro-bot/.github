...
By the **2026-09-23 survey** (HEAD `0c7489d`, 49 commits since `a11f1b7`) the long-parked Web Push blocker is gone: issue **#238** closed `completed` on 2026-09-20 after the owner shipped a **public `/privacy` page** (#496) whose disclosures are bound to structured claims by tests. That work is what makes the rest of the interval interesting. The policy was written from the Gateway source rather than the issue text. The in-repo `web/src/privacy/README.md` records that the issue's own content list was *wrong on five counts*, and the repo then corrected its own plan twice (#499 "fabricated release step", #501 "four unearned claims") and its own policy once (#502 run-linkage disclosure). The other headline cuts the opposite way. The `wiki-writer` gate pin (`37abb495`) is now **two `private-leak.ts` fixes behind** the control-plane package it vendors. That is the stale-gate risk the 2026-09-09 survey predicted. Nothing is exposed only because the writer has no deployment path, and that gap is now filed as **#504**. See [2026-09-23 Survey Delta](#2026-09-23-survey-delta).
...
| --------------- | --------------------------------------------------------------- |
| Created         | 2026-06-14                                                      |
| Last push       | 2026-09-22 (survey 2026-09-23, HEAD `0c7489d`); 2026-09-08 (survey 2026-09-09, HEAD `a11f1b7`; prior `a7bbb79` @ 2026-08-08, `58f9634` @ 2026-07-23, `cb5190d` @ 2026-07-09, `5c631a5` @ 2026-06-26, `2504939` @ 2026-06-15) |
...
| Package manager | pnpm **11.27.0** @ 2026-09-23 (was 11.25.0 → 11.20.0 → 11.15.1 → 11.10.0 → 11.8.0 → 11.5.0) — note the `Dockerfile` still runs `corepack prepare pnpm@11.8.0` (see 2026-09-23 delta) |
| Node.js         | `>=24` (engines)                                                |
...
| Releases        | **CalVer (`YYYY.MM.N`)** — latest `2026.09.19` (2026-09-21) as of 2026-09-23; previously `2026.09.6` (2026-09-07, authored `fro-bot[bot]`); was `2026.08.14`; GHCR image only, `package.json` version stays `0.0.0` |
...
**2026-09-23: the gate cleared.** #238 closed `completed` on 2026-09-20. The unblocking artifact is a static `web/privacy.html` served at a public, unauthenticated `/privacy` route. It is backed by `web/src/privacy/claims.ts`, the structured facts, plus three test suites: `content.test.ts` binds each claim to the rendered prose, `claims.test.ts` runs a *negative sweep* so that no endpoint, key material, route/storage path, or account identifier can compile into the public page, and `build-output-no-tracking.test.ts` checks that the built output carries no tracking. The CI Design Check now scans `web/privacy.html` alongside `web/src`. The same interval hardened push against itself. #508 stops unknown reconcile inputs from destroying live subscriptions, and #511 bars auto-registering a subscription without a user action. Consent is now enforced in code as well as stated in the policy.
...
**2026-09-23: the repo fixed the claim itself.** #498 (`docs: correct the wiki-writer existence claim`) rewrote the README sentence. It now says the service "is implemented in `wiki-writer/` but is not deployed, not integrated, and holds no runtime authority — no workflow deploys it, nothing in `src/` reaches it, and its source never enters the dashboard runtime image". That matches the tree reading this page gave it. The remaining gaps are filed as **#504** (`wiki-writer is implemented but has no deploy path, no caller, and no durable replay store`). The service is honestly described now, and it is still undeployed.
...
**2026-09-23: still present, pin `dd5b534` (v0.114.1).** No follow-up workflow draft is needed. Five issues and 0 PRs are open: **#518** (`Daily Fro Bot Report — 2026-09-23 (UTC)`, today), **#504** (new, owner-filed 2026-09-20: `wiki-writer` has no deploy path, no caller, and no durable replay store), **#193** (untouched since 2026-07-10, 75 days), **#112** (untouched since 2026-06-26, 89 days), and **#8**. **#238 closed `completed` 2026-09-20.** That partly revises the 2026-09-09 reading that "the standing to-do list does not move": the owner cleared the one issue gating a built subsystem. The two dev-tooling and infra-identity issues still haven't moved. The agent's second authored merge (#481, redaction fix) landed in this window. See [2026-09-23 Survey Delta](#2026-09-23-survey-delta).

## 2026-09-23 Survey Delta

Read method: `git log`/`diff --stat` from `a11f1b7` to `0c7489d` plus a recursive tree listing, with content reads limited to README files, manifests, the `Dockerfile`, and workflows. This follows the one-tree-call-per-survey rule adopted after the 2026-09-09 corrections. Commit mix: 49 commits. Of the PRs merged since 2026-09-09, 31 are `fro-bot[bot]` (Renovate), 17 are owner-authored, and 1 is agent-authored.

### The gate pin fell behind the gate (new finding)

`wiki-writer/package.json` still pins `@fro-bot/wiki-write-core` at `fro-bot/.github#37abb495` (committed 2026-09-02). Since then the control-plane package has taken nine commits under `packages/wiki-write-core`. Two of them are **behavioral fixes to the private-leak scanner**:

- **#3839** (2026-09-06): `scan ++-prefixed added lines instead of parsing them as +++ headers`
- **#3867** (2026-09-07): `scan the context git puts after a hunk header`

Both rewrite `src/private-leak.ts` and the committed `dist/private-leak.js`, and both change `dist/gate-contract.json`. The public-only invariant is the one gate that must never lag, and the dashboard's copy is the version *before* two known misparses were fixed. This is the exact failure the 2026-09-09 survey named ("a stale `dist/` in the consumer means the write path is enforcing an older version of the gates than the repo it writes into"), and it arrived within four days of the pin.

Mitigations, in order of weight:

1. **No live exposure.** The writer has no deploy path and no caller (README after #498; issue #504), so the stale scanner guards nothing today.
2. **The #425 drift check is not evidence of currency.** Whether it would catch this depends on what it compares. A check that reads the pinned package's own `gate-contract.json` is consistent with itself by construction and cannot see upstream move. This survey did not read that source, so the question stays open. It is the first thing to confirm before #504 lands.
3. **Nothing will bump it.** Renovate does not detect `github:…&path:` references (recorded 2026-09-09), so the only refresh mechanism is a person.

The rule for [[github-actions-ci]]: **a SHA-pinned security gate needs a staleness alarm, not only a drift check.** Drift checks prove the consumer matches the pin, and only a staleness alarm proves the pin still matches the gate. Deploying the writer (#504) should be blocked on either a Renovate `customManagers` entry for the git ref or a CI step that diffs the pinned `gate-contract.json` against the control-plane `main` copy.

### Dockerfile: a second pnpm version nobody updates (new finding, present since ≤2026-06-26)

Both build stages run `corepack enable && corepack prepare pnpm@11.8.0 --activate`, while `package.json` declares `packageManager: pnpm@11.27.0`. This interval took two more `pnpm` Renovate PRs (#460 → 11.26.0, #480 → 11.27.0) and neither touched the Dockerfile. `11.8.0` is the version recorded at the 2026-06-26 survey, so the line has probably been frozen since the Dockerfile was written. Under corepack's documented precedence, the project's `packageManager` field wins inside `/app`, so the effective build version is most likely 11.27.0 and the `prepare` line is a dead default. That is an inference, not a verified build log. Either way the Dockerfile states a version the build does not use. It is the coupled-constant class in [[docker-containers]]: two declarations of one fact, with a bot watching only one of them. Fix it by deleting the `prepare` version or deriving it from `package.json`. A third `pnpm` pin would make it worse.

The same file carries a hardening step this page never recorded (present at `a11f1b7`): the runtime stage `rm -rf`s `npm`, `npx`, `pnpm`, `pnpx`, `corepack`, `yarn`, and their caches *before* `USER node`, and copies only `node_modules/`, not manifests or package-manager state. A compromised process in the final image has no package manager to fetch with. Cataloged in [[docker-containers]].

**Nuance on the README's "source never enters the runtime image".** That is accurate: the runtime copies `src/`, `public/`, `web/dist/`, and `/app/node_modules/`, and never copies `wiki-writer/src`. But `prod-deps` installs the whole workspace with `shamefullyHoist: true`, so the *dependency closure* of `wiki-writer`, including the git-fetched `@fro-bot/wiki-write-core`, is likely present under the copied root `node_modules`. This is unverified. It matters little today because the writer's Octokit versions equal the root's, and the gate package holds logic but no credentials. It is the right check to run when #504 designs the writer's own image.

### Agent-authored security fix, and the review path now executes

**#481** `fix(security): route unhandled request errors through the redacting logger` was authored by `fro-bot` (the agent identity, not the Renovate `fro-bot[bot]` one) on branch `fix/hono-onerror-redaction-chokepoint`, touched 2 files (+77/−1), and merged 2026-09-20. It is the second agent-authored merge after #414. #414 was a lint-level no-op removal; By its title and branch name, #481 closes a real redaction gap: unhandled request errors were being logged without passing through the redacting logger. The diff was not read. The branch-pr channel repaired on 2026-08-31 now carries security work.

Of the last 30 `Fro Bot` runs, 20 were `skipped` (10 `issues`, 6 `issue_comment`, 4 `pull_request`), **7 were `pull_request` successes**, and 3 were `schedule` successes. The executing PR reviews are all on owner branches (for example `fix/push-no-auto-register`, `fix/stream-no-snapshot-terminal`), and the skips are on `renovate/*` branches. The skip ratio fell from 27/30 (2026-09-09) to 20/30. The guards didn't change. The owner opened more PRs.

### `clonedeps` retired, vendored contract now unanchored

#493 removed `.slim/clonedeps.json`, the `.ignore` file, and the `.gitignore` block. Those were the local inspection checkout of `fro-bot/agent@v0.78.0` that the operator-contract `README.md` refresh procedure ("re-copy upstream and re-apply the documented import rewrites") implicitly relied on. The vendored barrel is unchanged and still frozen at `v0.78.0` while the action pin is **`v0.114.1`**, a skew of 36 minor versions. Refreshing it now means cloning the agent from scratch. The privacy work went the other way: `web/src/privacy/README.md` says its facts "describe behavior implemented in `fro-bot/agent`" and instructs "re-survey that source before changing a claim". The repo already has the right habit for prose. The vendored types have no equivalent. Carried drift, **5th survey**: the contract `README.md` header still reads `OPERATOR_CONTRACT_VERSION = 1.5.0`.

### Carried and unchanged

- **Test Scripts Load** still walks `find src`, and `SCHEDULE_PROMPT` 4b still scopes strip-only checks to `src/**/*.ts`. `wiki-writer/src` is uncovered for a second survey.
- `pnpm-workspace.yaml` override ledger unchanged (four entries). `minimumReleaseAgeExclude` grew by one (`@bfra.me/eslint-config` 0.52.2).
- Seven workflows, all pin-only changes: agent `b799b64` v0.109.4 → **`dd5b534` v0.114.1** (nine Renovate merges); `bfra-me/.github` Renovate reusable v4.26.0 → **v4.31.0** (five merges); `codeql-action` digest `1c5b675`; `docker/setup-buildx-action` v4.4.1; `docker/build-push-action` v7.4.0; Node base digest `0e0ff40c…`.
- Toolchain: React / react-dom / `@types/react*` **19.3.0**, vite **8.3.0**, eslint **10.11.0**, `@bfra.me/eslint-config` 0.52.2, `@opencode-ai/plugin` 1.18.31, `yaml` 2.9.1, `hono` 4.13.8 (lockfile). TypeScript held at **6.0.3** for a fifth survey. Vitest held at 4.1.11.
- New in-repo learnings (titles only): `public-route-swallowed-by-caddy-extensionless-rewrite`, `gateway-operator-oauth-rate-limit-shared-key-behind-caddy`, `privacy-policy-understated-run-linkage`, `state-machines-without-a-do-nothing-branch`. Also new: `docs/runbooks/gateway-access.md`. Two of the four are reverse-proxy traps. [[marcusrbrown--infra]]'s Caddy front is where this repo's bugs now surface.
- The Renovate Dependency Dashboard (#8) body was not re-read this survey (out of scope for the read constraints), so the 2026-09-09 pending-approval inventory is carried, not re-verified.
...
| 2026-09-23 | `0c7489d` | Seventh survey (49 commits; 31 Renovate / 17 owner / 1 agent merged PRs). **Web Push unblocked:** #238 closed `completed` via #496. It added a public `/privacy` page (`web/privacy.html`) bound to structured `web/src/privacy/claims.ts` by three test suites: claim↔prose binding, a negative sweep so no endpoint/key/path/account identifier reaches the public page, and a no-tracking build check. The policy was sourced from the Gateway, not the issue (the in-repo README lists five inaccuracies in #238's own list). The owner self-corrected the plan twice (#499, #501) and the policy once (#502). Push consent was hardened in code (#508, #511). Design Check now scans `web/privacy.html`. **New risk: the `wiki-writer` gate pin `37abb495` is two `private-leak.ts` fixes behind** the control-plane package (#3839, #3867, both changing `gate-contract.json`). There is no live exposure only because the writer is undeployed (README corrected by #498; gaps filed as **#504**). Renovate can't see the ref, and #425's drift check may be self-consistent by construction. **Dockerfile `corepack prepare pnpm@11.8.0`** runs against `packageManager` 11.27.0, a second pnpm pin no bot updates; runtime-stage package-manager stripping recorded for the first time. **Agent-authored #481** (redaction-logger fix) merged, the 2nd agent merge after #414. `Fro Bot` runs: 7/30 `pull_request` successes (skips 27 → 20 of 30). **`clonedeps` removed** (#493), leaving the vendored contract frozen at agent v0.78.0 vs action pin **v0.114.1** with no local refresh source; contract README still `1.5.0` (5th survey). Pins: agent v0.109.4 → **v0.114.1** (`dd5b534`), bfra-me reusable v4.26.0 → **v4.31.0**, buildx v4.4.1, build-push v7.4.0, Node digest `0e0ff40c…`. pnpm 11.27.0, React 19.3.0, vite 8.3.0, eslint 10.11.0; TS held 6.0.3. Latest release `2026.09.19`. Carried: Test Scripts Load / `SCHEDULE_PROMPT` 4b still `src`-only. Open: #518, #504, #193, #112, #8. |
