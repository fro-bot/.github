...
HEAD `5a890ee` (2026-09-25T00:58Z), **125 commits** since `fe0144c`: `mrbro-bot[bot]` Renovate is about 80, `marcusrbrown` about 45. Read scope was the unauthenticated compare API only: commit subjects and bodies, the file list, and patches for workflows, manifests, READMEs, the ignore allowlist, and the OpenCode/cortexkit/systematic configs. TypeScript sources were not read. The 2026-09-10 interval produced four postmortems. This interval turned three of them into **CI-enforced invariants**, so a regression now fails a check instead of relying on someone to notice it.

**1. The ignore allowlist had more dead negations, and now has a guard (#2615, #2625).** The 2026-08-26 rename to `.dotfiles/ignore` fixed one way allowlist entries die. Two more were still in the file:
- **A leading space makes a gitignore pattern literal.** ` !/.editorconfig`, ` !/.markdownlint-cli2.yaml`, ` !/.prettierignore`, and the indented `.claude/agents|commands|rules` lines matched nothing.
- **Tracked paths with no negation at all.** Five files inside the git dir (`ignore` itself, `README.md`, `.gitconfig`, `.prettierignore`, `.prettierrc.yaml`) were swallowed by `/.dotfiles/*`.

Both failures were invisible because **gitignore does not apply to paths that are already tracked**. The index protects the file, not the allowlist, so the damage appears only if a file is ever untracked, and then it cannot be re-added. Plain `git check-ignore` hides this too: it consults the index and reports "not ignored" for any tracked path. **Only `check-ignore --no-index` reports what the rules actually say.**

The new `ignore-audit.ts` (with tests, plus a `mise run ignore:audit` task) asserts the invariant directly: every path in `git ls-files` must be re-addable. It also flags leading-whitespace negations by line number. CI runs it with an explicit `--ignore-file .dotfiles/ignore` and passes `-c core.excludesFile=<path>` on each invocation. The reason is in a comment: a normal checkout has no `core.excludesFile` binding (that binding is set only by the bare-repo bootstrap), so without the explicit path the check would **pass vacuously**. Generalized in [[dotfiles]].

**2. The Bun scripts are now type-checked, and fixing that exposed ambient resolution (#2626).** Neither script project had type-checking. Both tsconfigs declared `"types": ["bun-types"]`, which resolved locally only because OpenCode's own **untracked** `~/.config/opencode/node_modules/` happened to exist. In CI, `.claude/scripts` failed with TS2688. A second case of the same pattern: `opencode-doctor.ts` type-checked against `@opencode-ai/sdk` 1.4.9 from that ambient copy, not the current SDK. The fix:
- A pinned toolchain manifest at `.dotfiles/package.json`: `typescript 7.0.2`, `@types/bun 1.4.2`, `prettier 3.9.8`, `@opencode-ai/sdk 1.18.32`, with a `bun.lock`.
- A `paths` mapping to the SDK's declaration entry. The SDK is exports-only with no `types`/`main`, so a directory target *silently* falls back to the ambient copy.
- `tsc --noEmit` for both projects on the ubuntu leg of the Script Tests matrix.

The manifest sits in `.dotfiles/` for a structural reason: `.config/opencode/` has a per-directory `.gitignore` that ignores `package.json`/`bun.lock`/`node_modules` for its whole subtree, and **a per-directory gitignore always wins over the bare repo's excludesFile allowlist**, so no negation could ever track a manifest placed there. 33 latent type errors surfaced in `ollama-distill.test.ts`. All were mock-typing problems (the `fetch.preconnect` static, proxy casts, a `.ts` import extension), and none were behavioral bugs.

**3. Formatting is checked in CI, and the CI tool is the pinned local tool (#2628, #2629, #2630).** Prettier is now pinned in the toolchain manifest. The `format` and `format-check` mise tasks call that exact binary instead of whatever is on `PATH`, and CI runs `prettier --check .devcontainer .dotfiles .github` on one leg. `.prettierrc.yaml` gained `embeddedLanguageFormatting: off`, because the TypeScript samples in `docs/solutions/` mirror real scripts that Prettier does not format, and restyling them would make the docs contradict the code. TypeScript and Prettier are now each pinned **twice**, in the manifest and as machine-wide mise tools, so a new Renovate `script toolchain` group matches both files and they move in one PR. Otherwise local and CI checks would drift apart.

**4. A second devcontainer break from an unpinned upstream, with the same fix (#2633 → #2636, #2637).** Upstream keychain rewrote 3.x as a Python zipapp. 2.8.5 had a `keychain` make target and 3.0.4 does not, so `make -C "$TMP_DIR" keychain` failed. The version had been resolved at build time with `find_version_from_git_tags` from `latest`, so this was the same shape as the 2026-09-01 mise/pyenv outage: **an upstream release broke the build with no commit on this side**. The fix copies #2497: `KEYCHAIN_VERSION=3.0.5` pinned under a `# renovate:` marker, the keychain script added to the `_VERSION` manager's `managerFilePatterns` at authoring time (with the reason "a marker with no matching file pattern never fires"), `set -eo pipefail`, and a `curl -fsSL` download of the prebuilt `.pyz` verified against published `SHA256SUMS`. #2637 repointed the source to the canonical `danielrobbins/keychain` repo. **The pattern is now established:** every `curl`-fetched devcontainer installer (mise, uv, starship, keychain) is pinned, Renovate-managed, fails on pipe errors, and keychain is also checksummed. Renovate has already delivered `danielrobbins/keychain` v3.0.5 (#2684). **Loose end:** Fro Bot filed **#2633** for the break, #2636 fixed it without a closing keyword, and #2633 is still open nine days later. An agent-filed issue has no human owner to close it by hand.

**5. The Fro Bot checkout now scopes its credential by trigger (#2576, "harden credential posture").** `fro-bot.yaml` added `persist-credentials: ${{ !contains(fromJSON('["pull_request", "issue_comment", "issues"]'), github.event_name) }}`. The PAT is withheld on the attacker-reachable review and triage triggers and kept only on schedule/dispatch, where the delivery path restored by #2498 needs to push. This is the same expression [[fro-bot--dashboard]] adopted on 2026-08-31, so it is now in two fleet repos, cataloged in [[github-actions-ci]]. Agent **v0.109.4 → v0.115.1** (`930ffc9`) went through 12 Renovate bumps. The prompt body is unchanged.

**6. Claude Code settings: stop tracking the file, sync from a template (#2610, #2611, #2635).** Claude Code writes machine-local state into `~/.claude/settings.json` (`enabledPlugins`, `extraKnownMarketplaces`, and `permissions` rewritten on every grant), and `settings.local.json` is not a supported user-level scope. So `settings.json` is untracked, `.claude/settings.template.json` is tracked, and `mise run claude:settings` merges the template into the local file: template keys win, keys only in the target survive, arrays union. #2611 made the writes atomic (temp file in the destination directory plus rename, preserving the mode) and added `--keep N` backup pruning. It also caught a sharp bug: `--keep abc` coerced to `NaN`, passed the `keep <= 0` guard (`NaN <= 0` is false), reached `slice(NaN)` → `slice(0)`, and **deleted every backup, including the one just written, while exiting 0**. The fix validates at parse time and guards `Number.isInteger` at the destructive call site.

**7. `AGENTS.md` moved from `$HOME` into `.dotfiles/` and was rewritten around failure modes (#2627).** At the work-tree root it loaded for every session started anywhere under the home directory. The rewrite keeps only what an agent would get wrong without being told: the work tree and project root being different directories, allowlist negations that die silently, untracking deleting live files from `$HOME`, per-directory gitignores overriding the excludesFile, and verification that passes locally but fails on a fresh checkout. **Contradiction with prior surveys:** the page previously recorded "AGENTS.md serves as the canonical knowledge base" at the root (203 tracked files "per `AGENTS.md`"). As of #2627 it is at `.dotfiles/AGENTS.md`, is no longer an inventory, and `/AGENTS.md` was removed from the allowlist. `.agents/skills/` also gained its own allowlist `.gitignore` (#2609), because global skills are installed per machine with `npx skills add`. Only `copilot-cli`, `openai-imagegen`, and the new **`probing-mcp-oauth`** (#2606, with a `docs/runbooks/mcp-remote-server-setup.md` runbook) are checked in. A `docs/runbooks/macos-system-setup.md` runbook also landed.

**8. New prompt-cache plateau detector (#2568).** `cache-plateau.ts` (846 lines plus 32 tests, `mise run opencode:cache-plateau`, run in CI) scans the OpenCode session store and Magic Context's decision log, both read-only with `busy_timeout=5000` for live WAL databases. It flags runs of consecutive low-reuse turns where cache reads stay flat while the prompt grows, excluding runs explained by an intentional context rewrite. It computes reuse as `cache.read / (input + cache.read + cache.write)`, with `input` as the uncached portion only. That is the normalization rule from finding 7 of the 2026-09-10 survey, now written into a tool. This operationalizes the Anthropic-only anchoring finding: the detector watches for the failure the upstream fix has not shipped.

**9. OpenCode routing churn continues.**
- **OMO-slim** 2.2.11 → **2.2.22**, with a Renovate hold moved from 2.2.11 to 2.2.19. That hold is stale against the plugin pin of 2.2.22, which was bumped by hand. Treat the hold as advisory, not enforced.
- `openai/gpt-5.6-*` → **`openai/gpt-6-sol`/`gpt-6-luna`** across presets (#2689). Anthropic seats `claude-opus-5` → **`claude-opus-5-5`** (#2688).
- The `openai` preset's oracle moved to `opencode-go/qwen3.8-max`. Fixers and designers were raised to `variant: high`, and librarian/explorer lost their explicit `low`.
- `backgroundJobs.continueOnIdle` was replaced by `orchestratorWake.enabled: false`.
- The **`clonedeps` skill was deleted** (#2645/#2646). Skills tree 11 → 10, and every orchestrator is now `["*", "!clonedeps", "!worktrees"]`. That negation guards a name that no longer exists.
- **Stale MCP refs persist unchanged**: `aha`/`atlassian`/`box`/`slack` are still in every `librarian.mcps` against two registered servers. This is the third survey the drift has held.
- Magic Context 0.41.4 → **0.42.6**: historian and dreamer gained `fallback_models: ["opencode-go/deepseek-v4.1-flash"]`, the `maintain-docs` dreamer task is routed to `claude-opus-5-5` (#2672), `sidekick` was removed, and the per-model Anthropic cache TTL list collapsed to **`"anthropic/*": "never"`** (#2648).
- Plugins: anthropic-auth 1.23.0, openai-auth **0.9.0**, aft 0.57.2, systematic **3.20.0** (12 bumps).
- **`systematic.jsonc` `workflow_guard.mode: "protected"` → `"disabled"`** (#2708, the HEAD commit, with no body explaining why). Variants were raised across categories. Relevant to [[fro-bot--systematic]]: the guard's first consumer-side downgrade.

**10. mise and toolchain.**
- pnpm moved to the **`aqua:pnpm/pnpm` backend** (#2582, now 12.6.0). The legacy asdf plugin does not discover pnpm 12's `.mjs` entry points and fails with an unset `BIN_PATH`. Recorded in [[mise]].
- `MISE_VERSION` 2026.9.4 → **2026.9.13** in both pins (9 bumps). uv 0.12.10 → 0.12.19 and joined the automerge list.
- Harness **`1.18.30-harness.7c479429`**. claude-code 2.1.163 → 2.1.270. copilot 1.0.87, agent-browser 0.38.1, skills 1.7.0, deno 2.9.7, biome 2.5.14, pipx 1.17.6 / poetry 2.5.1.
- `common-utils` feature 2.5.9 → 2.6.0. The `bfra-me/.github` Renovate reusable workflow v4.27.0 → v4.33.0.
- The `@cortexkit/aft-opencode` 0.32.0 Renovate hold was removed.

**11. Daemon census improved.** Fro Bot scheduled runs are **15/15 green** (09-10 → 09-24, `30 15` UTC). Across the last 100 workflow runs, **16 did work**: 12 `pull_request`/success and 4 `schedule`/success. 84 were skipped (46 `issues`, 32 `pull_request`, 6 `issue_comment`). The 2026-09-10 count was 3/100. The likely cause is the much larger share of owner-authored PRs this interval (about 45 commits by `marcusrbrown`), which pass the trust gate that Renovate PRs do not. The last 10 `Main` push runs on `main` are green.

**Fro Bot workflow: present.** No follow-up onboarding draft is needed.
...
- **Default branch:** `main`
- **Created:** 2011-06-09
- **Last push:** 2026-09-25 (was 2026-09-10)
- **`node_id`:** `MDEwOlJlcG9zaXRvcnkxODY5MTU0` (repo id `1869154`)
- **License:** _Still undetectable (2026-09-25, SHA `5a890ee`):_ the repo API still returns `license: null`, which makes this the **seventh consecutive survey** without one. _Prior reading (2026-09-10, SHA `fe0144c`):_ the GitHub License API continues to return `null` (`license: null`, `/license` endpoint 404) and no `LICENSE`/`UNLICENSE` file appears in the tree. This is now the **sixth consecutive survey** with no machine-detectable license (first flagged SHA `4df0c2d`, 2026-06-16). Prior surveys through SHA `70c211bc` (2026-06-06) recorded **The Unlicense (public domain)**. Treat license status as **unspecified**.
...
- **Open issues @ 2026-09-25:** `open_issues_count` 7, all issues and no open PRs. #444 (Dependency Dashboard) and #1924 (DB perf tuning) carry over. **#2633** (keychain break, filed by Fro Bot) is fixed by #2636 but still open. The other four are rolling daily reports, #2682/#2685/#2695/#2699 (09-21 → 09-24). PR #2560 (uv) merged 09-12.
- **Stars:** 20 (unchanged @ 2026-09-25; forks 0)
...
_2026-09-25:_ the Script Tests matrix legs now also run `cache-plateau.test.ts`, `settings-sync.test.ts` (under `.claude/scripts`), `ignore-audit.test.ts`, and the live `ignore-audit.ts` allowlist audit. The ubuntu leg only runs `bun install --frozen-lockfile` in `.dotfiles/`, `tsc --noEmit` on both script projects, and `prettier --check .devcontainer .dotfiles .github`. The `Script Tests` aggregator name is unchanged.
...
**2026-09-25 update:** the pin is now **`fro-bot/agent@v0.115.1`** (SHA `930ffc9fed93d5a6e5f2c0f9a51b4bc2fcf1e15e`). `actions/checkout` now sets a trigger-scoped `persist-credentials`, which is false on `pull_request`/`issue_comment`/`issues` (#2576). The prompts are unchanged. The paragraph below is the 2026-09-10 reading.
...
| 2026-09-25 | `5a890ee` | **Postmortems became gates**: 125 commits. **Dead allowlist negations** fixed (a leading space makes a pattern literal, and 5 git-dir files had no negation) and guarded by the new `ignore-audit.ts` in CI, which uses `check-ignore --no-index` plus an explicit `--ignore-file` so the check cannot pass vacuously (#2615/#2625). **Script type-checking in CI** (#2626) from a pinned `.dotfiles/package.json` toolchain (ts 7.0.2, @types/bun, prettier, @opencode-ai/sdk). It exposed types resolving from OpenCode's untracked `node_modules`, and per-directory `.gitignore` overriding the allowlist. **Prettier `--check` in CI** with the pinned binary and a Renovate `script toolchain` group (#2629/#2630). **Keychain 3.x zipapp rewrite broke the devcontainer** with no repo change, the second unpinned-upstream break after mise/pyenv. Fixed with a pinned, checksum-verified `.pyz`, Renovate-managed (#2636). Fro Bot's own #2633 is left open. **Trigger-scoped `persist-credentials`** in `fro-bot.yaml` (#2576). Agent v0.109.4 → **v0.115.1**. Claude `settings.json` untracked in favor of a template plus atomic `settings-sync` (the `--keep NaN` backup-wipe bug caught). **`AGENTS.md` moved to `.dotfiles/`** and was rewritten around failure modes (contradicts the prior "root knowledge base" reading). New `cache-plateau.ts` detector. OMO-slim 2.2.22, `gpt-6-sol/luna`, `claude-opus-5-5`, `clonedeps` removed. Magic Context 0.42.6 with deepseek fallbacks and `anthropic/*` TTL `never`. systematic 3.20.0 with **`workflow_guard` disabled** (#2708). pnpm on the **`aqua:` backend** (12.6.0). MISE_VERSION 2026.9.13. Harness 1.18.30. Stale `librarian.mcps` refs persist (third survey). Fro Bot 15/15 scheduled green, 16/100 runs did work (was 3/100). License still null (7th survey). |
