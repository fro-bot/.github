...
## Delta Log (2026-08-08, SHA `9e54dbc` — collision holds; no-op re-survey, queue-only motion)

A survey of the *name* `marcusrbrown/marcusrbrown.github.io` again resolves to repo **id `1174807412`** (the mrbro.dev developer portfolio, `package.json` `name: mrbro.dev`, homepage `https://mrbro.dev/`, description "My portfolio."), not the brand site (id `1021912280`, now [[marcusrbrown--marcusrbrown-com]]). The name-to-repo binding is unchanged since 2026-07-20 — the collision the 2026-07-13 header warned of is steady.

`main` HEAD is **still `9e54dbc`** — the same SHA as the 2026-08-01, -02, -05, and -07 surveys. The default-branch ref hasn't advanced in eight days (last trunk commit `fix(analytics): preserve umami pageview context (#257)`, 2026-07-31), so there is no tree-level drift: stack, workflow inventory, single-file three-mode Fro Bot design, and the single `30 3` daily oversight+autoheal cron all hold. `pushed_at` advanced to `2026-08-08T04:41:21Z`, but that reflects PR-branch pushes, not trunk. No structural drift, no contradictions with prior ingests. Because the tree is frozen, no durable delta lands on the canonical [[marcusrbrown--mrbro-dev]] page this cycle.

The only motion is in the current occupant's issue/PR queue, which is now static across two intervals:

- **Open issues 4:** #269 (Daily Fro Bot Report — 2026-08-08, the rolling report, renumbered up from #268) / #261 (`ci(fro-bot): narrow scheduled-run token permissions`) / #258 (`ci: make performance budget failures visible and actionable`) / #212 (mention-run git-push-auth failure). #1 (Dependency Dashboard, authored by `mrbro-bot[bot]`) is also open. The report-issue number climbing #268 → #269 is daily-report churn, not new work.
- **Open PRs 4 — all carried, none drained:** #266 (`fix(security): remediate high audit advisories`, author `fro-bot`) — now one survey interval old; #263 (`docs: refresh repository inventory`, author `fro-bot`) — three intervals old; #254 (`docs: correct automation script count`, author `fro-bot`) — the oldest carried PR; #253 (`chore(blog): refresh snapshot and preview images`, author `mrbro-bot[bot]`). API `open_issues_count` reads 9 because it folds the 4 PRs into the 4 issues + #1. The docs/security PR queue has now gone a full interval without a single merge — the accretion noted at 2026-08-07 (docs-refresh queue draining slower than it fills) is holding, and #266's security-remediation PR is now among the stalled set. A security-labeled fix sitting unmerged for a survey interval echoes the 2026-06-23 stuck-security-PR smell on the brand site: a red gate or an automerge pattern miss keeps the fix out of trunk.
...
| 2026-08-08 | `9e54dbc` | **Collision holds; no-op re-survey (tree frozen 8 days), queue-only motion — PR queue now static.** Name still resolves to id `1174807412` (mrbro.dev). `main` HEAD **unchanged** at `9e54dbc` since 2026-07-31 — no trunk commits, no tree drift; `pushed_at` 2026-08-08T04:41Z reflects PR-branch pushes only. No durable delta on the canonical [[marcusrbrown--mrbro-dev]] page this cycle. Queue static: open issues 4 (#269 Daily Fro Bot Report renumbered up from #268 / #261 `ci(fro-bot): narrow scheduled-run token permissions` / #258 perf-budget CI visibility / #212 mention-run git-push-auth) + #1 Dependency Dashboard (`mrbro-bot[bot]`); open PRs **4, all carried unmerged** (#266 `fix(security): remediate high audit advisories` — now 1 interval old; #263 `docs: refresh repository inventory` — 3 intervals; #254 `docs: correct automation script count` — oldest; #253 `chore(blog): refresh snapshot and preview images`, `mrbro-bot[bot]`); API `open_issues_count` 9 folds the 4 PRs. The docs/security PR queue drained zero this interval — a security-labeled fix (#266) now stalled a full cycle, echoing the 2026-06-23 stuck-security-PR smell. Agent v0.93.1 (`a4976f4`), 8 workflows, single `30 3` cron, Renovate #5.2.7, pnpm 11.1.3 / Node >=24 — all steady. No `gh` auth (unauthenticated API + raw fetch). Fro Bot active — no onboarding PR. |
