...
## Delta Log (2026-08-21, SHA `9e54dbc` — collision holds; no-op re-survey, queue accretes a *duplicate* PR)

A survey of the *name* `marcusrbrown/marcusrbrown.github.io` again resolves to repo **id `1174807412`** (the mrbro.dev developer portfolio, homepage `https://mrbro.dev/`, description "My portfolio.", topics `blog/developer/github-pages/portfolio/react/typescript/vite`), not the brand site (id `1021912280`, now [[marcusrbrown--marcusrbrown-com]]). The name-to-repo binding is unchanged since 2026-07-20 — the collision the 2026-07-13 header warned of is steady.

`main` HEAD is **still `9e54dbc`** — the same SHA as every survey since 2026-08-01. The default-branch ref hasn't advanced in **21 days** (last trunk commit `fix(analytics): preserve umami pageview context (#257)`, 2026-07-31T21:42:11Z; `updated_at` still 2026-07-31T21:42:16Z), so there is no tree-level drift: stack, workflow inventory, single-file three-mode Fro Bot design, and the single `30 3` daily oversight+autoheal cron all hold. `pushed_at` advanced to `2026-08-21T04:13:53Z`, but that reflects PR-branch pushes, not trunk. No structural drift, no contradictions with prior ingests. Because the tree is frozen, no durable delta lands on the canonical [[marcusrbrown--mrbro-dev]] page this cycle.

The queue delta this cycle is the first genuinely new signal in a week — not another renumber, but a **duplicate PR**:

- **Open issues 5:** #284 (`Daily Fro Bot Report — 2026-08-21 (UTC)`, renumbered up from #282 — daily-report churn, not new work) / #271 (`ci: reduce performance workflow permissions`, carried) / #270 (`ci: pass Vitest coverage flag through pnpm script`, carried) / #261 (`ci(fro-bot): narrow scheduled-run token permissions`, carried) / #258 (`ci: make performance budget failures visible and actionable`, carried) / #212 (`fix(fro-bot): provide authenticated git push for mention runs`, carried). #1 (Dependency Dashboard, `mrbro-bot[bot]`) is also open. All non-dashboard issues are self-filed by `fro-bot`.
- **Open PRs 5 (up from 4):** **#283 (`docs: correct automation script count`, author `fro-bot`) — new this cycle, created 2026-08-21T04:14:05Z on branch `docs/refresh-agent-script-count-2026-08-21`.** This is a **byte-for-byte title duplicate of the already-open #254** (`docs: correct automation script count`, open since 2026-07-31 on branch `docs/refresh-agent-script-count`). The autoheal daemon re-drafted the *same* docs fix into a *fresh* PR rather than updating or rebasing the stale one — the propose-without-merge pattern (durable since 2026-06-12) has now escalated to **duplicate-propose-without-merge**: the daemon is fighting its own unmerged queue by cloning it. Also carried: #266 (`fix(security): remediate high audit advisories`, author `fro-bot`) / #263 (`docs: refresh repository inventory`, author `fro-bot`) / #253 (`chore(blog): refresh snapshot and preview images`, author `mrbro-bot[bot]`). API `open_issues_count` reads **12** (was 11) — the tick is exactly the new PR #283 folded into the count. **None have merged since 2026-08-07** — the docs/security-remediation queue has now sat open ~eight-to-nine survey intervals, and #254 specifically is now shadowed by a same-titled twin.

The `fro-bot.yaml` and `package.json` at `9e54dbc` were re-read this cycle to keep the "Fro Bot active" claim honest: agent pin steady `fro-bot/agent@a4976f45a51458c349eb232aa1795f6fa25d5500 # v0.93.1`; single `30 3 * * *` daily oversight+autoheal cron (#234 consolidation holds); `package.json` name `mrbro.dev`, `packageManager pnpm@11.1.3`, `engines.node >=24.0.0`, `pnpm.overrides` empty (ledger consolidated in `pnpm-workspace.yaml`). 8 workflows (`blog-refresh`, `ci`, `copilot-setup-steps`, `deploy`, `e2e-tests`, `fro-bot`, `performance`, `renovate`); Renovate `#5.2.7`; pnpm 11.1.3 / Node >=24. Fro Bot workflow present and active — **no onboarding follow-up draft PR warranted for this name.** This run had a `gh` credential gap again (unauthenticated GitHub API + raw fetch only) — the same authenticated-access gap the current occupant's own issue #212 tracks. No new facts about the *brand site* (id `1021912280`) were observable through this name; defer to [[marcusrbrown--marcusrbrown-com]].
...
| 2026-08-21 | `9e54dbc` | **Collision holds; no-op re-survey (tree frozen 21 days), queue accretes a *duplicate* PR.** Name still resolves to id `1174807412` (mrbro.dev). `main` HEAD **unchanged** at `9e54dbc` since 2026-07-31 — no trunk commits, no tree drift; `pushed_at` 2026-08-21T04:13:53Z reflects PR-branch pushes only, `updated_at` still 2026-07-31. No durable delta on the canonical [[marcusrbrown--mrbro-dev]] page this cycle. Daily-report renumber #282 → **#284 (`Daily Fro Bot Report — 2026-08-21 (UTC)`)**, plus the first non-renumber signal in a week: **new PR #283 (`docs: correct automation script count`, author `fro-bot`, branch `docs/refresh-agent-script-count-2026-08-21`) is a byte-for-byte title duplicate of the already-open #254** (same title, open since 2026-07-31, branch `docs/refresh-agent-script-count`) — the daemon re-drafted the same docs fix into a fresh PR instead of updating the stale one, escalating propose-without-merge to **duplicate-propose-without-merge**. Open issues 5 (#284 / #271 / #270 / #261 / #258 / #212 — all carried) + #1 Dependency Dashboard (`mrbro-bot[bot]`); **open PRs 5 (up from 4)**: #283 (new dup) / #266 `fix(security): remediate high audit advisories` / #263 `docs: refresh repository inventory` / #254 `docs: correct automation script count` / #253 `chore(blog): refresh snapshot and preview images` — none merged since 2026-08-07; API `open_issues_count` **12** (was 11, tick = new PR #283). `fro-bot.yaml` + `package.json` re-read at HEAD: agent v0.93.1 (`a4976f4`), single `30 3` cron, pnpm 11.1.3 / Node >=24, `pnpm.overrides` empty. 8 workflows, Renovate #5.2.7 — all steady. No `gh` auth (unauthenticated API + raw fetch). Fro Bot active — no onboarding PR. |
