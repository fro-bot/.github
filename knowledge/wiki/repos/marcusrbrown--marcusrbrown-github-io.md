...
## Delta Log (2026-08-10, SHA `9e54dbc` — collision holds; no-op re-survey, queue-only motion)

A survey of the *name* `marcusrbrown/marcusrbrown.github.io` again resolves to repo **id `1174807412`** (the mrbro.dev developer portfolio, `package.json` `name: mrbro.dev`, homepage `https://mrbro.dev/`, description "My portfolio."), not the brand site (id `1021912280`, now [[marcusrbrown--marcusrbrown-com]]). The name-to-repo binding is unchanged since 2026-07-20 — the collision the 2026-07-13 header warned of is steady.

`main` HEAD is **still `9e54dbc`** — the same SHA as the 2026-08-01, -02, -05, and -07 surveys. The default-branch ref hasn't advanced in nine days (last trunk commit `fix(analytics): preserve umami pageview context (#257)`, 2026-07-31), so there is no tree-level drift: stack, workflow inventory, single-file three-mode Fro Bot design, and the single `30 3` daily oversight+autoheal cron all hold. `pushed_at` advanced to `2026-08-09T07:41:56Z`, but that reflects PR-branch pushes, not trunk (the repo `updated_at` field is still frozen at `2026-07-31T21:42:16Z`). No structural drift, no contradictions with prior ingests. Because the tree is frozen, no durable delta lands on the canonical [[marcusrbrown--mrbro-dev]] page this cycle.

The only motion is in the current occupant's issue/PR queue — and this cycle it accretes two more self-filed CI-hardening items, sharpening the least-privilege/CI-visibility trajectory noted at 2026-08-05:

- **Open issues 4:** #273 (Daily Fro Bot Report — 2026-08-10, the rolling report, renumbered up from #268) / **#271 (`ci: reduce performance workflow permissions`) — new self-filed least-privilege hardening item, a sibling to #261's scheduled-run token narrowing** / **#270 (`ci: pass Vitest coverage flag through pnpm script`) — new self-filed CI-plumbing fix** / #261 (`ci(fro-bot): narrow scheduled-run token permissions`) / #258 (`ci: make performance budget failures visible and actionable`) / #212 (mention-run git-push-auth failure). #1 (Dependency Dashboard, authored by `mrbro-bot[bot]`) is also open. The report-issue number climbing #268 → #273 is daily-report churn, not new work; #270 and #271 are the substantive additions. Note: the perf-budget cluster is now three-deep (#258 visibility, #271 permissions) alongside the coverage-plumbing #270 — Fro Bot is grinding on its own CI ergonomics while the trunk stays quiet.
- **Open PRs 3:** #266 (`fix(security): remediate high audit advisories`, author `fro-bot`) — still unmerged from the prior cycle; #263 (`docs: refresh repository inventory`, author `fro-bot`) — still unmerged (now three survey intervals open); #254 (`docs: correct automation script count`, author `fro-bot`) — still unmerged; #253 (`chore(blog): refresh snapshot and preview images`, author `mrbro-bot[bot]`) — still unmerged. API `open_issues_count` reads 11 because it folds the PRs into the issue count. The docs-refresh queue (#263/#254) and the security-remediation PR (#266) are all still draining slowly — the report-without-remediation / self-filed-without-merge pattern continues to accrete faster than it lands.
...
| 2026-08-10 | `9e54dbc` | **Collision holds; no-op re-survey (tree frozen 9 days), queue-only motion.** Name still resolves to id `1174807412` (mrbro.dev). `main` HEAD **unchanged** at `9e54dbc` since 2026-07-31 — no trunk commits, no tree drift; `pushed_at` 2026-08-09 reflects PR-branch pushes only (`updated_at` still frozen 2026-07-31). No durable delta on the canonical [[marcusrbrown--mrbro-dev]] page this cycle. Queue motion sharpens the least-privilege/CI-ergonomics trajectory: open issues 4 (#273 Daily Fro Bot Report renumbered up from #268 / **#271 `ci: reduce performance workflow permissions` — new self-filed hardening item** / **#270 `ci: pass Vitest coverage flag through pnpm script` — new self-filed CI-plumbing fix** / #261 `ci(fro-bot): narrow scheduled-run token permissions` / #258 perf-budget CI visibility / #212 mention-run git-push-auth) + #1 Dependency Dashboard (`mrbro-bot[bot]`); open PRs 3 (#266 `fix(security): remediate high audit advisories` — carried; #263 `docs: refresh repository inventory` — carried, now 3 intervals open; #254 `docs: correct automation script count` — carried; #253 `chore(blog): refresh snapshot and preview images` — carried); API `open_issues_count` 11 folds the PRs. Agent v0.93.1 (`a4976f4`), 8 workflows, single `30 3` cron, Renovate #5.2.7, pnpm 11.1.3 / Node >=24 — all steady. No `gh` auth (unauthenticated API + raw fetch). Fro Bot active — no onboarding PR. |
