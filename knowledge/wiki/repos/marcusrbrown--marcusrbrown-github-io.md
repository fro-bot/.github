...
## Delta Log (2026-08-22, SHA `9e54dbc` — collision holds; no-op re-survey, duplicate-PR + report renumber)

A survey of the *name* `marcusrbrown/marcusrbrown.github.io` again resolves to repo **id `1174807412`** (the mrbro.dev developer portfolio, homepage `https://mrbro.dev/`, description "My portfolio.", topics `blog/developer/github-pages/portfolio/react/typescript/vite`), not the brand site (id `1021912280`, now [[marcusrbrown--marcusrbrown-com]]). The name-to-repo binding is unchanged since 2026-07-20 — the collision the 2026-07-13 header warned of is steady.

`main` HEAD is **still `9e54dbc`** — the same SHA as every survey since 2026-08-01. The default-branch ref hasn't advanced in **22 days** (last trunk commit `fix(analytics): preserve umami pageview context (#257)`, 2026-07-31T21:42:11Z; `updated_at` still 2026-07-31T21:42:16Z), so there is no tree-level drift: stack, workflow inventory, single-file three-mode Fro Bot design, and the single `30 3` daily oversight+autoheal cron all hold. `pushed_at` advanced to `2026-08-21T07:31:13Z`, but that reflects PR-branch pushes, not trunk. No structural drift, no contradictions with prior ingests. Because the tree is frozen, no durable delta lands on the canonical [[marcusrbrown--mrbro-dev]] page this cycle.

Queue motion since 2026-08-20 is the daily-report renumber plus a fresh **duplicate** docs PR:

- **Open issues 5:** #285 (`Daily Fro Bot Report — 2026-08-22 (UTC)`, renumbered up from #282 — daily-report churn, not new work) / #271 (`ci: reduce performance workflow permissions`, carried) / #270 (`ci: pass Vitest coverage flag through pnpm script`, carried) / #261 (`ci(fro-bot): narrow scheduled-run token permissions`, carried) / #258 (`ci: make performance budget failures visible and actionable`, carried) / #212 (`fix(fro-bot): provide authenticated git push for mention runs`, carried). #1 (Dependency Dashboard, `mrbro-bot[bot]`) is also open. All non-dashboard issues are self-filed by `fro-bot`.
- **Open PRs 5:** **#283 (`docs: correct automation script count`, author `fro-bot`) — new this cycle, and a title-exact *duplicate* of the still-open #254**; #266 (`fix(security): remediate high audit advisories`, `fro-bot` — carried) / #263 (`docs: refresh repository inventory`, `fro-bot` — carried) / #254 (`docs: correct automation script count`, `fro-bot` — carried) / #253 (`chore(blog): refresh snapshot and preview images`, `mrbro-bot[bot]` — carried). API `open_issues_count` reads 12 because it folds the 5 PRs into the issue count. **None have merged since 2026-08-07** — the docs/security-remediation queue (#266/#263/#254) has now sat open ~nine survey intervals. The report-without-remediation / propose-without-merge pattern first flagged at 2026-06-12 has crossed into a new failure mode: the daemon has now *re-drafted an already-open PR* (#283 duplicating #254) rather than landing the original. Twenty-two days of a frozen trunk, and the merge loop isn't just stalled — it's re-proposing patches it already proposed. The autoheal writes faster than the human lands, and now it's shadowing its own backlog.

The `fro-bot.yaml` at `9e54dbc` was re-read this cycle to keep the "Fro Bot active" claim honest: agent pin steady `fro-bot/agent@a4976f45a51458c349eb232aa1795f6fa25d5500 # v0.93.1`; single `30 3 * * *` daily oversight+autoheal cron (#234 consolidation holds); category-10 Upstream Modernization Watch (Sundays UTC) present; fork-guard + review-mode-validation steps present. 8 workflows (`blog-refresh`, `ci`, `copilot-setup-steps`, `deploy`, `e2e-tests`, `fro-bot`, `performance`, `renovate`); Renovate `#5.2.7`; `package.json` re-read at HEAD (name `mrbro.dev`, `packageManager pnpm@11.1.3`, `engines.node >=24.0.0`, `pnpm.overrides` empty — ledger consolidated in `pnpm-workspace.yaml`). Fro Bot workflow present and active — **no onboarding follow-up draft PR warranted for this name.** This run had a `gh` credential gap again (unauthenticated GitHub API + raw fetch only) — the same authenticated-access gap the current occupant's own issue #212 tracks. No new facts about the *brand site* (id `1021912280`) were observable through this name; defer to [[marcusrbrown--marcusrbrown-com]].
...
| 2026-08-22 | `9e54dbc` | **Collision holds; no-op re-survey (tree frozen 22 days), duplicate-PR + report renumber.** Name still resolves to id `1174807412` (mrbro.dev). `main` HEAD **unchanged** at `9e54dbc` since 2026-07-31 — no trunk commits, no tree drift; `pushed_at` 2026-08-21 reflects PR-branch pushes only, `updated_at` still 2026-07-31. No durable delta on the canonical [[marcusrbrown--mrbro-dev]] page this cycle. Motion: daily-report renumber #282 → **#285 (`Daily Fro Bot Report — 2026-08-22 (UTC)`)** plus a **new duplicate docs PR #283 (`docs: correct automation script count`, `fro-bot`) — a title-exact duplicate of the still-open #254**. Open issues 5 (#285 / #271 / #270 / #261 / #258 / #212 — all carried) + #1 Dependency Dashboard (`mrbro-bot[bot]`); open PRs now **5** (#283 new duplicate / #266 `fix(security): remediate high audit advisories` / #263 `docs: refresh repository inventory` / #254 `docs: correct automation script count` / #253 `chore(blog): refresh snapshot and preview images` — #266/#263/#254/#253 all carried, none merged since 2026-08-07); API `open_issues_count` 12 folds the 5 PRs. The report-without-remediation pattern crosses into re-drafting an already-open PR (#283 ≡ #254) — the daemon is shadowing its own backlog. `fro-bot.yaml` + `package.json` re-read at HEAD: agent v0.93.1 (`a4976f4`), single `30 3` cron, pnpm 11.1.3 / Node >=24, `pnpm.overrides` empty. 8 workflows, Renovate #5.2.7 — all steady. No `gh` auth (unauthenticated API + raw fetch). Fro Bot active — no onboarding PR. |
