...
## Delta Log (2026-07-30, SHA `aa23250` — front-of-house polish: mobile nav, footer, build-time snapshots)

Re-surveyed via the `marcusrbrown.github.io` name binding (still resolves to id `1174807412` — collision holds; `package.json` `name: mrbro.dev`, homepage `https://mrbro.dev/`, description "My portfolio."). `main` advanced `345bc21` → **`aa23250`** (`docs: document mobile nav hit-target overlap (#251)`, committed 2026-07-30T05:03:56Z) across a ten-PR burst (#240–#251). This run had **no `gh` credential** — the same harness-level gap the repo's own issue #212 tracks — so reads were the untrusted-input surface via unauthenticated GitHub API + raw fetch: directory listings, `README.md`, `package.json`, `renovate.json5`, workflow files, plus the public issues/PR listing.

Unlike the last three cycles (pnpm-major crossing, in-repo design-gate, Fro Bot workflow surgery), this delta is almost entirely **front-of-house UI/UX polish plus a build-time-snapshot delivery shift** — the app's presentation layer, not its automation spine:

- **Mobile navigation hit-target fix (#249/#250/#251).** `feat(header): polish navigation and mobile layout (#249)` reworked the header/nav; `fix(header): prevent mobile nav target overlap (#250)` then patched overlapping tap targets on mobile, and `docs: document mobile nav hit-target overlap (#251)` captured the learning. A design-gate-adjacent a11y/touch-ergonomics loop — the same catch-fix-document cadence the Impeccable gate has been driving (cf. the #231/#232 hero-CTA contrast fix at 2026-07-28).
- **Footer redesigned as a structured "site-close" (#244).** `feat(footer): redesign site footer as a structured site-close` — a deliberate compositional bookend for the page. Plausibly the remediation surface for the long-open **#213** ("Homepage missing footer landmark") a11y bug, though #213's live state was not re-verified this run.
- **`/projects` now served from a build-time snapshot (#243).** `fix(projects): serve /projects from a build-time snapshot` moves the projects route off a live GitHub-API read to a prebuilt snapshot — a reliability/latency shift that pulls dynamic content into the build step, consistent with the blog's existing build-time gist-publishing model (#188/#190) and the `project-preview-refresh` snapshot script (#228). The portfolio is steadily migrating dynamic API reads to prerendered artifacts.
- **Custom social preview images honored (#242).** `feat(previews): use each repo's custom social preview when set` — project cards now use a repo's configured GitHub social-preview image when present, falling back to the self-hosted generated PNGs (#202/#228). Continuation of the preview-image saga; the self-hosted blobs are now a fallback, not the only path.
- **Blog index refined as "curated evidence" (#248)** plus routine snapshot/preview refreshes (#241/#245/#246). The blog-refresh automation is running its delivery loop — and note #240 (`fix(blog): deliver refreshed content via PR, not direct push to main`): the blog-refresh job now **opens a PR instead of pushing to `main` directly**, the natural follow-on to the #239 hook-skip and #238 dedicated-PAT work. The automation is being domesticated into the normal review path rather than committing straight to trunk.
- **Stack steady at `aa23250`:** React `^19.0.0`, React Router `^7.15.0`, TypeScript **5.9.3** (still pre-v6), Vite **7.3.6**, pnpm **11.1.3** (`engines.pnpm >=11.1.3`, `engines.node >=24.0.0`), `pnpm.overrides` still **empty** (ledger consolidated in `pnpm-workspace.yaml`). License MIT (`package.json`; API `license` null).
- **Renovate:** `github>marcusrbrown/renovate-config#5.2.7` + `group:allNonMajor` (unchanged).
...
- **Open issues (readable this run):** #252 (Daily Fro Bot Report — 2026-07-30, the consolidated daily report), #212 (authenticated git push for mention runs — still open), #1 (Dependency Dashboard). API `open_issues_count` reads 3. Notably **#213 (missing footer landmark) is no longer in the open listing** — consistent with the #244 footer redesign having closed it, though the causal link was not confirmed directly. Gaps (no Probot `settings.yml`, no CodeQL/Scorecard) still hold. No contradictions with prior ingests.

## Delta Log (2026-08-01, SHA `9e54dbc` — privacy-preserving, fail-closed Umami analytics subsystem)

Re-surveyed via the `marcusrbrown.github.io` name binding (still resolves to id `1174807412` — collision holds; `package.json` `name: mrbro.dev`, homepage `https://mrbro.dev/`, description "My portfolio."). `main` advanced `aa23250` → **`9e54dbc`** (`fix(analytics): preserve umami pageview context (#257)`, committed 2026-07-31T21:42:11Z). This run had **no `gh` credential** — the same harness gap issue #212 tracks — so reads were the untrusted-input surface via unauthenticated GitHub API + raw fetch: directory listings, `package.json`, `renovate.json5`, workflow files, `docs/analytics.md`, the #256 commit file list, plus the public issues/PR listing.

The delta is a single large feature landed across two commits (#256 `feat: add privacy-preserving Umami analytics`, #257 `fix: preserve umami pageview context`): a **self-hosted, disabled-by-default, human-gated web-analytics subsystem** that is a near-textbook application of Marcus's no-unconsented-telemetry baseline.

- **Self-hosted processor, not a third-party vendor.** The tracker points at Marcus's own Umami instance at `metrics.fro.bot` (an [[marcusrbrown--infra]]-hosted app — cf. the Umami app on the infra monorepo, `3.2.0`), never Google Analytics or any external SaaS. The `/privacy` disclosure page states plainly the processor is self-hosted and "not a third-party analytics processor."
- **Fail-closed and disabled by default.** Activation is driven by a single GitHub repo variable `UMAMI_WEBSITE_ID`, mapped to `VITE_UMAMI_WEBSITE_ID` **only on the `Build project` step** in `deploy.yaml` (step-scoped, not job/workflow env, "so it cannot leak into unrelated steps"). The variable is deliberately **unset** as of 2026-07-31 — the production build therefore contains **no tracker tag at all**. Vite injects exactly one tracker tag only for a configured production build; dev builds and unconfigured prod builds emit nothing. Malformed `VITE_UMAMI_WEBSITE_ID` values (non-canonical UUIDs) fail closed.
- **Human activation gate blocked on version-controlled infra evidence.** `docs/analytics.md` is an operator runbook with a Go/No-Go matrix: activation is forbidden until a version-controlled `marcusrbrown/infra` evidence artifact — pinned to the exact deployed infra commit and Umami version — proves pageview/custom-event records are retained ≤13 months and that monthly session parent records are deleted after their last retained child expires. The runbook explicitly says the current self-hosted Umami "retains data indefinitely, so production collection is not approved." This is remediation work that belongs to [[marcusrbrown--infra]], gating the consumer here.

  **Update 2026-09-22 (from the producer-side [[marcusrbrown--infra]] survey): the infra-side artifact exists, and has since 2026-07-31.** `apps/umami/` carries a full 13-calendar-month retention subsystem — `retention.sh` (check/dry-run default, `--apply` opt-in), `retention.sql`/`retention-check.sql`, unit + integration + systemd tests, host-native `systemd/umami-retention.{service,timer}` — and **`apps/umami/evidence/retention/`** holding `README.md`, `TEMPLATE.md`, and `2026-07-31T2012Z-mrb-go.md`. That attestation pins infra commit `aa01b493`, a content-addressed retention release hash matched against `/opt/umami/retention/current`, the Umami and Postgres image digests, SHA-256 of both installed systemd unit files, and — cross-referencing in this direction — *"`mrbro.dev` requirements/plan revision: `docs/plans/2026-07-30-001-feat-privacy-preserving-umami-metrics-plan.md` at `6eaac703`"*. It records a full recovery drill (`pg_restore --list` in a pinned disposable container with `--network none` and tmpfs data dir; exact source-vs-restored manifest `diff`, empty, PASS).

  Two caveats before reading this as Go. The status field says `GO (operator override)`, not verified — *"the calendar/catch-up timer edge remains explicitly unobserved"* — and the backup section records `Daily automated backup present: NO`. Whether those satisfy this repo's Go/No-Go matrix is a question for `docs/analytics.md`, not for the wiki; what is settled is that the **artifact the matrix demanded now exists and is version-controlled**. `UMAMI_WEBSITE_ID` state here is unchanged as of the last survey of this repo, so the consumer side is still disabled-by-default. The wiki carried "not yet observed in the infra tree" for six weeks after it landed because the evidence arrived as app-internal source, which producer-side surveys scoped to listings/READMEs/manifests/workflows do not read — the same blind spot recorded in [[github-actions-ci]] as *A Survey That Reads Only Workflows and Manifests Cannot See Application Subsystems*.
- **Bounded, typed event catalog.** Interaction events flow through a typed catalog/adapter in `src/utils/analytics.ts` (net −205 lines — legacy consent/session/queue/error/search/performance machinery deleted). Event families are a closed set: `navigation`, `section_view`, `project_open`, `blog_open`, `contact_open`, `external_profile_open`, `theme_change`. Properties are restricted to approved enumerations plus snapshot-backed project IDs / blog slugs. Pageview tracking is restricted to a closed set of approved static routes and known blog slugs.
- **Privacy contract enforced in code.** Do Not Track suppresses both pageviews and custom events; query strings, URL hashes, search queries, error strings, and user-authored theme values are never collected; no cookies, PII, raw-IP storage, persistent cross-month identifiers, fingerprinting, or cross-site tracking. Visitor hash is one-way and monthly-rotating. Umami auto-pageviews are disabled; `AnalyticsTracker.tsx` (mounted inside React Router) sends manual pageviews from normalized `location.pathname`, deduping same-path and query/hash-only mutations, flushing the latest pending route once when Umami becomes ready.
- **CSP-safe static bootstraps.** The feature also hardened the browser bootstrap surface: executable bootstraps moved out of inline `index.html` (net −100 lines there) into tested public scripts `public/scripts/{spa-redirect,spa-restore,theme-preloader}.js`, and the old `src/utils/theme-preloader.ts` (+ its test) was **removed** — CSP-compatible document policies. Node-only Ajv/frontmatter validation was split out of the browser entry graph into `src/utils/blog-validation.ts`, keeping `src/utils/blog.ts` browser-only.
- **New surfaces:** `/privacy` route (`src/pages/Privacy.tsx`, reads the same build-time enabled boolean so page status agrees with the artifact), `docs/analytics.md` runbook, `docs/plans/…umami-metrics-plan.md` + `docs/brainstorms/…umami-metrics-requirements.md`, and dense test coverage (new `tests/e2e/analytics.spec.ts` +584, `tests/scripts/static-bootstraps.test.ts` +419, `tests/scripts/analytics-config.test.ts`, plus component/page/hook tests) that intercepts tracker + collector traffic and asserts fail-closed rollback.
