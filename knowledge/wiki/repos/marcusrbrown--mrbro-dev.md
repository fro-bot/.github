...
| Linting | ESLint 10 flat config (`eslint.config.ts`) | `@bfra.me/eslint-config` |
| Formatting | Prettier | `@bfra.me/prettier-config/120-proof` |
| Type Config | TypeScript | `@bfra.me/tsconfig` |
...
| Renovate | `renovate.yaml` | (presumed standard) | Dependency management |
...
- Triggers: PR events, issue/comment events (`@fro-bot` mention), daily schedule (15:30 UTC), manual dispatch
- Uses `fro-bot/agent@v0.38.0` with `FRO_BOT_PAT` token
...
- Fork PR guard: skips bot-authored and fork PRs
...
- Production site review uses `npx agent-browser` to check mrbro.dev pages
...
- Outputs single "Daily Autohealing Report" issue
...
- **Renovate:** Extends `marcusrbrown/renovate-config#4.5.7`. Post-upgrade runs: `pnpm install`, `pnpm run build`, `pnpm run fix` (twice). Groups all non-major updates.
...
- **Shared Configs:** `@bfra.me/eslint-config`, `@bfra.me/tsconfig`, `@bfra.me/prettier-config/120-proof` — same shared config ecosystem as the Fro Bot org.
...
- Uses `fro-bot/agent@v0.38.0` in both workflow files
- Shares `@bfra.me/*` config ecosystem with the Fro Bot org
- Renovate extends `marcusrbrown/renovate-config` (same as [[marcusrbrown--ha-config]])
- Authentication via `APPLICATION_ID`/`APPLICATION_PRIVATE_KEY` secrets (GitHub App) in CI, `FRO_BOT_PAT` for agent workflows
