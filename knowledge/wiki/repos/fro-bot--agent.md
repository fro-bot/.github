...
**Limited re-check (2026-09-26, 06:08 UTC).** The five most recent `main` workflow runs
observed through `gh run list` all reference `72f11fa`, including a Renovate run
started at 06:07 UTC; this is a run SHA, not independent proof of the current
HEAD. The public root README still describes the Bun monorepo, persistent
Action sessions, and authenticated workspace executor. `gh workflow list`
shows the self-hosted Fro Bot workflow active, and `gh workflow view` confirms
the daily report, weekly project-wiki update, and manual/reusable dispatch
paths. Root `package.json` code search still declares `bun@1.4.2`, while the
shared setup action declares `1.3.14`: the previously recorded version-pin
contradiction remains. The latest published release remains v0.115.1; these
checks do not establish whether unread files changed after the previous
survey. No missing-workflow follow-up is indicated.
...
| ---------- | ---------- | ---------------------------------------------------- |
| 2026-09-26 | `72f11fa` (observed run SHA) | Limited re-check: recent `main` workflow runs still reference the previously surveyed SHA; public root README, active self-hosted Fro Bot workflow, release v0.115.1, and the Bun manifest/setup pin mismatch confirmed through permitted surfaces. Current HEAD and unread files were not compared. |
