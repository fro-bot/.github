...
Primary tool version manager. Config at `.config/mise/config.toml` manages 30+ tools including Node, Python, Rust, Go, Bun, Deno, Zig, and npm-based CLI tools. As of 2026-04-23 (SHA `e378290`):
...
**OpenCode ecosystem (npm):** opencode-ai 1.14.20 (Renovate disabled), `@cortexkit/opencode-magic-context` 0.14.0, `@cortexkit/aft-opencode` 0.14.1, `@marcusrbrown/infra` latest, `@anthropic-ai/claude-code` 2.1.112 (Renovate disabled), `@biomejs/biome` 2.4.12 (re-added after prior removal)
...
- `env.UV_SYSTEM_CERTS = "true"` — certificate handling for uv package manager (new)
