...
- [[fro-bot--fro-bot-github-io]] — Org-level custom domain holder, serves `fro.bot` via CNAME (legacy build, no application code)
...
- **fro.bot** — [[fro-bot--fro-bot-github-io]], org-level domain holder (CNAME file only, legacy build). Subpaths like `fro.bot/systematic/` are served by other repos deploying to `gh-pages` branches under the `fro-bot` org.
...
The Vite-based sites use `base: '/'` for custom domain compatibility (no path prefix needed).
