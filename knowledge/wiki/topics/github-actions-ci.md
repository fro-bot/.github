...
### Control Case: The Same Settings-Sync Caller, Wired Correctly (2026-09-01)

The mis-pathed `uses:` defect documented above for [[marcusrbrown--esphome-life]] — an `update-repo-settings.yaml` that calls `bfra-me/.github/.github/workflows/renovate.yaml` — now has a clean control. [[marcusrbrown--presentations]] carries a file of the same name, from the same template lineage, calling `…/workflows/update-repo-settings.yaml`. Same owner, same two-secret signature, same daily-cron shape, same SHA-pin-with-version-comment discipline, same reusable-workflow family — and it works.

Two things this pins down that a single broken instance could not:

- **The upstream capability is present and always was.** The esphome.life defect is purely a caller-side miswiring, not a gap that got filled later. The one-token fix is confirmed applicable, not merely plausible.
- **The two files are indistinguishable by every signal a reviewer or bot looks at.** Both are ten lines, both resolve, both report `success`, both are SHA-pinned and version-annotated, both are groomed by Renovate. The *only* difference is the basename of the `uses:` target. This is the strongest available argument for the mechanical assertion proposed earlier: **a caller whose filename is `X.yaml` should call an upstream workflow whose basename is also `X.yaml`**, and any exception should be explicit. It is a one-line check that catches a defect nothing else in the pipeline can see.

### Required Contexts Satisfied by a Different Event Than You Think (2026-09-01)

From [[marcusrbrown--presentations]]. Branch protection on `main` requires `Build`, `Test`, and **`Renovate / Renovate`**. But `renovate.yaml` has no `pull_request` trigger that produces that check on a PR head — it triggers on `issues.edited`, `pull_request.edited`, `push` to any branch except `main`, `workflow_dispatch`, and `workflow_run`. The required context reaches a PR head SHA via the **branch `push`**, not via the PR.

That is invisible while every branch is pushed to the repo itself (here, all of them are, by `mrbro-bot[bot]`). It breaks silently for a fork PR: no push to the base repo means no `Renovate` check run on that SHA means a required context that can never be satisfied, and a PR that cannot merge for reasons the UI states unhelpfully.

Generalizable rule: **a required status check is a claim about which events produce it, not just which workflow produces it.** When adding a context to branch protection, verify it is generated by an event that fires for *every* PR you intend to accept — including fork PRs, if the repo is public. Contexts produced only by `push` or only by `workflow_run` are a latent block on outside contribution. Related to, but distinct from, the "required check that cannot fail loudly" case at [[bfra-me--ha-addon-repository]]: there the check was structurally incapable of failing; here it is structurally incapable of *appearing*.

### Coverage Inversion: The Gate Runs the Trivial Suite and Skips the Real One (2026-09-01)

From [[marcusrbrown--presentations]]. `ci.yaml` runs `yarn test` against a 2017 deck whose only test is Create React App's stock 375-byte smoke render. In the same repository sits `demo/run-preset.test.sh` — a 17 KB Bash assertion suite (`assert_eq`, `assert_contains`, `assert_file`, `assert_clean_repo`) covering a script that performs `git reset --hard` on a live worktree. CI never invokes it.

The mechanism is ordinary and worth naming because it is so easy to miss: the CI workflow was written when the repo had exactly one buildable subtree, and it grew by *addition* — a Bun install, a Slidev build, a Pages assembly step — without anyone re-asking "what tests exist now?" Test discovery was implicit in the toolchain (`yarn test` finds what Jest finds), and a suite written in a different language with a different runner is invisible to it.

Rules:

- **A test file that no CI job names is decoration.** Grep the workflow for every test entrypoint in the tree, not just the ones the primary package manager knows about. Polyglot repos are where this bites — a Bash, Python, or Go suite alongside a Node project is exactly the shape that falls out of a `yarn test`-shaped gate.
- **Weight the gap by what the untested code does.** Here the uncovered suite guards a destructive script; the covered suite guards a render smoke test. Coverage percentage is the wrong lens — *consequence of failure* is the right one.
- **Caveat before "just add a step":** a suite written for a local fixture (here, an `ISOLATED_REPO` git worktree) may not be hosted-runner-viable as-is. The finding is "CI doesn't run it," not "CI trivially could."
