...
| [[marcusrbrown--vbs]]         | Present (single-file unified single-job `fro-bot.yaml` at **v0.100.0** SHA-pinned `7b9a281` as of 2026-08-19; autoheal job folded in via #594 on 2026-05-30). **2026-08-19 adds the `Validate review mode inputs` guard** (propagated from [[bfra-me--renovate-action]]) and grows the autoheal sweep **5 → 8 categories** (added Quality-Gates-Verification, Cross-Project-Intelligence, Sunday-gated Upstream-Modernization-Watch) | Autoheal `30 3 * * *`, Maintenance `30 15 * * *`; modes `review`/`maintenance`/`autoheal` via dispatch; fork-PR + bot-author guard at job `if` level; two rolling reports (`Daily Maintenance Report` #429 / `Daily Autohealing Report` #563) |
