...
- [[marcusrbrown--cortexkit-anthropic-auth]] — 4 workflow files: `ci.yml` (**`on: pull_request` only** — no default-branch verification), `release.yaml` (tag-driven, npm Trusted Publishing/OIDC + provenance, tag-commit integrity check, no manifest mutation in CI), `fro-bot.yaml` (three-mode single-file, agent **v0.45.0** — the fleet's oldest pin by a wide margin), `copilot-setup-steps.yml`. Dependabot instead of Renovate, and it has never opened a PR. **As of 2026-09-02 the Fro Bot workflow is `disabled_inactivity`** (GitHub's 60-day shutoff, last run 2026-07-30) and had already stopped writing its report six weeks earlier while running green. The fleet's reference case for automation that is present in the tree and absent in reality — see the three 2026-09-02 sections below. **2026-09-18 re-survey:** unchanged in every field (tree frozen 110 days, daemon dead 50), and the full workflow enumeration inverts the earlier lesson — the three workflows reporting `state: active` have been inert **sixty days longer** than the one GitHub flagged. Source of the two 2026-09-18 sections on workflow liveness and instrument disagreement.
...
### `state: active` Measures Enablement, Not Liveness (2026-09-18)

The [[marcusrbrown--cortexkit-anthropic-auth]] page already contributes the 60-day `disabled_inactivity` shutoff. Enumerating that repository's *full* workflow list on the seventh survey inverts the lesson: the `state` field flagged the only workflow that had been doing anything.

| Workflow | Trigger | `state` | Last run | Dead for |
| --- | --- | --- | --- | --- |
| `fro-bot.yaml` | `schedule` + events | **`disabled_inactivity`** | 2026-07-30 | 50 d |
| `ci.yml` | `pull_request` | `active` | 2026-05-31 | **110 d** |
| `release.yaml` | `push` tags, `workflow_dispatch` | `active` | 2026-05-31 | **110 d** |
| `copilot-setup-steps.yml` | `push` | `active` | 2026-05-31 | **110 d** |

Every workflow in the repository is inert. Three report `active` and have produced nothing for **sixty days longer** than the one reporting `disabled_inactivity`.

This is a definition, not a bug. `state` records whether a workflow is *permitted to run*. GitHub's inactivity policy only ever targets `schedule` triggers, because a cron is the only kind that consumes minutes on a repository nobody is touching; event-triggered workflows cost nothing while idle, so nothing disables them, so they carry a green label forever. The field answers "is this allowed to fire," and every automated health question anyone actually has is "did this fire."

Two consequences for fleet tooling:

- **A lint scanning for `state != "active"` is inversely correlated with dormancy.** The more thoroughly a repository is abandoned, the fewer of its workflows will ever be flagged, because abandonment removes the events that would have exercised them. The lint is still worth running — it is the only signal for the scheduled-daemon case, which is the case that matters most — but its silence is not evidence.
- **The complementary check is last-run recency per workflow, at one request each.** `GET /repos/{o}/{r}/actions/workflows/{file}/runs?per_page=1` returns `total_count` plus the newest run; its `created_at` discriminates all four rows above with no interpretation. Cheaper than parsing workflow YAML, strictly more informative than reading `state`, and it composes with the existing fleet sweep.

The general form: **an event-triggered workflow's liveness is a property of its triggers, not of itself.** A correctly written, correctly wired, fully permitted `pull_request` gate on a repository that receives no pull requests is indistinguishable — by every field GitHub exposes — from a working one. Only run history separates "ready and waiting" from "will never fire again." This is the static-configuration counterpart to the runtime findings already on this page ([A Run's Conclusion Measures the Harness, Not the Deliverable](#a-runs-conclusion-measures-the-harness-not-the-deliverable-2026-09-02), [A Green Run That Is Green By Design](#a-green-run-that-is-green-by-design-2026-09-15)): there, a run existed and meant less than it looked; here, no run exists and the configuration still looks healthy.

Rider, for repos whose only default-branch verification is `on: pull_request`: the missing default-branch CI is normally filed as a **coverage** gap. It is also a **liveness** gap. A PR-only gate stops being exercised the moment PRs stop arriving, which is exactly when a repository starts drifting, and nothing in the Actions UI will say so.

### Two Cheap Instruments for the Same Question Will Eventually Disagree (2026-09-18)

Same survey, second finding, and it is about how this wiki measures rather than what it found.

Five surveys of [[marcusrbrown--cortexkit-anthropic-auth]] tracked its distance from upstream using `releases/latest` — one request, human-readable, and it produced the correct conclusion every time ("upstream is alive and pulling away"). The 2026-09-02 pass added the compare API. Sixteen days later the two instruments pointed in opposite directions:

| Instrument | Reading over 2026-09-02 → 2026-09-18 | Conclusion it supports |
| --- | --- | --- |
| `releases/latest` | 1 release (`v1.22.0`, 09-02), then a **15.6-day drought** against an all-time max inter-release gap of 16.7 d | Upstream has stalled; slowest publishing window in its history |
| `compare/…` → `behind_by` | **334 → 479** commits, +145 in 16 days ≈ **9.1/day** vs a prior ≈ 3.6/day | Upstream work rate roughly **2.5×'d** |

Both are accurate about what they measure. **A release is a publishing decision; a commit is the work.** Release cadence is gated on a maintainer's judgement about when to cut a version, and it legitimately goes to zero during a long-lived branch merge, a refactor, or a vacation — none of which mean the project slowed. Commit divergence has no such gate.

Which matters depends on the question:

- **"Is upstream still the live artifact, and how expensive is a resync?"** → `behind_by`. This is the fork-relevance question, and the release feed is a lagging proxy for it that will occasionally invert.
- **"Can I consume upstream today, and at what version?"** → `releases/latest` and the registry `dist-tags`. Commits are irrelevant; unreleased work is unconsumable.

The operational rule: **when two cheap metadata instruments can answer the same question, check whether they can disagree, and name which one the conclusion depends on — before the interval in which they diverge.** The failure this prevents is not being wrong; it is being right for five intervals for the wrong reason, then quietly flipping on the sixth with no signal that the basis changed. Sibling to [A Check-Run List Is an Append Log, Not a State Snapshot](#a-check-run-list-is-an-append-log-not-a-state-snapshot-2026-09-18) — that one is a single endpoint answering a different question than the one asked; this one is two endpoints answering the same question differently.

Cheap heuristic for picking: prefer the instrument with **no human decision in its production path**. Commits, run timestamps, and file SHAs are emitted as a side effect of work. Releases, tags, and dist-tags are emitted when someone decides to emit them, and their absence is therefore ambiguous by construction.
