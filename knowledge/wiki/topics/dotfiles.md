...
- **OpenCode** (`.config/opencode/`): Full plugin stack including `oh-my-openagent` v3.17.5 (multi-agent routing with category-level model assignment), `@cortexkit/opencode-magic-context` v0.15.0 (adaptive context compaction with auto-search and git commit indexing), `@cortexkit/aft-opencode` v0.15.1 (fine-tuning), and MCP servers (context7, grep_app, tavily, exa). Default model: `opencode-go/kimi-k2.6`. Hybrid routing: primary agents (sisyphus, metis) use direct Anthropic endpoints; auxiliary agents (explore, oracle, momus) use Copilot-hosted models. Category-based routing assigns `gemini-3.1-pro` for visual/artistry, `gpt-5.4` for deep/ultrabrain, and `anthropic/claude-opus-4-6` for unspecified-high tasks.
