...
## [2026-07-04 08:10] ingest | repo:marcusrbrown/tokentoilet

Eighth survey of `marcusrbrown/tokentoilet` (HEAD `c6e10e0`, last push 2026-07-03, up from `3be6b76` 2026-06-20). Reads limited per the untrusted-input constraint: repo metadata, root and `.github/workflows` directory listings, `readme.md`, `mvp.md`, `.env.example`, `package.json`, `fro-bot.yaml`, plus open issue/PR listings and the last ~90 commits via `gh`.

Headline: the dependency-churn-only streak broke — the first substantive application code since the MVP disposal flow landed this cycle. Token discovery now enumerates real wallet ERC-20 holdings via Alchemy `getTokenBalances` (#1179), fail-closed with no static fallback and a browser-exposed key gated by an Alchemy-dashboard domain allowlist. Disposal simulates transfers before prompting a signature (#1175). Discovery error handling hardened three ways: skip unmapped chains rather than abort (#1180), classify a rejected Alchemy key as unavailable not retryable (#1183), surface structured errors in the UI (#1184), with two docs/solutions learnings captured (#1186, #1181). A mainnet readiness spike concluded NO-GO (#1178) — Sepolia-only lock is now a documented decision, stale multi-chain RPC vars dropped (#1176), invalid multi-chain tests removed (#1172). Privacy: analytics telemetry flipped to opt-in/off by default (#1174), consistent with the no-unconsented-telemetry baseline. `viem` promoted to a direct devDep (2.54.1).

Both long-standing autoheal blockers resolved: #1143 (design-system 5 missing test/story files + Web3 validator false positives) closed via #1168, ending the 12+-week recurring-failure streak; #1142 (stale wallet-test TODOs) closed, the E2E-migration need now tracked in #1171. The 35+-day-blocked `@bfra.me/eslint-config` v0.51.1 PR (#1033) landed 2026-06-20 and both fro-bot security override PRs (#1156, #1144) merged — open PRs 5→0, the highest-velocity repo in the portfolio is caught up. Dependency churn continued underneath: Fro Bot agent v0.71.0→v0.82.0 (~15 releases, SHA `77d6a46`), pnpm 11.7.0→11.9.0, Renovate preset #5.2.3→#5.2.4, prettier→3.9.4, vite→8.1.2, eslint→10.6.0, tailwindcss→4.3.2, bfra-me/.github→v4.16.33. Storybook mixed-pin footgun persists (core 10.4.6, addons on 9.0.0-alpha.*). New hygiene issue #1189 (copilot instructions reference stale pnpm 11.7.0). Fro Bot workflow present and current — no follow-up draft PR needed.

Updated repo page `marcusrbrown--tokentoilet.md` (new source `c6e10e0`, frontmatter `updated`/tags, Overview, MVP post-MVP feature section, Tech Stack, Fro Bot Integration, Shared Ecosystem table, Renovate, three Notable Observations entries, Survey History, new Notable Deltas 2026-07-03). Made additive updates to topic page [[web3-defi]] (new sections: Alchemy token discovery, simulate-before-signing, telemetry-defaults-off, mainnet no-go). Updated `index.md` summary line. Cross-refs to [[marcusrbrown--ha-config]], [[marcusrbrown--vbs]], [[marcusrbrown--renovate-config]], [[web3-defi]] kept valid.

Sources: https://github.com/marcusrbrown/tokentoilet (SHA c6e10e0515d83d00fcece101f1b6d6b0549f5a95)

## [2026-07-04 08:11] ingest | repo:marcusrbrown/tokentoilet
