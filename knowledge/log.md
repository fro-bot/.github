...
## [2026-08-30 10:42] ingest | repo:bfra-me/.github

Seventh survey of `bfra-me/.github` (HEAD `1b21990`, up from `d9feab2` 2026-08-06). Public confirmed (`private: false`, `visibility: public`, MIT, `is_template: true`). Updated repo page `bfra-me--github.md` (frontmatter `updated`/new source/two new `related` entries, Identity, Layout, Workspace, two new Custom-Actions subsections, four new prose sections, Fro Bot Integration, Renovate, Cross-Repo Relationships, Operational Notes, Open Questions, Survey History row), topic page `github-actions-ci.md` (four new pattern sections + refreshed bfra-me bullet + new tag/related), and `index.md`. Reads limited to directory listings (git trees API), README/manifest/workflow files via `raw.githubusercontent.com`, and the unauthenticated public GitHub API — `gh` had no token (`GH_TOKEN` unset), the same fleet-wide credential gap recorded in the two prior ingests.

Headline: **the first structural change since 2026-07-02, and the first at the product layer rather than the control plane.** 125 commits, 238 files changed (78 added / 74 removed / 83 modified / 3 renamed).

- **`renovate-changesets` simplification landed** (planned 2026-08-15, shipped 2026-08-19; action `0.2.45`). Deleted wholesale: `src/detectors/` (~40 files — Docker compose/file/image/version analyzers, GHA workflow parser + comparators, Go, JVM Gradle/Maven/properties parsers, npm package + lockfile parsers, Python requirements + PM analyzers, security-advisory parser/patterns/severity classifier, and the `breaking-change-{analyzers,patterns,synthesizer}` trio) and `src/summaries/` (~11 template-context/summary-builder files), plus seven `*-change-detector.ts` façades, `detector-runner.ts`, `changeset-template-engine.ts` and nine matching tests. Replaced by `classify/renovate-classifier.ts`, `extract/renovate-body-extractor.ts` + `extract/non-package-renovate-operation.ts`, `format/changeset-summary-formatter.ts`, `changesets-release-policy.ts`, `compatibility-adapter.ts`; `detectors/security-vulnerability-types.ts` renamed up to `src/analysis-types.ts`. **src 125 → 75 files.** Planned in-repo: new `docs/brainstorms/renovate-changesets-simplification-requirements.md` + `docs/plans/2026-08-15-001-refactor-renovate-changesets-simplification-plan.md`.
- **New consumer-fixture contract-test tier** — the durable artifact, cataloged in [[github-actions-ci]]. Second Vitest project (`vitest.contract.config.ts`) + `test/contract/` (12 non-fixture files: 10 `*.contract.test.ts` + `setup.ts` + `changesets-oracle.ts`, 25 tests; 19 fixture files). Fixtures model **real downstream repos**: `bfra-github/` and a deliberately hostile `marcusrbrown-infra/repo/` (apps/packages/libs workspaces, unresolvable prettier config, versionless `apps/cliproxy`+`apps/vpn`, a `.changeset/config.json` that ignores a package while enabling private-package versioning, `packages/shared` with real dependents). Tests enter the real `run()` in `src/run.ts` (never `index.ts` — importing it executes the action as a side effect), copy real temp workspaces with `fs.cp(..., {recursive: true})`, and stub only `@actions/core`/`@octokit/rest`/one exec lookup — `getExecOutput` accepts exactly `git rev-parse --short HEAD` and **throws on anything else**, so an unplanned shell-out fails loudly. The repo's own learning doc names the prior failure mode plainly: the earlier slimming pass was "scoped by local reachability — it mapped what could be deleted here, never modeling the environment the action actually runs in," and consumer correctness was "an aspiration, never a gate." Six now-executable assumptions listed on the page.
- **`.ai/` planning corpus REMOVED** — resolves the 2026-08-06 open question ("worth watching whether any graduate"). Verdict: **1 shipped** (`feature-enhanced-renovate-changesets-action-1.md` → `docs/plans/enhanced-renovate-changesets-action.md`, executed as the refactor), **1 promoted as a plan** (`infrastructure-monorepo-build-optimization-1.md`), **8 deleted outright** — cross-platform bridge, **template federation**, Astro Starlight docs platform, intelligent workflow generation, release-testing infrastructure, workflow validation system, **org-health monitoring**, production-readiness validation — plus the `implementation-plan-prompts.md` index. The two most remit-expanding proposals are among the deleted, so the prior survey's caution about "active cross-org governance" is moot: abandoned, not pursued. Two deleted needs reappeared at realistic size as human-filed issues #2545 (release.ts test coverage) / #2546 (workflow-template standards validation), and two more survived as report-only autoheal categories rather than built systems.
- **`docs/solutions/` materialized 0 → 7 entries** (README-only at 2026-08-06). Repo-local frontmatter contract that explicitly decouples itself from its generator — "written by the `ce:compound` workflow, but the frontmatter contract below is repo-local and authoritative on its own." Two tracks (Bug / Knowledge), category directories named after `problem_type`, `[slug]-[YYYY-MM-DD].md` filenames, and candid annotation that inherited Rails/Hotwire enum values don't apply here. Five entries are a post-mortem of the refactor window. Third repo in the wiki on this convention after `fro-bot/.github` itself and [[marcusrbrown--dev-like]].
- **Autoheal categories 8 → 10.** Added **9. CROSS-PROJECT INTELLIGENCE** (scan `bfra-me/works`, [[bfra-me--renovate-action]], tracked fro-bot repos for adoptable automation patterns) and **10. PROGRESSIVE IMPROVEMENT** (tool-version/CI/convention/TODO drift), both report-only; category 3 renamed **ACTION & WORKFLOW INTEGRITY**, 4 → **CODE QUALITY & MONOREPO HEALTH**. Same 8 → 10 count and converging "WORKFLOW INTEGRITY" vocabulary as [[marcusrbrown--infra]] two weeks earlier.
- **Five new autoheal guardrails**, all generalized into [[github-actions-ci]]: (1) **TRUSTED AUTHORS** gate (owner / write-collaborator / `renovate[bot]`|`dependabot[bot]`|`fro-bot` only); (2) **poisoned-branch execution guard** — "if the PR touches workflows, automation prompts, package-manager config, lockfiles, or execution scripts, do not run project commands from that branch," the sharpest of the set because autoheal *repairs by executing* on a runner holding `FRO_BOT_PAT`, so the unsafe-to-**execute** file classes are broader than the unsafe-to-**merge** ones; (3) missing-data tool-skepticism ("security alerts unavailable… Do not guess"); (4) an explicit numbered **DELIVERY CONTRACT** closing "the agent that writes the fix is the agent that ships it — do not delegate the push/PR to a 'caller workflow', there is none," which reads as a scar from a run that wrote a correct fix to disk and shipped nothing; (5) guardrail preservation ("never make checks pass by disabling tests, lowering coverage thresholds, weakening lint/type rules"). Plus a **14-day bound** on the perpetual report issue (older sections collapse into one in-place `## Historical Summary`) and an AGENT NOTES rule banning agent-addressed follow-ups.
- **New `OPENCODE_PROMPT_ARTIFACT: 'true'`** on the agent step — uploads the runtime-assembled prompt as a run artifact, making the actual instruction text auditable rather than reconstructable. First observed use in the ecosystem.
- **TypeScript frozen `<7` by an annotated Renovate rule** — `allowedVersions: '<7'`, description "Hold TypeScript below v7 because typescript-eslint lacks TS7 support; remove once support lands." This resolves the 2026-08-06 held-back PRs #2526/#2527 as a *recorded decision with an exit condition* rather than an open queue item. Cataloged as a pattern against the fleet's unannotated bump backlogs ([[marcusrbrown--gpt]] HeroUI v3 ×5 surveys, [[marcusrbrown--extend-vscode]] TS v6 ~9 weeks, [[marcusrbrown--mothership]] TS6/Biome2).
- **Queue drained 2/7 → 4/2.** All seven held-back majors resolved: lint-staged **v17.3.0** and `actions/setup-node` **v7.0.0** landed, labeler v7 / dependency-review v5 / fossa v2 landed, typescript v7 closed in favour of the pin. Open PRs are two routine `bfra-me[bot]` maintenance items. Open issues gained the **first non-bot entries across seven surveys** (#2545, #2546, both `marcusrbrown`, 2026-08-15) alongside durable #2344 / #7. Inverse of the propose-without-merge backlogs at [[marcusrbrown--sparkle]] (15) and [[bfra-me--works]] (12).
- **Two-tag-family SHA-pin rot** recorded (AGENTS.md + `docs/solutions/integration-issues/`): actions ship as `{action}@{ver}`, the repo as `v{ver}`, cut on independent triggers; Renovate's built-in `github-actions` manager resolves only `v{ver}`, so a downstream repo SHA-pinning an action with a `# {action}@{ver}` comment gets **no updates, silently**. [[marcusrbrown--infra]] sat four months behind. Fix: reusable-workflow consumption, or a regex `customManager` with `extractVersionTemplate: '^{action}@(?<version>.+)$'`. Generalized into [[github-actions-ci]] — the failure is invisible from the publisher's side, so documenting the second tag family is the publisher's burden.
- Version churn: pkg **v4.16.44 → v4.22.0** (first move off the `16` minor in surveyed history), Node 24.18.1 → **24.20.0**, pnpm 11.17.0 → **11.24.0**, agent v0.96.0 → **v0.106.1** (`d8c47fd`; sole fleet leader, ahead of dev-like/sparkle v0.105.1 and dotfiles/copilot-delegate v0.105.0), eslint 10.8.0 → 10.9.1, vitest + coverage-v8 4.1.10 → 4.1.11, vite 8.1.5 → 8.2.2, tsx 4.23.1 → 4.23.12, `@bfra.me/{eslint-config,prettier-config,tsconfig}` → 0.52.1 / 0.16.11 / 0.13.2, `bfra-me/renovate-action` 10.1.0 → **10.25.1** (24 minors, zero workflow-source change — corroborates the vendored-engine-major reading of v10). New root devDeps cluster around Changesets/workspace tooling (`@changesets/{config,should-skip-package,types}`, `@manypkg/get-packages`, `@svitejs/changesets-changelog-github-compact`, `jiti`, `eslint-config-prettier`, `eslint-plugin-prettier`) with new `build:composite:*` / `typescript:validate-type-only` / `workspace:export-*` / `workspace:standardize-scripts` scripts.
- Overrides **4 → 7** (first expansion since the esbuild remediation): added `brace-expansion@<5.0.9`, `fast-uri@>=3.0.0 <3.1.5`, `js-yaml@>=3.0.0 <3.15.1 → 3.15.2`; `flatted` 3.4.3 → 3.4.4. The brace-expansion/fast-uri pair matches the same-window additions at [[marcusrbrown--marcusrbrown-com]], [[bfra-me--works]], and [[fro-bot--dashboard]] — fleet convergence on two transitive advisories.
- **New unresolved config observation:** `pnpm-workspace.yaml` gained `minimumReleaseAgeExclude:` (exempting `@bfra.me/eslint-config` and `@bfra.me/prettier-config` from install-time release-age quarantine) with **no `minimumReleaseAge` declared anywhere surveyed** — not in `pnpm-workspace.yaml`, not in `internal.json5`, no `.npmrc` in the tree. Recorded as an open question rather than asserted; an allowlist whose enabling key isn't visible in-repo is a quiet no-op risk across package-manager upgrades.
- **Drift, two items.** (1) `CONTRIBUTING.md` pnpm/Changesets staleness persists for a **third** survey and widened again (still "pnpm 10.8.1 or later" / "Changesets 2.29.5" vs enforced 11.24.0 / 2.31.1) — and the sharper finding is that **the file was modified this window** and the stale prerequisite survived the edit, so this is no longer "the detector never looked." It also documents `test:coverage` / `test:watch` scripts that don't exist. (2) **New** — `AGENTS.md` line 15 still describes `renovate-changesets` as "(125 src files)" against an actual **75**. This is the exact claim `AUTOHEAL_PROMPT` category 5b exists to catch ("verify AGENTS.md accuracy: check that the directory structure and file counts described in AGENTS.md match reality"). An org control center running a daily self-audit is carrying stale self-description; logged as an open question because settling it requires reading the #2344 report body, not directory listings.
- **Unchanged / durable (fifth window):** 16 workflows (same names; all modified, but by SHA-pin maintenance only), 2 `review`/`autoheal` modes with `default: autoheal`, single `30 15 * * *` unified pass, `target-repo` narrowing, `cancel-in-progress: false`, workflow-level `permissions: contents: read`, OWNER/MEMBER/COLLABORATOR mention gate, 3 custom actions, 4-package workspace with `ignoreWorkspaceRootCheck`, `allowBuilds: {esbuild, unrs-resolver}`, `.github/settings.yml` self-extending `common-settings.yaml`, branch protection (12 contexts, `enforce_admins: true`, `required_linear_history`, `required_approving_review_count: 0`).
- **Fro Bot workflow present and active** (unified engine, agent v0.106.1) — no onboarding follow-up draft PR warranted.

Sources: https://github.com/bfra-me/.github (SHA 1b219902fd37d2f4206a9505e3cda8dabba786e0)

## [2026-08-30 10:44] ingest | repo:bfra-me/.github
