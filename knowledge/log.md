...
## [2026-09-03 10:05] ingest | repo:marcusrbrown/gpt

Surveyed marcusrbrown/gpt (public, MIT, node_id `R_kgDOK0Z5CA`) at HEAD `556bc738e9c5af627c98621418f6b46cdbb3984c` — the 9th survey in the series and the first where the tree is the least interesting artifact. Reads were limited to repo metadata, recursive git trees, manifests, workflow files, and issue/PR/run listings; no source files were read. GitHub API was accessed unauthenticated (60 req/hr), so file contents were pulled from `raw.githubusercontent.com` pinned to the survey SHA to stay inside the budget — worth repeating as a method note, since it makes tree-diffing two SHAs cost two API calls total.

- **The tree is effectively frozen, and it is measured, not inferred.** Recursive blob diff `f6117f0` (2026-08-08) → `556bc73`: **342 blobs before, 342 after, 0 added, 0 removed, 4 changed** — `.github/workflows/fro-bot.yaml`, `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`. Zero movement under `src/`, `tests/`, `docs/`, `RFCs/`, `scripts/`, `.github/agents/` in 26 days. The `fro-bot.yaml` diff is exactly one line: the agent pin.
- **Headline — the autoheal daemon has been delivering nothing for 15 days while reporting success daily.** Since **2026-08-20** every run summary in perpetual issue #2431 declares delivery mode `working-dir`, the contract where the caller workflow owns diff detection, commit, push, and PR creation. `fro-bot.yaml` has no such step; the job ends at `Run Fro Bot` / `timeout: 0`. Verified against the tree rather than taken on the report's word: the claimed `pnpm-workspace.yaml` overrides for `browserslist`/`ip-address`/`ws`/`brace-expansion` are absent (three overrides at HEAD — `fast-uri`, `langsmith`, `path-to-regexp` — unchanged since 08-08); the claimed new `RFCs/AGENTS.md` and `.ai/AGENTS.md` do not exist; the five claimed AGENTS.md drift repairs are byte-identical blobs. **The bot diagnosed itself on 2026-09-03**, noting the identical fix set had been recomputed six consecutive days and that the workflow has no visible commit step. It is correct, and it has been correct in a rolling issue nobody reads.
- **What makes it worse than a dead daemon:** issue writes land, file writes do not, so the only artifact the system can still produce is prose about work it did not deliver. Compare [[marcusrbrown--cortexkit-anthropic-auth]] (went silent — reads as absence) and [[bfra-me--ha-addon-repository]] (fails loudly 17× behind a skip-and-green required check). Same outcome, three distinct mechanisms, and this one is the only one that actively looks productive. Recorded in [[github-actions-ci]] with the cheap fix: assert delivery, not authorship — a post-agent `git status --porcelain` check converts the whole class from silent to loud.
- **Mechanism — Renovate bumps a harness pin as a version, not an interface.** `fro-bot.yaml`'s last 20 commits are *all* `chore(deps): update fro-bot/agent to vX` (v0.96.3 2026-08-07 → **v0.107.1** 2026-09-03, 12 minors in 27 days, same-day automerged), with zero body edits. `working-dir` first appears in the report stream one day after the v0.101.0 merge. A dependency bot reasons about version ranges and cannot reason about whether the consumer still satisfies the dependency's contract; for a library the type checker covers that gap, for a GitHub Action nothing does. Signature to recognize on sight: **a workflow file whose entire recent history is pin bumps against an action whose minor has moved ten or more.** Joins "SHA pinning validates the ref, not the path" and the 2026-09-03 "`>=` floor is a snapshot" rule as the third member of the family — pins whose correctness was established once and thereafter assumed; this one is the most expensive because a harness pin encodes behavior.
- **Two separate stalls, recorded as two.** The delivery break explains the file-write drought from 08-20. It does not explain the PR drought: the newest open PR is **#2693, created 2026-07-29**, three weeks earlier. 15 open PRs, none younger than 36 days. Cause of the earlier stall is not determinable from the permitted reads; filed under Open Questions rather than folded into the delivery story.
- **Correction to this page's own 2026-08-08 reading.** That survey called the Ollama a11y-contrast cluster "monotonically accreting" and predicted growth. It is **frozen at six** (#2664/#2665/#2672/#2673/#2674/#2692) since 2026-07-28, and every one has `updated_at` equal to its creation timestamp — opened, never rebased, never reviewed. It stopped because the generator stopped, not because the defect was fixed or dedup started working. Generalized: **a backlog that stops growing is not evidence the generator was fixed** — check whether the generator is still running before crediting a rule change.
- **DEDUPLICATION clause failure, third ecosystem instance and by far the largest.** The prompt's opening rule requires searching for an existing bot-authored item for the same root cause before creating. Six near-identical PRs over 20 days for one contrast defect, plus duplicate issues **#2171 / #2173** opened the same day with titles differing by one word. Prior instances: [[marcusrbrown--marcusrbrown-com]] #473/#523, [[marcusrbrown--mrbro-dev]] #283/#254. Two duplicates is a heuristic missing; six is a rule that does not execute. Working conclusion recorded in [[github-actions-ci]]: **a natural-language "check for duplicates first" instruction is not a deduplication mechanism** — it needs a deterministic key the tooling enforces (stable root-cause-derived branch name, idempotent label, or a pre-flight query whose empty result is a precondition for the create call).
- **Three of 13 required status checks pass without doing their nominal work.** `Deploy` is `if: github.ref == 'refs/heads/main'` and skips on every PR it gates; `E2E Test Coverage`/`E2E Test Report` skip together on a `dorny/paths-filter` miss; `Run Tests`/`Build` no-op on a cache hit — and the build key covers `src/**/*.{ts,tsx}`, `index.html`, `public/**`, `tsconfig*`, lockfile but **omits `vite.config.ts`, `src/index.css`, `eslint.config.ts`, `.tool-versions`**, so a PR touching only the build config or Tailwind entrypoint takes a hit and produces a green `Build` that compiled nothing. Cache-gated steps are the subtlest variant precisely because the gate is usually correct engineering; when the job is also a required check, the key's omissions are a merge-gate hole, not a performance detail. Also: `Prepare` is a job name in **five** workflows against one context, satisfied by whichever reports first. Generalized into [[github-actions-ci]] (four mechanisms, with [[bfra-me--ha-addon-repository]]'s bot-guard skip as the fourth) and into [[probot-settings]] as the third term of that page's rule: **a declared manifest is not an applied one; an applied setting is not a recorded one; an applied required check is not an executed one.**
- **Create-authority without close-authority.** The autoheal prompt permits opening issues and PRs without limit and permits closing exactly one class — duplicate copies of its own report issue. Result after ~5 months: 23 open issues, of which 8 are a HeroUI-v3 bed opened on a single day (2026-03-28) and 6 more are one 2026-03-25 tech-debt sweep. **#2142 "E2E Tests Disabled in CI Pipeline" is arguably already resolved** — `test-e2e.yaml.disabled` is a fossil but `test-coverage.yaml` runs `pnpm run test:e2e` on every qualifying PR — and the daemon has re-audited daily for months without being able to say so. Rule recorded: **an agent granted create-authority but denied close-authority produces a monotonically growing backlog whose age distribution is a measurement artifact rather than a signal.** The boundary is defensible; it needs an asymmetric release valve (propose-closure via label, or require reconciling existing open items in a category before opening a new one).
- **Rolling reports have no machine-stable envelope.** Twelve consecutive daily comments on #2431 use **seven distinct heading formats**, and the HTML-comment markers that do appear (`<!-- fro-bot:autoheal-summary -->`, `<!-- fro-bot-run-summary -->`, `<!-- fro-bot:daily-auto… -->`) are themselves unstable. The prompt specifies a rich schema for the issue *body* and nothing for the comment envelope. 59 accumulated reports that no tool can dedupe, diff, or trend — which is how a correctly-diagnosed delivery break stayed invisible for six days. **Specify the envelope, not just the sections:** one invariant marker plus one invariant heading, stated as literals, costs two lines.
- **Prompt drift and cross-mode leakage.** Both `PR_REVIEW_PROMPT` and `AUTOHEAL_PROMPT` describe the stack as "React 19, TypeScript 5.9, **Vite 7**" against an actual **Vite 8.2.2** — drift predating the earliest survey of this repo, inside the one file the autoheal loop is explicitly forbidden to edit. Separately `PR_REVIEW_PROMPT` carries two scheduled-mode directives ("category 7 MUST NOT bump pinned versions", "Do not create multiple summary issues") that cannot apply during a PR review — copy-paste bleed between the three prompt `env:` blocks of a single-file three-mode workflow.
- **The prompt authorizes what the harness forbids.** `AUTOHEAL_PROMPT`: *"This workflow may only push fixes, open/update PRs, open/update issues, and comment on PRs when a fix was pushed."* Job permissions back it (`contents`/`issues`/`pull-requests: write`). The harness declines. Every artifact is individually correct; the system is not; the failure is entirely in the gap between two consistent documents, and it is silent by construction.
- **Two under-recorded security controls, present before 2026-08-08 and missed by prior surveys.** (1) A **fork-head refusal step** on `issue_comment` triggers: calls `repos/{repo}/pulls/{n}` for `.head.repo.fork` and hard-fails unless it is exactly `false`, *before* the checkout that would fetch `refs/pull/N/head` with `FRO_BOT_PAT` in scope — correct shape, fails closed on `"unknown"`. (2) **Review-mode input validation** rejecting a promptless `mode=review` dispatch, the same guard [[bfra-me--renovate-action]] adopted.
- **Measurement correction carried across the whole survey series.** Prior rows recorded "open issues 25 → 22 → 23 → 36 → 38"; those are `open_issues_count`, which counts pull requests. Today's 38 = **23 issues + 15 PRs**, so the 2026-07-14 jump from 23 to 36 is substantially the arrival of the Ollama PR cluster, not an issue-filing surge. Prior rows annotated in place, not rewritten, per the additive-update rule.
- **Content corrections to this page.** (1) The Notebooks section listed `01-repo-ranger.ipynb` / `01-gpt-architect.ipynb` / `01-baroque-bitch.ipynb` since 2026-04-18; none exist at HEAD or at `f6117f0`, so the restructure predates the last survey and was carried forward unverified for at least one cycle. Current layout is `notebooks/agents/analysis/code-analyzer.ipynb` + `notebooks/templates/agent.ipynb`. Old listing retained with a dated annotation. (2) `.ai/` was described as "AI-specific configuration"; it is an 11-file planning corpus (`plan/` ×7, `docs/` ×3, `notes/` ×1), five of the plans being UI refactors describing the same unstarted work as the HeroUI-v3 issue bed. (3) `scripts/` was absent from the Key Directories table entirely despite carrying the test-report machinery (`lib/parsers/` + `lib/formatters/`, one module per test tier).
- **New this interval in the manifests:** `pnpm-workspace.yaml` gained a `minimumReleaseAgeExclude` block (`@bfra.me/prettier-config@0.16.10 || 0.16.11`, `@bfra.me/tsconfig@0.13.2`) with no in-repo `minimumReleaseAge` and no `.npmrc` — holes punched in a pnpm-default release-age cooldown, pinned to exact versions, so each entry dies on the next publish and nothing in CI notices. Expect monotonic growth of mostly-dead entries. Deps: pnpm 11.9.0 → **11.11.0**, Vite 8.2.0 → 8.2.2, ESLint 10.8.0 → **10.9.1**, Vitest 4.1.10 → 4.1.11, react-router-dom 7.18.2 → 7.18.3, LangChain line → langchain 1.5.10 / core 1.2.9 / openai 1.5.10 / anthropic 1.5.8, `@bfra.me/prettier-config` → 0.16.11, `@bfra.me/tsconfig` → 0.13.2, jsdom → 29.1.1, `eslint-plugin-react-hooks` → 7.0.1. Held: Node 24.18.0, HeroUI 2.8.10, MCP SDK 1.29.0, Zod 4.4.3, Monaco 4.7.0, `@typescript/native-preview` 7.0.0-dev.20260703.1 (two months), Renovate preset `marcusrbrown/renovate-config#5.2.4`.
- **Trigger economics worth knowing before reading any run history.** Of the last 100 `fro-bot.yaml` runs, **87 concluded `skipped`** — 63 `issues`, 12 `issue_comment`, 12 `pull_request` — against 13 `schedule` runs that executed. The bot's own writes to its perpetual issues re-fire the `issues` trigger and the job-level `if:` then rejects them by author. Skipped jobs bill nothing; the cost is that real executions are 13% of rows. Same family as the `issues: [edited]` no-op run storm already recorded for [[bfra-me--ha-addon-repository]].
- **Fro Bot workflow is present and correctly written — no onboarding follow-up draft PR warranted.** The actionable item is the inverse of an onboarding gap: the workflow exists, runs green daily, and delivers nothing. The fix is one job appended to an existing file.
- **Highest-value fleet follow-up from this survey:** grep every managed repo for `uses: fro-bot/agent@…` with no subsequent commit/push/PR step. A silent delivery break reproduces wherever this workflow shape was copied, and it presents as a healthy daemon in every metric a content-based or run-status-based sweep reads.

Process notes: (1) `knowledge/` remains ESLint-ignored and not Prettier-enforced; edits were made in place with no reformatting of untouched prose. (2) The malformed `related:` frontmatter on this page flagged in the 2026-09-02 log (wikilink syntax inside a slug list) is **fixed** this ingest — now bare slugs, matching the convention in `marcusrbrown--marcusrbrown-com.md` and `fro-bot--dashboard.md`. (3) The page now carries `node_id: R_kgDOK0Z5CA`, system-populated from the survey's own trusted metadata read. (4) Applying the 2026-09-02 method correction paid off immediately: querying `actions/workflows/…/runs`, the perpetual issue's comment stream, and the authorship distribution of open PRs is what surfaced the delivery break — a tree-only survey would have scored this repo "no structural change, steady dependency cadence" and been technically right about every word.

Sources: https://github.com/marcusrbrown/gpt (SHA 556bc738e9c5af627c98621418f6b46cdbb3984c; prior 2026-08-08 SHA f6117f0b1b79e37c2ab5476beead2056649724f5); https://github.com/marcusrbrown/gpt/issues/2431 (Daily Autohealing Report, 59 comments, last 2026-09-03); https://github.com/marcusrbrown/gpt/issues/2153 (Daily Maintenance Report, 104 comments); https://github.com/marcusrbrown/gpt/actions/workflows/fro-bot.yaml (3,750 runs; last 13 scheduled all success); https://github.com/marcusrbrown/gpt/blob/556bc73/.github/workflows/fro-bot.yaml; https://github.com/marcusrbrown/gpt/blob/556bc73/.github/settings.yml; https://github.com/marcusrbrown/gpt/blob/556bc73/pnpm-workspace.yaml

## [2026-09-03 10:09] ingest | repo:marcusrbrown/gpt
