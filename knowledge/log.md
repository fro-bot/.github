...
## [2026-09-01 12:20] ingest | repo:marcusrbrown/Presentations

Second survey of `marcusrbrown/Presentations` (HEAD `4613f997`, up from `34321a4` on 2026-08-05). Public confirmed (`private: false`, `visibility: public`, id `82713396`, not a fork, not a template). Updated repo page `marcusrbrown--presentations.md` (frontmatter `updated`/new source/expanded tags + `related`, dated two-state Overview, structure table with `First seen` column, four new sections, two contradiction resolutions, rewritten CI/CD section, Renovate dashboard section, strengthened-and-hedged Fro Bot recommendation, three new Notable Patterns, Survey History row), topic pages `probot-settings.md` (silently-ignored declarative key), `github-actions-ci.md` (settings-sync control case, required-context-event rule, coverage inversion), `github-pages.md` (multi-toolchain `_site` assembly, custom-domain inheritance, repo listed), and `index.md` (stub repo entry replaced with a full summary; probot-settings/github-actions-ci/github-pages entries extended).

Reads limited to directory listings, README/manifest/workflow files, and the unauthenticated public GitHub API — `gh` had no token (`gh auth status`: "not logged into any GitHub hosts"), the same fleet-wide credential gap recorded in the 2026-08-30 dev-like, 2026-08-30 esphome.life, and 2026-08-31 ha-addon-repository ingests. Delivery mode `working-dir`; no GitHub write operations attempted. `.github/corrections-context.json` read: `corrections: []`, nothing to preserve.

Headline: **the decks stood still and the infrastructure around them moved a long way — including two prior-survey claims that had to be corrected.**

- **21 commits since `34321a4`.** Six human (`#57`–`#60`, `#62`), the rest `mrbro-bot[bot]` Renovate automerges. Tree 42 → 53 entries (33 blobs); all growth inside `Cheap-LLMs-Meetup-Aug-2026/` plus a new root `index.html`. The 2017 deck's subtree is byte-identical apart from a `yarn.lock` maintenance pass.
- **`ci.yaml` changed category** (`feat(deploy): publish both presentation decks (#60)`, 1398 → 3102 B). It is no longer a build gate but the archive's publishing pipeline: `Build` still builds the CRA deck under Yarn/Node 20, then installs Bun (`oven-sh/setup-bun` v2.2.0) and builds the Slidev deck, assembles `_site/` (root `index.html` + both build outputs into per-deck subdirectories), and runs `configure-pages` v6 + `upload-pages-artifact` v5 behind a main-only guard; a new `Deploy` job (`needs: [build, test]`, `environment: github-pages`) holds the only `pages: write` + `id-token: write` grant and runs `deploy-pages` v5. Notably the assembly step is *unguarded*, so PR builds exercise the layout without publishing. Supersedes the prior page's "the Slidev deck has no CI coverage."
- **New finding (dead deploy path):** the CRA deck still carries `gh-pages 6.3.0` plus `predeploy`/`deploy` scripts that nothing invokes after `#60`, and Renovate independently lists `gh-pages` as abandoned upstream. Two signals for the same deletion.
- **New finding (coverage inversion):** `#57` added `Cheap-LLMs-Meetup-Aug-2026/demo/` — ~33 KB of `set -euo pipefail` Bash including `run-preset.test.sh`, a 17 KB assertion suite (`assert_eq`/`assert_contains`/`assert_file`/`assert_clean_repo`). **CI never runs it**, while CI *does* run the 2017 deck's stock 375-byte CRA smoke test. The uncovered suite guards `reset-demo.sh`, which performs `git reset --hard` on a live worktree. Generalized into [[github-actions-ci]]: a test file no CI job names is decoration; polyglot entrypoints are invisible to a `yarn test`-shaped gate; weight the gap by consequence of failure, not coverage percentage — with the caveat that a suite written for a local `ISOLATED_REPO` fixture may not be hosted-runner-viable as-is.
- **New finding (demo harness / cross-repo coupling):** the rehearsal rig targets [[marcusrbrown--mothership]] — `EXPECTED_REMOTE=git@github.com:marcusrbrown/mothership.git`, `DEMO_BRANCH=demo/cccc-aug-2026`, a pinned `BASELINE` commit, and a printed RED-test hint (`bun test src/app/StartupHandshake.test.ts -t 'LiveServerStatus'`). The talk demos agents doing TDD on Marcus's own agentic IDE. `run-preset.sh` provisions a temporary OpenCode config dir and manipulates `${HOME}/.config/opencode/opencode.json` to switch model presets, with `trap`s on `EXIT`/`INT`/`TERM` that reset-then-cleanup while preserving the original exit status. `reset-demo.sh` proves identity six ways (worktree exists, is a git worktree, is the *root*, both `remote.origin.url` and `pushurl` match, baseline commit object exists, demo branch exists) before destroying state and re-checks `--porcelain` afterward — recorded as a reusable template for destructive helpers. Flipped the repo's reported `language` from JavaScript to **Shell**.
- **New finding (design standard without a gate):** the new root `index.html` is a zero-dependency static file whose palette is declared entirely in **OKLCH** custom properties with inline WCAG-AA contrast justifications, a `prefers-color-scheme: dark` block annotated *"Matches Spectacle #1F2022"*, `a:focus-visible` rings, `max-width: 65ch`, and `aria-label` on the archive section — the full `.impeccable` design vocabulary from [[marcusrbrown--mrbro-dev]] / [[fro-bot--dashboard]], in a file with no build step, no linter, and no gate to enforce it.
- **Contradiction resolved (archive intent):** the 2026-08-05 survey left two readings open and warned that "the next successful settings-sync run will freeze the repo." That prediction is falsified. `update-repo-settings.yaml` targets the **correct** upstream workflow; Actions history shows `Update Repo Settings` concluding `success` on its daily `19 14` cron on 08-28/29/30/31 and on push to `main`; every *other* key in `settings.yml` demonstrably applies (description, homepage, 10 topics, branch protection); and a human edited that exact file on 2026-08-05 (`#62`) and left `archived: true` in place. Conclusion: **Probot Settings silently ignores `repository.archived`** — it drops the key without erroring. Recorded in [[probot-settings]] with three consequences: green means the workflow ran, not that a key was reconciled (only a read-back diff distinguishes applied / dropped / mis-targeted); silently-ignored declarative config is worse than absent config, because every reader — including this wiki's first survey — treats it as enforced; and lifecycle keys should be treated as unverified until observed taking effect.
- **New finding (control case):** this repo's `update-repo-settings.yaml` calls `bfra-me/.github/.github/workflows/update-repo-settings.yaml`, where [[marcusrbrown--esphome-life]]'s file of the same name calls `…/renovate.yaml`. Same owner, same family, same two-secret signature, same cron shape, both SHA-pinned with version comments, both green. This is the clean control proving the esphome.life defect is caller-side miswiring rather than a missing upstream capability, and the strongest argument yet for the proposed basename-match assertion.
- **New finding (required context, wrong event):** `settings.yml` now requires `Renovate / Renovate` on `main` (added in `#62`), but `renovate.yaml` has no `pull_request` trigger producing it — the context reaches a PR head SHA via the branch `push`. Invisible while every branch is pushed to the repo by `mrbro-bot[bot]`; a permanently unsatisfiable required context on the first fork PR. Generalized into [[github-actions-ci]] as the inverse of ha-addon-repository's "required check that cannot fail loudly": here the check is structurally incapable of *appearing*.
- **Contradiction resolved (`OUTLINE.md`):** the prior page recorded it as "deliberately removed from source control," correct at `34321a4` (no such blob in that tree). At `4613f997` it is **present at 15788 bytes**, alongside new `GLOSSARY.md` (4876 B) and `demo/NOTES.md` (14250 B). The removal was reverted post-talk; the durable privacy line is the directory `.gitignore`, which still permanently excludes `HANDOFF.md`/`TALK-TRACK.md`/`DEMO-RUNBOOK.md`. Both observations preserved per the additive/contradiction rule.
- **New finding (abandoned foundations at both ends):** Renovate's dashboard flags three abandoned dependencies — `react-scripts` (last release 2025-02-14, the *entire* build toolchain of the 2017 deck), `gh-pages` (2025-01-02), and **`@slidev/theme-seriph` (2024-02-02)**, the theme of the brand-new 2026 deck, chosen three weeks ago and already older in maintenance terms than most of the 2017 stack. Recorded as **newly chosen ≠ maintained**: recency of adoption says nothing about upstream liveness, and the only reason the scanner sees the Slidev deck at all is that `bun.lock` was checked in (`#56`).
- **Queue state:** 4 majors parked as unchecked *Pending Approval* boxes (`actions/checkout` v7 — three majors behind the pinned v4.4.0, `actions/setup-node` v7, Node.js v24 against a pinned 20.20.2, react monorepo v19), plus 2 open pin PRs (#54 `@slidev/cli`, #55 `@slidev/theme-seriph`) stalled ~28 days while every version-bump PR automerges same-day — inferred to fall outside the preset's automerge rules, flagged as inference since the preset matrix was not read. #55 is semantically a no-op (`^0.25.0` already resolves to the only release). No CVEs on osv.dev. All 8 `ci.yaml` actions are SHA-pinned *with* version comments, so no `setup-yq`-class blind spot.
- **Dependency churn:** `bfra-me/.github` v4.16.44 → **v4.23.0** (SHA `eb1772eb`) across ten minor boundaries in 22 days (#64–#77), matching the identical burst observed at [[marcusrbrown--esphome-life]] over the same window; [[marcusrbrown--renovate-config]] preset `#5.2.10` → `#5.2.12`.
- **Metadata deltas:** homepage `marcusrbrown.github.io/Presentations/` → **`mrbro.dev/Presentations/`** — not a hosting move, but the apex inherited from the user Pages site ([[marcusrbrown--mrbro-dev]]); topics 6 → 10 (`opencode`, `cheap-llms`, `slides`, `slidev`); root `.gitignore` is now OpenCode-aware (ignore-then-allowlist for `.opencode/commands|themes|tui.json`, plus `.worktrees`) despite no `.opencode/` files being tracked — anticipatory agent-workspace hygiene. Stars 1, forks 0, watchers 1, `open_issues_count` 3 (= 2 PRs + dashboard #41). `archived: false` re-confirmed.
- **No Fro Bot workflow — second consecutive survey.** The prior recommendation was gated on resolving the archive question; that gate is now cleared, and the survey produced four concrete items a resident daemon would plausibly have raised (unrun test suite, dead `gh-pages` path, already-abandoned theme, misleading `archived: true`). Recommendation strengthened but explicitly hedged: a 1-star, 0-fork personal archive with a same-day-draining Renovate queue is exactly the profile where agents accumulate unmerged proposals ([[marcusrbrown--sparkle]] 15, [[bfra-me--works]] 12), so a follow-up draft PR should prefer the converged-autoheal null-verdict shape from [[marcusrbrown--dev-like]] over propose-everything.

Sources: https://github.com/marcusrbrown/Presentations (SHA 4613f997); https://github.com/marcusrbrown/Presentations/issues/41 (Dependency Dashboard, 2026-08-31); https://github.com/marcusrbrown/Presentations/actions (Update Repo Settings scheduled runs, 2026-08-28 through 2026-08-31)

## [2026-09-01 10:23] ingest | repo:marcusrbrown/Presentations
