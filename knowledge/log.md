...
## [2026-08-31 02:15] ingest | repo:marcusrbrown/extend-vscode

Sixteenth survey of `marcusrbrown/extend-vscode` (HEAD `2a3ec002`, up from `9ecc7a55` 2026-08-02). Public confirmed (`private: false`, id `313368595`, not a fork, not archived). Updated repo page `marcusrbrown--extend-vscode.md` (frontmatter `updated`/new source/expanded `tags`+`related`, Overview refresh, enumerated `.ai/`+`.cursor/` corpus, new "Effective update policy" and "Release posture" sections, new reusable-workflow wiring + concurrency-typo subsections, corrected Fro Bot/Probot section, four new Notable Patterns, full 2026-08-31 Delta Log, new Survey History table), topic pages `vscode-extensions.md` (new Release Posture section), `github-actions-ci.md` (two new pattern sections + related link), `probot-settings.md` (working-reference wiring + `_extends` owner-resolution correction), and `index.md` (repo entry rewritten; vscode-extensions/github-actions-ci/probot-settings entries extended).

Reads limited to directory listings, README/manifest/workflow files, and the unauthenticated public GitHub API — `gh` had no token (`GH_TOKEN` unset), the same fleet-wide credential gap recorded in the 2026-08-30 dev-like and esphome.life ingests. Delivery mode `working-dir`; no GitHub write operations attempted, no issue opened or commented.

Headline: **the tree did not move, the queue drained to zero, and a finding carried across three prior surveys turned out to be wrong.**

- **15 commits since 2026-08-02 (#522–#535 plus the long-pending #508), every one a `mrbro-bot[bot]` Renovate automerge.** Recursive tracked-tree path list is **byte-identical** at `9ecc7a55` and `2a3ec002` — 156 blobs each, zero added, zero removed. Sixteenth consecutive survey with no structural, architectural, or workflow change.
- **Open PR queue drained to 0** — first zero-PR reading in the series. Open issues flat at 5 (#142 `Uplift vscode-bash`, #162 Dependency Dashboard, #317–#319 Advanced Testing Infrastructure Phases 3–5), untouched since 2025-08-17. Stars 2, watchers 2, subscribers 1, forks 0. Puts extend-vscode in the drain-clean cohort with [[marcusrbrown--dev-like]] and [[marcusrbrown--esphome-life]], opposite the propose-without-merge backlogs at [[marcusrbrown--sparkle]] (15 open) and [[bfra-me--works]] (12 open) — though see the suppression finding below for why that comparison is not apples-to-apples here.
- **Correction (three prior surveys retracted):** PR #508 was recorded on 2026-06-29, 2026-07-13, and 2026-08-02 as `chore(deps): update pnpm to v11 [SECURITY]` — a stalled major on the automerge track — with a standing hypothesis that a `packageManager`/lockfile lockstep mismatch was gating it. At merge (2026-08-13, commit `e7e222dd`) the title read `update pnpm to v10.34.4 [SECURITY]` and the diff is one line: `"packageManager": "pnpm@10.34.0"` → `"pnpm@10.34.4"`. Renovate had retargeted branch `renovate/npm-pnpm-vulnerability` from the v11 line to the in-major patch carrying the same fix — **CVE-2026-50021 / GHSA-q6j5-fjx5-2mc3**, pnpm's tarball-extraction worker skipping integrity verification when the lockfile `integrity` field is absent. The security fix landed; no major was ever pending. What survives: the 47-day stall (2026-06-27 → 2026-08-13) despite `security` + `automerge` labels was real. Generalized into [[github-actions-ci]] as **"a Renovate PR title is mutable state"** — titles and bodies are rewritten on retarget, branch names are stable, the diff is ground truth, and long-lived bot PRs must be re-read at merge rather than carried forward from open.
- **New finding (root cause for a lag pattern seen fleet-wide):** `.github/renovate.json5`'s first `packageRules` entry disables *all* patch updates except `typescript`. Measured consequences — the repo's own Renovate preset pin is frozen at `marcusrbrown/renovate-config#5.2.0` since 2026-05-14 while the fleet runs `#5.2.12` (12 patch releases behind), and `pnpm/action-setup` is stuck at v6.0.0 against a fleet v6.0.9. **The repo's dependency policy is defined by a preset that the repo's dependency policy prevents it from updating.** Security updates bypass `enabled: false`, which is why every `[SECURITY]` bump in the repo's history (`tmp` #494/#505, `form-data` #502, pnpm #508) merged same-day while ordinary patches never appeared: the queue reads as immaculate because an entire update class was muted. Third member of the family alongside esphome.life's calver suppression and its untracked CDN URL; cataloged in [[github-actions-ci]] with the audit question — for each `enabled: false` rule, what is currently sitting behind it?
- **New finding (fleet position):** with `packageManager: pnpm@10.34.4` and `lockfileVersion: '9.0'`, extend-vscode is now the **last pnpm-10 repository in the fleet** ([[marcusrbrown--sparkle]] 11.24.0, [[marcusrbrown--marcusrbrown]] 11.22.0, [[bfra-me--works]]/[[fro-bot--dashboard]] 11.20.0). `pnpm-workspace.yaml` still uses the pnpm-10 `onlyBuiltDependencies:` key, so the eventual v11 crossing carries an `allowBuilds:` rewrite plus lockfile regeneration, not just a field bump. The workspace override ledger is recorded for the first time: 8 transitive security pins (`form-data`, `glob`, `js-yaml`, `koa`, `on-headers`, `tar-fs`, `tmp`, `vite`) alongside `saveExact: true` — which turns out to be the mechanical enforcement behind the "pin-exact devDependency policy" this page has attributed to contributor discipline since 2026-05-26. It was a workspace flag.
- **New finding (release posture):** zero git tags, zero GitHub releases, `version: 0.1.0` across all 16 surveys — against a complete three-target semantic-release pipeline (Marketplace/OpenVSIX/npm), a per-platform emergency rollback workflow with confirmation gate, `release.config.mjs`, three publishing scripts, and `CHANGELOG.md` wired into the published `files[]`. The changelog's sole entry is formatted as semantic-release output and links to `releases/tag/v0.1.0`, which 404s because the tag does not exist. An unexercised release path accrues untested assumptions while Renovate keeps mutating it underneath. Cataloged in [[vscode-extensions]] with mitigation: wire the existing `publish:dry-run` script into CI so packaging and token validation stay exercised; let the release tool write its own changelog rather than seeding a fictional one.
- **Cross-repo corroboration:** `bfra-me/.github` advanced v4.16.x → **v4.23.0**, seven minor boundaries in fifteen days (#525–#528, #531, #533, #535, 08-17 → 08-31). The 2026-08-30 esphome.life survey recorded six-in-eleven-days to v4.22.0; this independently confirms upstream cadence rather than a per-repo artifact. Both callers absorbed it as automerge churn — `bfra-me/*` is deliberately excluded from the `GitHub Actions` grouping rule, so each release arrives as its own PR.
- **Settings-sync working reference established:** extend-vscode's `update-repo-settings.yaml` calls `bfra-me/.github/.github/workflows/update-repo-settings.yaml@eb1772eb` (v4.23.0) — the correct path, same tag as its `renovate.yaml` sibling. This makes the seven-survey esphome.life miswiring repair a literal copy-paste from a sibling repo, not a design exercise. Also noted: cron schedules are per-caller and not inherited (`23 0` here vs `23 12` there), and the correct and broken wirings are visually indistinguishable at review time — the only tell is that the `uses:` basename matches the caller's filename, which is the assertion worth automating. Recorded in [[probot-settings]].
- **Correction (second):** this page has asserted since inception that Probot settings extend `fro-bot/.github:common-settings.yaml`. The actual declaration is a bare `_extends: .github:common-settings.yaml`, which resolves within the declaring repository's **own owner** — `marcusrbrown/.github`. Same misattribution corrected for esphome.life on 2026-07-12. Recorded as a contradiction with both readings preserved; generalized in [[probot-settings]] with a note that other `marcusrbrown/*` pages carrying the old phrasing are suspect until re-verified.
- **Recorded late (present but never enumerated):** `.ai/analysis/` (5 kebab-case migration artifacts — closed work), `.ai/plan/` (8 feature/architecture plan docs — aspirational), `.ai/scripts/` (2 conversion scripts), `.cursor/rules/` (4 `.mdc` best-practice rules). `.ai/plan/feature-advanced-testing-infrastructure-1.md` is the source document for issues #317–#319, open and untouched for ~12.5 months. Same aspirational `.ai/` corpus shape as [[bfra-me--github]] (2026-07-16, 10 plan docs) — two repos arriving independently at a version-controlled plan directory whose contents outlive their execution. Noted for future agents: `.ai/plan/` is intent, never state.
- **Minor:** `main.yaml`'s concurrency group is `${{ github.workflow }}-$${{ github.event.number || github.ref }}` — a stray `$` yielding a literal dollar sign in every group name. Benign (the group still varies per ref) but recorded so it isn't mistaken for interpolation subtlety.
- **Carried forward:** `typescript` remains 5.9.3; #466 autoclosed unmerged 2026-07-11 and Renovate has not re-proposed v6 in ~7 weeks. Zero pending majors in the manifest — the queue is empty because the proposals are gone, not because the work is done. Dependency snapshot at HEAD: eslint 10.9.0, typescript-eslint 8.68.0, `@bfra.me/eslint-config` 0.52.0, Node 24.20.0, `@playwright/test` 1.62.0, prettier 3.9.0.
- Code scanning status **unverified** — the API returned `401` (unauthenticated), inconclusive rather than the `404` that would mean "not enabled." No claim recorded either way.
- **No Fro Bot workflow — sixteenth consecutive survey, ~19 weeks.** Six workflows, all dependency/publish/settings plumbing. Recommendation carried forward and strengthened: this survey produced three fully-specified actionable items (pnpm 10 → 11 with the `allowBuilds` rewrite, the frozen `#5.2.0` preset pin, the unexercised release pipeline) that a resident autoheal daemon would plausibly have surfaced months ago. A follow-up draft PR to onboard the agent should be proposed separately.
- **Maintenance note:** `marcusrbrown--extend-vscode.md` is now ~7,000 words, well past the schema's 500–2000 word guidance, driven entirely by 14 accumulated per-survey Delta Log entries. A future lint pass should consider splitting the pre-2026-07 delta entries into an archive sub-page rather than truncating them — the durable knowledge lives in the narrative sections, but the delta history is what caught the #508 misreading.

Sources: https://github.com/marcusrbrown/extend-vscode (SHA 2a3ec00223b951119cce53664dd8ce03ffa63d05); https://github.com/marcusrbrown/extend-vscode/pull/508 (merge commit e7e222dd, CVE-2026-50021 / GHSA-q6j5-fjx5-2mc3); https://github.com/bfra-me/.github (tag v4.23.0, SHA eb1772ebc71b6b69ee9d8c495afd77bf2fd57da5)

## [2026-08-31 11:47] ingest | repo:marcusrbrown/extend-vscode
