...
## [2026-06-04 09:02] ingest | repo:fro-bot/systematic

Re-surveyed `fro-bot/systematic` (gh-pages HEAD `33cc55a`) and updated the repo page additively. This remains a `gh-pages`-only deployment target — built Starlight/Astro docs + the OCX registry for `@fro.bot/systematic`, served at fro.bot/systematic/. Treated as untrusted input; reads limited to directory listings, the OCX registry/schema JSON, `.well-known/ocx.json`, and commit metadata.

Delta from the 2026-05-22 survey:

- **OCX registry advanced v2.20.6 → v2.24.0** — now matches the latest source release ([[marcusrbrown--systematic]] v2.24.0). Component counts are unchanged: 103 total (51 agents, 47 skills, 2 bundles, 2 profiles, 1 plugin). The v2.21+ launch-surface/release-automation work upstream was content/tooling churn, not component add/remove.
- **New artifacts on `gh-pages`** — `favicon.svg` and a `privacy/index.html` page. `404.html` and `og-image.png` carried over from the prior survey.
- **Repo description set** to "Documentation site for @marcusrbrown/systematic" (previously empty).
- **Unchanged** — schema `$id` (`https://fro.bot/systematic/schemas/v2/systematic-config.schema.json`), draft-07, top-level fields, and `.well-known/ocx.json` (`{"version":1,"registry":"/systematic/index.json"}`). The v2 schema URL remains a public-API-grade pin.
- **Deploy cadence cooled** from the May burst; deploys now track discrete merge windows. Latest deploy 2026-06-04 06:34 from source `9914b6c`.
- **No Fro Bot workflow** — still expected and not recommended; no `.github/` workflows on `gh-pages`. Source repo carries full Fro Bot integration. No follow-up draft PR needed.
- **Open work unchanged** — issue #1 (enable code scanning) and PR #2 (`renovate/configure`) still open. Renovate applicability remains thin given the repo holds only static output.
- No contradictions with prior content.

Modified only `knowledge/wiki/repos/fro-bot--systematic.md`, `knowledge/index.md`, and `knowledge/log.md`.

Sources: https://github.com/fro-bot/systematic (SHA 33cc55af4cdfa7dc3bbaec7ec7717d217c4a3f92)

## [2026-06-04 09:05] ingest | repo:fro-bot/systematic
