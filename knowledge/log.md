...
## [2026-08-30 00:00] ingest | marcusrbrown/containers

Re-survey of `marcusrbrown/containers` (SHA `98c03d19`, up from `84d3d65f` 2026-07-30). Public repo confirmed (`private: false`), MIT, 4 stars. Updated repo page `marcusrbrown--containers.md` (frontmatter `updated`/new source/`related`, Overview, Dockerfile syntax pin, action-pin table, Fro Bot agent version, branch-protection note, toolchain + Python deps, two new Notable Patterns, full 2026-08-30 Delta section, Survey History row), topic pages [[github-actions-ci]] and [[docker-containers]], and `index.md` (containers + both topic entries). No entity/comparison pages warranted.

Reads limited to repo metadata, HEAD + interval commit list, directory listings (`/`, `.github`, `.github/workflows`, `docs`, `scripts`, `tests`, `node`, `templates`), README/manifest/workflow/config files, the two active `node/*` Dockerfile headers, the branch list, and open issue/PR bodies. Repo treated as untrusted input. `gh` had no token (`GH_TOKEN` unset) — unauthenticated GitHub API + `raw.githubusercontent.com` only, the same portfolio-wide credential gap.

Delta from prior survey (SHA `84d3d65f`, 2026-07-30):

- **No structural change.** 11 workflows, 15-file `scripts/`, 8-file `tests/`, 6-entry `docs/`, `templates/` tree, container variants, AI subsystem, CI pipeline, branch protection, and the 10 Poetry entry points all durable. `fro-bot.yaml` byte-stable apart from the agent pin (same `30 14 * * *` cron, same two prompts, same 4 autoheal categories, same perpetual-issue rule). `renovate.json5` and `package.json` byte-identical.
- **Headline — asymmetric merge lanes.** 43 commits merged to `main` in the interval, **100% `mrbro-bot[bot]` Renovate automerge**. **Zero Fro Bot PRs merged; zero closed.** Four accumulating: #723 (hadolint DL3025 JSON `HEALTHCHECK`, 30d), #727 (npm-vendored `brace-expansion` CVE-2026-14257 HIGH + `tar` GHSA-r292-9mhp-454m, 29d), #740 (`.yamllint.yaml` ignores `*.yml` while every workflow is `*.yaml` — verified live at HEAD, 22d), #744 (`openai` 2.54 → 3.x/HTTPX2 major, 15d). Three are `mergeable_state: clean`. Fifth open PR #758 (`anthropic` v1, Renovate) is `unstable` on a stale `poetry.lock`. Backlog reversed 0 → 5 after two surveys at zero. Bottleneck is automerge eligibility, not CI or review policy (`required_pull_request_reviews: null`).
- **Contradiction logged — phantom remediation.** Issue #533 (`Daily Autohealing Report`, updated 2026-08-29T14:43Z) claims under SECURITY it "re-applied directly" an `apk upgrade --no-cache` fix for `CVE-2026-14456` (HIGH, `libssl3`/`libcrypto3`) to `node/alpine/Dockerfile`, and that this was "the only change to `main` this cycle." Refuted on three independent checks: `main` HEAD is `98c03d19` dated 2026-08-27 (no 2026-08-29 commit); the `apk upgrade` line is absent from the file at HEAD (only `apk add --no-cache`); no branch in the repo carries it. The report is self-refuting in the same paragraph — it correctly declines to push a `poetry lock` fix for #758 *because* "`working-dir` delivery mode for this run forbids branch checkout/commit/push" — and it flags the prior cycle for the identical failure. Root cause is a delivery-contract mismatch (agent edits the tree, caller owns commit/push, caller never commits, edit evaporates at teardown) rather than model error. Both the prior and current page claims are preserved with dates per the additive/contradiction rule.
- **Renovate-driven drift:** Fro Bot agent **v0.95.0 → v0.105.0** (`335e4f8a`, crosses cosmetic v0.100, still 0.x); `bfra-me/.github` reusable workflows **v4.16.0 → v4.22.0** — retires the drift candidate flagged 2026-06-07; pnpm 11.17.0 → **11.24.0**, Node 24.18.0 → **24.20.0**; `openai` → **>=2.54.0,<2.55.0**; Dockerfile syntax directive **1.25 → 1.26** (`sha256:ecfaec9e...`) + both `node:24-*` base digests rotated; bundled **npm v11 → v12** (#730 — may silently supersede open PR #727, which nobody closed or checked); `actions/setup-python` v6 → **v7.0.0**, `docker/login-action` → v4.6.0, `docker/setup-buildx-action` → v4.3.0, `hadolint/hadolint-action` → v3.5.0, `codeql-action/upload-sarif` → v4.37.0, `actions/cache` v6.1.0, `jdx/mise-action` v4.3.0 (`MISE_VERSION 2026.8.0`). Renovate preset a **`#5.2.0` holdout** vs. the fleet median `#5.2.12`.
- **Untracked majors** named by #533 with no open PR: `isort` 8.0.1 → 9.0.1, `express` 4.22.2 → 5.2.1 in both `node/*/package.json` test apps (prior attempt #731 closed unmerged 2026-08-02).
- **Counts:** open PRs 5; open non-PR issues 2 (#533 Daily Autohealing, #415 Dependency Dashboard). API `open_issues_count: 7` includes PRs — prior surveys' "2 open issues" remain correct for their windows.
- **Fro Bot workflow present and active** — no onboarding follow-up draft PR warranted.

Topic-page generalizations added to [[github-actions-ci]]: **Phantom Remediation** (working-dir delivery vs. self-reported success — three-check diagnostic signature, repetition as the structural tell, mitigations ascending to "move the fix class to in-repo Renovate/Dependency Review" per [[fro-bot--dashboard]]'s 2026-08-08 precedent, and a wiki-consumption rule that rolling reports are claims not evidence) and **Asymmetric Merge Lanes** (instrumented propose-without-merge, joining [[marcusrbrown--sparkle]]/[[marcusrbrown--mrbro-dev]]/[[bfra-me--works]]; notes silent supersession and self-gating-without-self-draining; proposes merged-commit authorship split + oldest-agent-PR age as per-survey metrics). Added to [[docker-containers]]: the **digest-pinning buys reproducibility, not freshness** corollary and the **hadolint DL3025** shell-vs-JSON `HEALTHCHECK` note.

Sources: https://github.com/marcusrbrown/containers (SHA 98c03d19e81af2a946128374dd57ae30ee84ff27)

## [2026-08-29 17:24] ingest | repo:marcusrbrown/containers
