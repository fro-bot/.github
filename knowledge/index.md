...
- [[fro-bot--agent]] — GitHub Action harness for OpenCode + oMo agents with persistent session state; core runtime powering Fro Bot's PR review, issue triage, scheduled maintenance, and wiki-update across all managed repos. v0.51.0: 4-member pnpm workspace (action + new `workspace-agent` Hono sandbox service + runtime + Discord `gateway`); gateway now a working Discord control plane (bindings, GitHub App auth, `/fro-bot add-project`, mention→OpenCode execution, sensitive-tool approval prompts); OMO Slim opt-in orchestration (`enable-omo-slim`, mutually exclusive with `enable-omo`); expanded S3/KMS inputs; shared constants relocated to `@fro-bot/runtime`; open regression #741 (mitmproxy egress 502)
- [[fro-bot--fro-bot-github-io]] — fro-bot/fro-bot.github.io
- [[fro-bot--systematic]] — Built docs + OCX registry deployment target for `@fro.bot/systematic` at fro.bot/systematic/; `gh-pages`-only repo (no Fro Bot workflow needed); also hosts the pinned JSON Schema for `systematic.json` user config at `/schemas/v2/` (treat as public API); registry advanced to v2.24.0 (matches latest source release), 103 components unchanged (51 agents, 47 skills, 2 bundles, 2 profiles, 1 plugin); added `favicon.svg` + `privacy/` page and a repo description as of 2026-06-04
