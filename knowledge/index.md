...
- [[fro-bot--systematic]] — Built docs + OCX registry deployment target for `@fro.bot/systematic` at fro.bot/systematic/; `gh-pages`-only repo (no Fro Bot workflow needed); also hosts the pinned JSON Schema for `systematic.json` user config at `/schemas/v2/`; 2026-06-14: registry advanced to v2.31.0 (matches latest source release) with 104 components (51 agents, 48 skills, 2 bundles, 2 profiles, 1 plugin); schema label moved `title`→`description` and `schemas/latest` `$id` now points at the v2 URL (latest ≡ v2)
