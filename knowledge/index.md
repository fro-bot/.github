...
- [[marcusrbrown--extend-vscode]] — VS Code extension toolkit (TypeScript, dual Node/Web targets, tsup, Vitest, semantic-release to Marketplace+OpenVSIX+npm); steady Renovate cadence (pnpm 10.34.0, `@types/vscode` 1.120.0, `type-fest` 5.7.0, security patch `tmp` v0.2.6); `esbuild-plugin-polyfill-node` now explicit manifest entry confirming deliberate Node polyfill in web build; `typescript` v6 (#466) still the sole pending major; **still no Fro Bot agent workflow** (~7 weeks open)
